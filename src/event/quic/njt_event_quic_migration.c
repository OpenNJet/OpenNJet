    ...
    njt_quic_header_t *pkt, njt_quic_path_challenge_frame_t *f)
{
     ...
     */
    if (njt_quic_frame_sendto(c, &frame, 1200, pkt->path) != NJT_OK) {
        return NJT_ERROR;
    }
...
njt_quic_send_path_challenge(njt_connection_t *c, njt_quic_path_t *path)
{
    ...
    if (njt_quic_frame_sendto(c, &frame, 1200, path) != NJT_OK) {
        return NJT_ERROR;
    }
...
njt_quic_path_validation_handler(njt_event_t *ev)
{
    njt_msec_t              now;
    njt_queue_t            *q;
    njt_msec_int_t          left, next, pto;
    njt_quic_path_t        *path, *bkp;
    njt_connection_t       *c;
    njt_quic_send_ctx_t    *ctx;
    njt_quic_connection_t  *qc;
    ...
    while (q != njt_queue_sentinel(&qc->paths)) {
        ...
        if (njt_quic_free_path(c, path) != NJT_OK) {
            njt_quic_close_connection(c, NJT_ERROR);
            return;
        }
    }
