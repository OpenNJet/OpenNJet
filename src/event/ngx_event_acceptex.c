...
ngx_event_acceptex(ngx_event_t *rev)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_EVENT, c->log, 0, "AcceptEx: %d", c->fd);

    if (rev->ovlp.error) {
        ngx_log_error(NJT_LOG_CRIT, c->log, rev->ovlp.error,
...
ngx_event_post_acceptex(ngx_listening_t *ls, ngx_uint_t n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        ngx_log_debug1(NJT_LOG_DEBUG_EVENT, &ls->log, 0,
                       ngx_socket_n " s:%d", s);

        if (s == (ngx_socket_t) -1) {
            ngx_log_error(NJT_LOG_ALERT, &ls->log, ngx_socket_errno,
                          ngx_socket_n " failed");
        ...
        *log = ls->log;
        ...
        if (ngx_add_event(rev, 0, NJT_IOCP_IO) == NJT_ERROR) {
            ngx_close_posted_connection(c);
            return NJT_ERROR;
        }
            ...
            == 0)
        {
            ...
            if (err != WSA_IO_PENDING) {
                ngx_log_error(NJT_LOG_ALERT, &ls->log, err,
                              "AcceptEx() %V failed", &ls->addr_text);
