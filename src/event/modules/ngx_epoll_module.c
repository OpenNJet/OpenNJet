...
#if (NJET_HAVE_EVENTFD)
        if (ngx_epoll_notify_init(cycle->log) != NJET_OK) {
...
#if (NJET_HAVE_CLEAR_EVENT)
    ngx_event_flags = NJET_USE_CLEAR_EVENT
#else
    ngx_event_flags = NJET_USE_LEVEL_EVENT
#endif
                      |NJET_USE_GREEDY_EVENT
                      |NJET_USE_EPOLL_EVENT;
...
ngx_epoll_notify_init(ngx_log_t *log)
{
    ...
    if (notify_fd == -1) {
        ngx_log_error(NJET_LOG_EMERG, log, ngx_errno, "eventfd() failed");
        return NJET_ERROR;
    }

    ngx_log_debug1(NJET_LOG_DEBUG_EVENT, log, 0,
...
ngx_epoll_notify_handler(ngx_event_t *ev)
{
    ...
    if (++ev->index == NJET_MAX_UINT32_VALUE) {
        ev->index = 0;
        ...
        ngx_log_debug3(NJET_LOG_DEBUG_EVENT, ev->log, 0,
                       "read() eventfd %d: %z count:%uL", notify_fd, n, count);

        if ((size_t) n != sizeof(uint64_t)) {
            ngx_log_error(NJET_LOG_ALERT, ev->log, err,
...
ngx_epoll_test_rdhup(ngx_cycle_t *cycle)
{
    ...
    if (events) {
        ...
        ngx_log_error(NJET_LOG_ALERT, cycle->log, NJET_ETIMEDOUT,
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (event == NJET_READ_EVENT) {
        e = c->write;
        prev = EPOLLOUT;
#if (NJET_READ_EVENT != EPOLLIN|EPOLLRDHUP)
...
#if (NJET_HAVE_EPOLLEXCLUSIVE && NJET_HAVE_EPOLLRDHUP)
    if (flags & NJET_EXCLUSIVE_EVENT) {
        events &= ~EPOLLRDHUP;
    }
#endif
    ...
    ev->active = 1;
#if 0
    ev->oneshot = (flags & NJET_ONESHOT_EVENT) ? 1 : 0;
#endif
...
ngx_epoll_add_connection(ngx_connection_t *c)
{
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, c->fd, &ee) == -1) {
        ngx_log_error(NJET_LOG_ALERT, c->log, ngx_errno,
...
ngx_epoll_notify(ngx_event_handler_pt handler)
{
    ...
    if ((size_t) write(notify_fd, &inc, sizeof(uint64_t)) != sizeof(uint64_t)) {
        ngx_log_error(NJET_LOG_ALERT, notify_event.log, ngx_errno,
...
ngx_epoll_eventfd_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_EVENT, ev->log, 0, "eventfd handler");

    n = read(ngx_eventfd, &ready, 8);

    err = ngx_errno;

    ngx_log_debug1(NJET_LOG_DEBUG_EVENT, ev->log, 0, "eventfd: %d", n);
    ...
    if (n != 8) {
        if (n == -1) {
            ...
            ngx_log_error(NJET_LOG_ALERT, ev->log, err, "read(eventfd) failed");
            return;
        }
    ...
    while (ready) {
        ...
        if (events > 0) {
            ...
            for (i = 0; i < events; i++) {
                ...
                ngx_log_debug4(NJET_LOG_DEBUG_EVENT, ev->log, 0,
                               "io_event: %XL %XL %L %L",
...
ngx_epoll_create_conf(ngx_cycle_t *cycle)
{
    ...
    epcf->events = NJET_CONF_UNSET;
    epcf->aio_requests = NJET_CONF_UNSET;
