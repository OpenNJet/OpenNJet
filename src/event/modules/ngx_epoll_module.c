...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
...
#if (NJT_HAVE_EVENTFD)
        if (ngx_epoll_notify_init(cycle->log) != NJT_OK) {
    ...
    ngx_event_flags = NJT_USE_CLEAR_EVENT
#else
    ngx_event_flags = NJT_USE_LEVEL_EVENT
#endif
...
ngx_epoll_notify_init(ngx_log_t *log)
{
    ...
    if (notify_fd == -1) {
        ngx_log_error(NJT_LOG_EMERG, log, ngx_errno, "eventfd() failed");
        return NJT_ERROR;
    }

    ngx_log_debug1(NJT_LOG_DEBUG_EVENT, log, 0,
...
ngx_epoll_notify_handler(ngx_event_t *ev)
{
    ...
    if (++ev->index == NJT_MAX_UINT32_VALUE) {
        ...
        ngx_log_debug3(NJT_LOG_DEBUG_EVENT, ev->log, 0,
                       "read() eventfd %d: %z count:%uL", notify_fd, n, count);

        if ((size_t) n != sizeof(uint64_t)) {
            ngx_log_error(NJT_LOG_ALERT, ev->log, err,
...
ngx_epoll_test_rdhup(ngx_cycle_t *cycle)
{
    ...
    if (events) {
        ...
        ngx_log_error(NJT_LOG_ALERT, cycle->log, NJT_ETIMEDOUT,
...
ngx_epoll_add_connection(ngx_connection_t *c)
{
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, c->fd, &ee) == -1) {
        ngx_log_error(NJT_LOG_ALERT, c->log, ngx_errno,
...
ngx_epoll_notify(ngx_event_handler_pt handler)
{
    ...
    if ((size_t) write(notify_fd, &inc, sizeof(uint64_t)) != sizeof(uint64_t)) {
        ngx_log_error(NJT_LOG_ALERT, notify_event.log, ngx_errno,
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    if (flags & NJT_UPDATE_TIME || ngx_event_timer_alarm) {
        ngx_time_update();
    }
...
ngx_epoll_eventfd_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_EVENT, ev->log, 0, "eventfd handler");

    n = read(ngx_eventfd, &ready, 8);

    err = ngx_errno;

    ngx_log_debug1(NJT_LOG_DEBUG_EVENT, ev->log, 0, "eventfd: %d", n);
    ...
    if (n != 8) {
        if (n == -1) {
            ...
            ngx_log_error(NJT_LOG_ALERT, ev->log, err, "read(eventfd) failed");
            return;
        }

        ngx_log_error(NJT_LOG_ALERT, ev->log, 0,
    ...
    while (ready) {
        ...
        ngx_log_debug1(NJT_LOG_DEBUG_EVENT, ev->log, 0,
        ...
        if (events > 0) {
            ...
            for (i = 0; i < events; i++) {
                ...
                ngx_log_debug4(NJT_LOG_DEBUG_EVENT, ev->log, 0,
                               "io_event: %XL %XL %L %L",
