...
ngx_poll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_EVENT, cycle->log, 0, "poll timer: %M", timer);
    ...
    if (err) {
        ngx_log_error(NJT_LOG_ALERT, cycle->log, err, "WSAPoll() failed");
        return NJT_ERROR;
    }
    ...
    for (i = 0; i < nevents && ready; i++) {
        ...
        if (c->fd == (ngx_socket_t) -1) {
            ngx_log_error(NJT_LOG_ALERT, cycle->log, 0, "unexpected event");
    ...
    if (ready != 0) {
        ngx_log_error(NJT_LOG_ALERT, cycle->log, 0, "poll ready != events");
    }
