...
ngx_iocp_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    ngx_event_flags = NJT_USE_IOCP_EVENT;
...
ngx_iocp_timer(void *data)
{
    ...
    for ( ;; ) {
        ...
        ngx_time_update();
#if 1
        ngx_log_debug0(NJT_LOG_DEBUG_EVENT, ngx_cycle->log, 0, "timer");
#endif
    }
...
ngx_iocp_del_connection(ngx_connection_t *c, ngx_uint_t flags)
{
    ...
    if (CancelIo((HANDLE) c->fd) == 0) {
        ngx_log_error(NJT_LOG_ALERT, c->log, ngx_errno, "CancelIo() failed");
        return NJT_ERROR;
    }
#endif
    ...
    ngx_uint_t flags)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_EVENT, cycle->log, 0, "iocp timer: %M", timer);
    ...
    ngx_log_debug4(NJT_LOG_DEBUG_EVENT, cycle->log, 0,
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_EVENT, cycle->log, err, "iocp event:%p", ev);
        ...
        || err == ERROR_OPERATION_ABORTED /* the operation was canceled */)
    {
        ...
        ngx_log_debug1(NJT_LOG_DEBUG_EVENT, cycle->log, err,
