...
ngx_kqueue_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
...
#ifdef EVFILT_USER
        if (ngx_kqueue_notify_init(cycle->log) != NJT_OK) {
            return NJT_ERROR;
        }
#endif
    }
    ...
    ngx_event_flags = NJT_USE_ONESHOT_EVENT
    ...
    if (timer) {
        ...
        ngx_event_flags |= NJT_USE_TIMER_EVENT;
    }
    ...
    ngx_event_flags |= NJT_USE_LOWAT_EVENT;
#endif
...
ngx_kqueue_set_event(ngx_event_t *ev, ngx_int_t filter, ngx_uint_t flags)
{
    ...
    if (nchanges >= max_changes) {
        ngx_log_error(NJT_LOG_WARN, ev->log, 0,
        ...
        if (kevent(ngx_kqueue, change_list, (int) nchanges, NULL, 0, &ts)
            == -1)
        {
            ngx_log_error(NJT_LOG_ALERT, ev->log, ngx_errno, "kevent() failed");
            return NJT_ERROR;
        }
    ...
    if (flags & NJT_FLUSH_EVENT) {
        ...
        ngx_log_debug0(NJT_LOG_DEBUG_EVENT, ev->log, 0, "kevent flush");
    ...
    ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
        ...
        if (event_list[i].flags & EV_ERROR) {
            ngx_log_error(NJT_LOG_ALERT, cycle->log, event_list[i].data,
...
ngx_kqueue_dump_event(ngx_log_t *log, struct kevent *kev)
{
    if (kev->ident > 0x8000000 && kev->ident != (unsigned) -1) {
        ngx_log_debug6(NJT_LOG_DEBUG_EVENT, log, 0,
