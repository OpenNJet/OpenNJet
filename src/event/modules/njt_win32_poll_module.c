...
static njt_connection_t  **event_index;
...
njt_poll_init(njt_cycle_t *cycle, njt_msec_t timer)
{
        ...
        || cycle->old_cycle->connection_n < cycle->connection_n)
    {
        ...
        index = njt_alloc(sizeof(njt_connection_t *) * cycle->connection_n,
                          cycle->log);
        if (index == NULL) {
            return NJT_ERROR;
        }
        ...
        if (event_index) {
            njt_memcpy(index, event_index,
                       sizeof(njt_connection_t *) * nevents);
...
njt_poll_process_events(njt_cycle_t *cycle, njt_msec_t timer, njt_uint_t flags)
{
    ...
    if (err) {
        njt_log_error(NJT_LOG_ALERT, cycle->log, err, "WSAPoll() failed");
        return NJT_ERROR;
    }
    ...
    for (i = 0; i < nevents && ready; i++) {
        ...
        if (event_list[i].fd == (njt_socket_t) -1) {
            /*
        ...
        if (c->fd == (njt_socket_t) -1) {
            ...
            if (i == nevents - 1) {
                ...
                event_list[i].fd = (njt_socket_t) -1;
            }
