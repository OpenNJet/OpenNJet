...
ngx_select_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if ((event == NJET_READ_EVENT && ev->write)
        || (event == NJET_WRITE_EVENT && !ev->write))
    {
        ngx_log_error(NJET_LOG_ALERT, ev->log, 0,
    ...
    if ((event == NJET_READ_EVENT && max_read >= FD_SETSIZE)
        || (event == NJET_WRITE_EVENT && max_write >= FD_SETSIZE))
    {
        ngx_log_error(NJET_LOG_ERR, ev->log, 0,
    ...
    } else if (event == NJET_WRITE_EVENT) {
    ...
    ngx_uint_t flags)
{
    ...
    if (err) {
        ngx_log_error(NJET_LOG_ALERT, cycle->log, err, "select() failed");
