...
static ngx_connection_t  pending_connects[NJT_MAX_PENDING_CONN];
...
int ngx_iocp_wait_connect(ngx_connection_t *c)
{
    for ( ;; ) {
        ...
        LeaveCriticalSection(&connect_lock);
        ngx_log_error(NJT_LOG_NOTICE, c->log, 0,
    ...
    if (!started) {
        if (ngx_iocp_new_thread(1) == NJT_ERROR) {
            return NJT_ERROR;
        }
        started = 1;
    }
...
void ngx_iocp_wait_events(int main)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_error(NJT_LOG_ALERT, c[n].log, 0,
                      "WSAWaitForMultipleEvents() "
