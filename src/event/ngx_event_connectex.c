...
#define NJET_MAX_PENDING_CONN  10


static CRITICAL_SECTION  connect_lock;
static int               nconnects;
static ngx_connection_t  pending_connects[NJET_MAX_PENDING_CONN];
...
int ngx_iocp_wait_connect(ngx_connection_t *c)
{
    for ( ;; ) {
        ...
        if (nconnects < NJET_MAX_PENDING_CONN) {
        ...
        LeaveCriticalSection(&connect_lock);
        ngx_log_error(NJET_LOG_NOTICE, c->log, 0,
                      "max number of pending connect()s is %d",
                      NJET_MAX_PENDING_CONN);
        msleep(100);
    }

    if (!started) {
        if (ngx_iocp_new_thread(1) == NJET_ERROR) {
            return NJET_ERROR;
        }
        started = 1;
    }
...
void ngx_iocp_wait_events(int main)
{
    ...
    for ( ;; ) {
        ...
        if (ne.lNetworkEvents & FD_CONNECT) {
            ...
            if (PostQueuedCompletionStatus(iocp, 0, NJET_IOCP_CONNECT,
                                           &conn[n].write->ovlp) == 0)
            {
                ngx_log_error(NJET_LOG_ALERT, log, ngx_socket_errno,
        ...
        ngx_log_error(NJET_LOG_ALERT, c[n].log, 0,
                      "WSAWaitForMultipleEvents() "
