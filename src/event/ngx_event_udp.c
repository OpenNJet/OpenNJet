...
ngx_event_recvmsg(ngx_event_t *ev)
{
    ...
    do {
        ...
        if (n == -1) {
            ...
            ngx_log_error(NJT_LOG_ALERT, ev->log, err, "recvmsg() failed");
        ...
        if (ls->wildcard) {
            ...
            {
                if (ngx_get_srcaddr_cmsg(cmsg, local_sockaddr) == NJT_OK) {
                    break;
                }
            }
        }
        ...
        *log = ls->log;
        ...
        if (ngx_insert_udp_connection(c) != NJT_OK) {
            ngx_close_accepted_udp_connection(c);
            return;
        }
    ...
    socklen_t socklen, struct sockaddr *local_sockaddr, socklen_t local_socklen)
{
    ...
    if (sockaddr->sa_family == AF_UNIX) {
        ...
        if (socklen <= (socklen_t) offsetof(struct sockaddr_un, sun_path)
            || saun->sun_path[0] == '\0')
        {
            ngx_log_debug0(NJT_LOG_DEBUG_EVENT, ngx_cycle->log, 0,
                           "unbound unix socket");
            return NULL;
        }
    }
