...
ngx_event_accept(ngx_event_t *ev)
{
    ...
    do {
        ...
        if (s == (ngx_socket_t) -1) {
            ...
            if (err == NJET_ECONNABORTED) {
                level = NJET_LOG_ERR;

            } else if (err == NJET_EMFILE || err == NJET_ENFILE) {
                level = NJET_LOG_CRIT;
            }
            ...
            if (use_accept4 && err == NJET_ENOSYS) {
                use_accept4 = 0;
            ...
            if (err == NJET_EMFILE || err == NJET_ENFILE) {
        ...
        if (c == NULL) {
            if (ngx_close_socket(s) == -1) {
                ngx_log_error(NJET_LOG_ALERT, ev->log, ngx_socket_errno,
        ...
        *log = ls->log;
        ...
        if (ev->deferred_accept) {
            rev->ready = 1;
#if (NJET_HAVE_KQUEUE || NJET_HAVE_EPOLLRDHUP)
            rev->available = 1;
#endif
        }
        ...
        if (log->log_level & NJET_LOG_DEBUG_EVENT) {
            ...
            ngx_log_debug3(NJET_LOG_DEBUG_EVENT, log, 0,
...
ngx_trylock_accept_mutex(ngx_cycle_t *cycle)
{
    if (ngx_shmtx_trylock(&ngx_accept_mutex)) {
        ...
        ngx_log_debug0(NJET_LOG_DEBUG_EVENT, cycle->log, 0,
                       "accept mutex locked");
        ...
        if (ngx_enable_accept_events(cycle) == NJET_ERROR) {
            ngx_shmtx_unlock(&ngx_accept_mutex);
            return NJET_ERROR;
        }
    ...
    if (ngx_accept_mutex_held) {
        if (ngx_disable_accept_events(cycle, 0) == NJET_ERROR) {
            return NJET_ERROR;
        }
...
ngx_enable_accept_events(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_add_event(c->read, NJET_READ_EVENT, 0) == NJET_ERROR) {
            return NJET_ERROR;
        }
    }
...
ngx_disable_accept_events(ngx_cycle_t *cycle, ngx_uint_t all)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_del_event(c->read, NJET_READ_EVENT, NJET_DISABLE_EVENT)
            == NJET_ERROR)
        {
            return NJET_ERROR;
        }
...
ngx_reorder_accept_events(ngx_listening_t *ls)
{
    ...
    if (ngx_add_event(c->read, NJET_READ_EVENT, NJET_EXCLUSIVE_EVENT)
        == NJET_ERROR)
    {
        return;
    }
...
ngx_debug_accepted_connection(ngx_event_conf_t *ecf, ngx_connection_t *c)
{
    ...
    for (i = 0; i < ecf->debug_connection.nelts; i++) {
        ...
        c->log->log_level = NJET_LOG_DEBUG_CONNECTION|NJET_LOG_DEBUG_ALL;
        break;
