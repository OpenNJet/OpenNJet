...
ngx_event_accept(ngx_event_t *ev)
{
    ...
    if (ev->timedout) {
        if (ngx_enable_accept_events((ngx_cycle_t *) ngx_cycle) != NJT_OK) {
            return;
        }
    ...
    do {
        ...
        if (s == (ngx_socket_t) -1) {
            ...
            if (err == NJT_EAGAIN) {
                ngx_log_debug0(NJT_LOG_DEBUG_EVENT, ev->log, err,
                               "accept() not ready");
                return;
            }
        ...
        if (c == NULL) {
            if (ngx_close_socket(s) == -1) {
                ngx_log_error(NJT_LOG_ALERT, ev->log, ngx_socket_errno,
        ...
        *log = ls->log;
        ...
        if (log->log_level & NJT_LOG_DEBUG_EVENT) {
            ...
            ngx_log_debug3(NJT_LOG_DEBUG_EVENT, log, 0,
        ...
        if (ngx_add_conn && (ngx_event_flags & NJT_USE_EPOLL_EVENT) == 0) {
...
ngx_trylock_accept_mutex(ngx_cycle_t *cycle)
{
    if (ngx_shmtx_trylock(&ngx_accept_mutex)) {
        ...
        ngx_log_debug0(NJT_LOG_DEBUG_EVENT, cycle->log, 0,
                       "accept mutex locked");
        ...
        if (ngx_enable_accept_events(cycle) == NJT_ERROR) {
            ngx_shmtx_unlock(&ngx_accept_mutex);
            return NJT_ERROR;
        }
    ...
    if (ngx_accept_mutex_held) {
        if (ngx_disable_accept_events(cycle, 0) == NJT_ERROR) {
            return NJT_ERROR;
        }
...
ngx_enable_accept_events(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_add_event(c->read, NJT_READ_EVENT, 0) == NJT_ERROR) {
            return NJT_ERROR;
        }
    }
...
ngx_disable_accept_events(ngx_cycle_t *cycle, ngx_uint_t all)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_del_event(c->read, NJT_READ_EVENT, NJT_DISABLE_EVENT)
            == NJT_ERROR)
        {
            return NJT_ERROR;
        }
...
ngx_reorder_accept_events(ngx_listening_t *ls)
{
    ...
    if (ngx_add_event(c->read, NJT_READ_EVENT, NJT_EXCLUSIVE_EVENT)
        == NJT_ERROR)
    {
        return;
    }
...
ngx_close_accepted_connection(ngx_connection_t *c)
{
    ...
    if (ngx_close_socket(fd) == -1) {
        ngx_log_error(NJT_LOG_ALERT, c->log, ngx_socket_errno,
                      ngx_close_socket_n " failed");
    }
