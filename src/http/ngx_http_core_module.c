...
static ngx_conf_enum_t  ngx_http_core_request_body_in_file[] = {
    { ngx_string("off"), NJT_HTTP_REQUEST_BODY_FILE_OFF },
    { ngx_string("on"), NJT_HTTP_REQUEST_BODY_FILE_ON },
    { ngx_string("clean"), NJT_HTTP_REQUEST_BODY_FILE_CLEAN },
    { ngx_null_string, 0 }
};


static ngx_conf_enum_t  ngx_http_core_satisfy[] = {
    { ngx_string("all"), NJT_HTTP_SATISFY_ALL },
    { ngx_string("any"), NJT_HTTP_SATISFY_ANY },
    { ngx_null_string, 0 }
};


static ngx_conf_enum_t  ngx_http_core_lingering_close[] = {
    { ngx_string("off"), NJT_HTTP_LINGERING_OFF },
    { ngx_string("on"), NJT_HTTP_LINGERING_ON },
    { ngx_string("always"), NJT_HTTP_LINGERING_ALWAYS },
    { ngx_null_string, 0 }
};


static ngx_conf_enum_t  ngx_http_core_server_tokens[] = {
    { ngx_string("off"), NJT_HTTP_SERVER_TOKENS_OFF },
    { ngx_string("on"), NJT_HTTP_SERVER_TOKENS_ON },
    { ngx_string("build"), NJT_HTTP_SERVER_TOKENS_BUILD },
    { ngx_null_string, 0 }
};


static ngx_conf_enum_t  ngx_http_core_if_modified_since[] = {
    { ngx_string("off"), NJT_HTTP_IMS_OFF },
    { ngx_string("exact"), NJT_HTTP_IMS_EXACT },
    { ngx_string("before"), NJT_HTTP_IMS_BEFORE },
    { ngx_null_string, 0 }
};


static ngx_conf_bitmask_t  ngx_http_core_keepalive_disable[] = {
    { ngx_string("none"), NJT_HTTP_KEEPALIVE_DISABLE_NONE },
    { ngx_string("msie6"), NJT_HTTP_KEEPALIVE_DISABLE_MSIE6 },
    { ngx_string("safari"), NJT_HTTP_KEEPALIVE_DISABLE_SAFARI },
    { ngx_null_string, 0 }
};


static ngx_path_init_t  ngx_http_client_temp_path = {
    ngx_string(NJT_HTTP_CLIENT_TEMP_PATH), { 0, 0, 0 }
};
...
static ngx_conf_enum_t  ngx_http_gzip_http_version[] = {
    { ngx_string("1.0"), NJT_HTTP_VERSION_10 },
    { ngx_string("1.1"), NJT_HTTP_VERSION_11 },
    { ngx_null_string, 0 }
};


static ngx_conf_bitmask_t  ngx_http_gzip_proxied_mask[] = {
    { ngx_string("off"), NJT_HTTP_GZIP_PROXIED_OFF },
    { ngx_string("expired"), NJT_HTTP_GZIP_PROXIED_EXPIRED },
    { ngx_string("no-cache"), NJT_HTTP_GZIP_PROXIED_NO_CACHE },
    { ngx_string("no-store"), NJT_HTTP_GZIP_PROXIED_NO_STORE },
    { ngx_string("private"), NJT_HTTP_GZIP_PROXIED_PRIVATE },
    { ngx_string("no_last_modified"), NJT_HTTP_GZIP_PROXIED_NO_LM },
    { ngx_string("no_etag"), NJT_HTTP_GZIP_PROXIED_NO_ETAG },
    { ngx_string("auth"), NJT_HTTP_GZIP_PROXIED_AUTH },
    { ngx_string("any"), NJT_HTTP_GZIP_PROXIED_ANY },
    { ngx_null_string, 0 }
};
    ...
    ngx_http_phase_handler_t *ph)
{
        ...
        && clcf->client_max_body_size < r->headers_in.content_length_n)
    {
        ...
        ngx_http_finalize_request(r, NJT_HTTP_REQUEST_ENTITY_TOO_LARGE);
        return NJT_OK;
    }
    ...
    if (rc == NJT_DONE) {
        ...
        ngx_http_finalize_request(r, NJT_HTTP_MOVED_PERMANENTLY);
        return NJT_OK;
    }
...
ngx_http_core_auth_delay(ngx_http_request_t *r)
{
    ...
    if (clcf->auth_delay == 0) {
        ngx_http_finalize_request(r, NJT_HTTP_UNAUTHORIZED);
        return NJT_OK;
    }
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    if (r->uri.data[r->uri.len - 1] == '/') {
        ...
        ngx_http_finalize_request(r, NJT_HTTP_FORBIDDEN);
        return NJT_OK;
    }

    ngx_log_error(NJT_LOG_ERR, r->connection->log, 0, "no handler found");
...
ngx_http_update_location_config(ngx_http_request_t *r)
{
    ...
    if ((ngx_io.flags & NJT_IO_SENDFILE) && clcf->sendfile) {
...
ngx_http_set_etag(ngx_http_request_t *r)
{
    ...
    etag->value.data = ngx_pnalloc(r->pool, NJT_OFF_T_LEN + NJT_TIME_T_LEN + 3);
    ...
    ngx_str_t *ct, ngx_http_complex_value_t *cv)
{
    ...
    if (ngx_http_complex_value(r, cv, &val) != NJT_OK) {
        return NJT_HTTP_INTERNAL_SERVER_ERROR;
    }
...
ngx_http_auth_basic_user(ngx_http_request_t *r)
{
    ...
    if (ngx_decode_base64(&auth, &encoded) != NJT_OK) {
        r->headers_in.user.data = (u_char *) "";
        return NJT_DECLINED;
    }
...
ngx_http_gzip_ok(ngx_http_request_t *r)
{
    ...
    if (ngx_memcmp(ae->value.data, "gzip,", 5) != 0
        && ngx_http_gzip_accept_encoding(&ae->value) != NJT_OK)
    {
        return NJT_DECLINED;
    }
    ...
    u_char *xff, size_t xfflen, ngx_array_t *proxies, int recursive)
{
    ...
    do {
        ...
        if (ngx_cidr_match(addr->sockaddr, proxies) != NJT_OK) {
...
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    *cf = pcf;
    ...
    if (rv == NJT_CONF_OK && !cscf->listen) {
        ...
        if (ngx_http_add_listen(cf, cscf, &lsopt) != NJT_OK) {
            return NJT_CONF_ERROR;
        }
    }
...
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    if (cf->args->nelts == 3) {
        ...
        if (len == 1 && mod[0] == '=') {
            ...
            if (ngx_http_core_regex_location(cf, clcf, name, 0) != NJT_OK) {
                return NJT_CONF_ERROR;
            }

        } else if (len == 2 && mod[0] == '~' && mod[1] == '*') {

            if (ngx_http_core_regex_location(cf, clcf, name, 1) != NJT_OK) {
                return NJT_CONF_ERROR;
            }
    ...
    if (ngx_http_add_location(cf, &pclcf->locations, clcf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
...
ngx_http_core_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->root.data == NULL) {
        ...
        if (prev->root.data == NULL) {
            ...
            if (ngx_conf_full_name(cf->cycle, &conf->root, 0) != NJT_OK) {
                return NJT_CONF_ERROR;
            }
        }
    }
    ...
    ngx_conf_merge_size_value(conf->subrequest_output_buffer_size,
                              ...
                              (size_t) ngx_pagesize);
    ngx_conf_merge_value(conf->aio, prev->aio, NJT_HTTP_AIO_OFF);
...
ngx_http_core_root(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (clcf->root.data[0] != '$') {
        if (ngx_conf_full_name(cf->cycle, &clcf->root, 0) != NJT_OK) {
            return NJT_CONF_ERROR;
        }
    }
...
ngx_http_core_keepalive(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (clcf->keepalive_timeout == (ngx_msec_t) NJT_ERROR) {
        return "invalid value";
    }
