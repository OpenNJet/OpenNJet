...
#define njt_str3_cmp(m, c0, c1, c2, c3)                                       \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)

#define njt_str3Ocmp(m, c0, c1, c2, c3)                                       \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)

#define njt_str4cmp(m, c0, c1, c2, c3)                                        \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)

#define njt_str5cmp(m, c0, c1, c2, c3, c4)                                    \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)             \
        && m[4] == c4

#define njt_str6cmp(m, c0, c1, c2, c3, c4, c5)                                \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)             \
        && (((uint32_t *) m)[1] & 0xffff) == ((c5 << 8) | c4)

#define njt_str7_cmp(m, c0, c1, c2, c3, c4, c5, c6, c7)                       \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)             \
        && ((uint32_t *) m)[1] == ((c7 << 24) | (c6 << 16) | (c5 << 8) | c4)

#define njt_str8cmp(m, c0, c1, c2, c3, c4, c5, c6, c7)                        \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)             \
        && ((uint32_t *) m)[1] == ((c7 << 24) | (c6 << 16) | (c5 << 8) | c4)

#define njt_str9cmp(m, c0, c1, c2, c3, c4, c5, c6, c7, c8)                    \
    *(uint32_t *) m == ((c3 << 24) | (c2 << 16) | (c1 << 8) | c0)             \
        && ((uint32_t *) m)[1] == ((c7 << 24) | (c6 << 16) | (c5 << 8) | c4)  \
        && m[8] == c8
...
njt_int_t
njt_http_parse_request_line(njt_http_request_t *r, njt_buf_t *b)
{
    u_char  c, ch, *p, *m;
    enum {
        sw_start = 0,
    ...
    for (p = b->pos; p < b->last; p++) {
        ...
        case sw_method:
            if (ch == ' ') {
                ...
                case 3:
                    if (njt_str3_cmp(m, 'G', 'E', 'T', ' ')) {
                        r->method = NJT_HTTP_GET;
                        break;
                    }

                    if (njt_str3_cmp(m, 'P', 'U', 'T', ' ')) {
                ...
                case 4:
                    if (m[1] == 'O') {
                        ...
                        if (njt_str3Ocmp(m, 'P', 'O', 'S', 'T')) {
                            r->method = NJT_HTTP_POST;
                            break;
                        }

                        if (njt_str3Ocmp(m, 'C', 'O', 'P', 'Y')) {
                            r->method = NJT_HTTP_COPY;
                            break;
                        }

                        if (njt_str3Ocmp(m, 'M', 'O', 'V', 'E')) {
                            r->method = NJT_HTTP_MOVE;
                            break;
                        }

                        if (njt_str3Ocmp(m, 'L', 'O', 'C', 'K')) {
                ...
                case 5:
                    if (njt_str5cmp(m, 'M', 'K', 'C', 'O', 'L')) {
                        r->method = NJT_HTTP_MKCOL;
                        break;
                    }

                    if (njt_str5cmp(m, 'P', 'A', 'T', 'C', 'H')) {
                        r->method = NJT_HTTP_PATCH;
                        break;
                    }

                    if (njt_str5cmp(m, 'T', 'R', 'A', 'C', 'E')) {
                ...
                case 6:
                    if (njt_str6cmp(m, 'D', 'E', 'L', 'E', 'T', 'E')) {
                        r->method = NJT_HTTP_DELETE;
                        break;
                    }

                    if (njt_str6cmp(m, 'U', 'N', 'L', 'O', 'C', 'K')) {
                ...
                case 7:
                    if (njt_str7_cmp(m, 'O', 'P', 'T', 'I', 'O', 'N', 'S', ' '))
                    {
                        r->method = NJT_HTTP_OPTIONS;
                    }

                    if (njt_str7_cmp(m, 'C', 'O', 'N', 'N', 'E', 'C', 'T', ' '))
                    {
                ...
                case 8:
                    if (njt_str8cmp(m, 'P', 'R', 'O', 'P', 'F', 'I', 'N', 'D'))
                    {
...
njt_int_t
njt_http_parse_header_line(njt_http_request_t *r, njt_buf_t *b,
    ...
    for (p = b->pos; p < b->last; p++) {
        ...
        case sw_name:
            ...
            if (ch == '/'
                ...
                && njt_strncmp(r->header_name_start, "HTTP", 4) == 0)
            {
...
njt_int_t
njt_http_parse_uri(njt_http_request_t *r)
{
    u_char  *p, ch;
    enum {
        sw_start = 0,
...
njt_int_t
njt_http_parse_complex_uri(njt_http_request_t *r, njt_uint_t merge_slashes)
{
...
njt_int_t
njt_http_parse_status_line(njt_http_request_t *r, njt_buf_t *b,
...
njt_int_t
njt_http_parse_unsafe_uri(njt_http_request_t *r, njt_str_t *uri,
    njt_str_t *args, njt_uint_t *flags)
{
    ...
    for ( /* void */ ; len; len--) {
        ...
        if (njt_path_separator(ch) && len > 2) {

            /* detect "/../" and "/.." */

            if (p[0] == '.' && p[1] == '.'
                && (len == 3 || njt_path_separator(p[2])))
            {
                goto unsafe;
            }
        }
    ...
    if (quoted) {
        ...
        dst = njt_pnalloc(r->pool, uri->len);
        if (dst == NULL) {
            return NJT_ERROR;
        }
        ...
        njt_unescape_uri(&dst, &src, uri->len, 0);
...
njt_table_elt_t *
njt_http_parse_multi_header_lines(njt_http_request_t *r,
...
njt_table_elt_t *
njt_http_parse_set_cookie_lines(njt_http_request_t *r,
    njt_table_elt_t *headers, njt_str_t *name, njt_str_t *value)
{
...
void
njt_http_split_args(njt_http_request_t *r, njt_str_t *uri, njt_str_t *args)
{
    u_char  *p, *last;

    last = uri->data + uri->len;

    p = njt_strlchr(uri->data, last, '?');
