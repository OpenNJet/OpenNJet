...
static ngx_conf_bitmask_t  ngx_http_proxy_next_upstream_masks[] = {
    ...
    { ngx_string("http_502"), NJT_HTTP_UPSTREAM_FT_HTTP_502 },
    { ngx_string("http_503"), NJT_HTTP_UPSTREAM_FT_HTTP_503 },
    { ngx_string("http_504"), NJT_HTTP_UPSTREAM_FT_HTTP_504 },
...
static ngx_http_variable_t  ngx_http_proxy_vars[] = {
    ...
    { ngx_string("proxy_add_x_forwarded_for"), NULL,
      ngx_http_proxy_add_x_forwarded_for_variable, 0, NJT_HTTP_VAR_NOHASH, 0 },

#if 0
    { ngx_string("proxy_add_via"), NULL, NULL, 0, NJT_HTTP_VAR_NOHASH, 0 },
#endif
...
static ngx_path_init_t  ngx_http_proxy_temp_path = {
    ngx_string(NJT_HTTP_PROXY_TEMP_PATH), { 1, 2, 0 }
};
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    if (plcf->proxy_lengths == NULL) {
        ...
        if (ngx_http_proxy_eval(r, ctx, plcf) != NJT_OK) {
            return NJT_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
...
ngx_http_proxy_create_key(ngx_http_request_t *r)
{
    ...
    if (plcf->cache_key.value.data) {
        ...
        if (ngx_http_complex_value(r, &plcf->cache_key, key) != NJT_OK) {
            return NJT_ERROR;
        }
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    if (u->method.len) {
        ...
        if (ngx_http_complex_value(r, plcf->method, &method) != NJT_OK) {
            return NJT_ERROR;
        }
...
ngx_http_proxy_copy_filter(ngx_event_pipe_t *p, ngx_buf_t *buf)
{
    ...
    if (p->upstream_done) {
        ngx_log_debug0(NJT_LOG_DEBUG_HTTP, p->log, 0,
...
ngx_http_proxy_chunked_filter(ngx_event_pipe_t *p, ngx_buf_t *buf)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_error(NJT_LOG_ERR, p->log, 0,
                      "upstream sent invalid chunked response");

        return NJT_ERROR;
    }

    ngx_log_debug2(NJT_LOG_DEBUG_HTTP, p->log, 0,
    ...
    if (ngx_event_pipe_add_free_buf(p, buf) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(r->pool, NJT_OFF_T_LEN);
...
ngx_http_proxy_rewrite_cookie(ngx_http_request_t *r, ngx_table_elt_t *h)
{
    ...
    if (ngx_array_init(&attrs, r->pool, 2, sizeof(ngx_keyval_t)) != NJT_OK) {
        return NJT_ERROR;
    }

    if (ngx_http_proxy_parse_cookie(&h->value, &attrs) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    ngx_array_t *flags)
{
    ...
    for (i = 0; i < nelts; i++) {
        ...
        if (ngx_http_complex_value(r, &flags_values[i], &value) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    size_t prefix, size_t len, ngx_http_proxy_rewrite_t *pr)
{
    ...
    if (ngx_http_complex_value(r, &pr->pattern.complex, &pattern) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    if (ngx_http_complex_value(r, &pr->replacement, &replacement) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    size_t prefix, size_t len, ngx_http_proxy_rewrite_t *pr)
{
    ...
    if (ngx_http_regex_exec(r, pr->pattern.regex, &pattern) != NJT_OK) {
        return NJT_DECLINED;
    }
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (ngx_http_proxy_merge_ssl(cf, conf, prev) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
    ...
    if (conf->ssl && ngx_http_proxy_set_ssl(cf, conf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
...
ngx_http_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_strncasecmp(url->data, (u_char *) "http://", 7) == 0) {
        ...
        ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "invalid URL prefix");
        return NJT_CONF_ERROR;
    }
...
ngx_http_proxy_redirect(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (value[1].data[0] == '~') {
        ...
        if (value[1].data[0] == '*') {
            ...
            if (ngx_http_proxy_rewrite_regex(cf, pr, &value[1], 1) != NJT_OK) {
                return NJT_CONF_ERROR;
            }

        } else {
            if (ngx_http_proxy_rewrite_regex(cf, pr, &value[1], 0) != NJT_OK) {
                return NJT_CONF_ERROR;
            }
        }
...
ngx_http_proxy_set_ssl(ngx_conf_t *cf, ngx_http_proxy_loc_conf_t *plcf)
{
    ...
    if (plcf->upstream.ssl_verify) {
        ...
        if (ngx_ssl_crl(cf, plcf->upstream.ssl, &plcf->ssl_crl) != NJT_OK) {
            return NJT_ERROR;
        }
    }
