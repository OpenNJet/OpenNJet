...
static njt_int_t
njt_http_split_clients_variable(njt_http_request_t *r,
...
njt_conf_split_clients_block(njt_conf_t *cf, njt_command_t *cmd, void *conf)
{
    ...
    if (njt_array_init(&ctx->parts, cf->pool, 2,
                       sizeof(njt_http_split_clients_part_t))
        != NJT_OK)
    {
        return NJT_CONF_ERROR;
    }
...
static char *
njt_http_split_clients(njt_conf_t *cf, njt_command_t *dummy, void *conf)
{
    njt_int_t                       n;
    njt_str_t                      *value;
    njt_http_split_clients_ctx_t   *ctx;
