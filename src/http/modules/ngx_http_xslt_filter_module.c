...
ngx_http_xslt_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for (cl = in; cl; cl = cl->next) {
        ...
        if (ngx_http_xslt_add_chunk(r, ctx, cl->buf) != NJT_OK) {
    ...
    ngx_http_xslt_filter_ctx_t *ctx)
{
    ...
    for (i = 0; i < conf->sheets.nelts; i++) {
        ...
        if (conf->params
            && ngx_http_xslt_params(r, ctx, conf->params, 0) != NJT_OK)
        {
        ...
        if (ngx_http_xslt_params(r, ctx, &sheet[i].params, 1) != NJT_OK) {
            xsltFreeTransformContext(ctx->transform);
            xmlFreeDoc(doc);
            return NULL;
        }
    ...
    ngx_array_t *params, ngx_uint_t final)
{
    ...
    for (i = 0; i < params->nelts; i++) {
        ...
        if (ngx_http_complex_value(r, &param[i].value, &string) != NJT_OK) {
            return NJT_ERROR;
        }
...
ngx_http_xslt_entities(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (xlcf->dtd == NULL) {
        ngx_conf_log_error(NJT_LOG_ERR, cf, 0, "xmlParseDTD() failed");
        return NJT_CONF_ERROR;
    }
