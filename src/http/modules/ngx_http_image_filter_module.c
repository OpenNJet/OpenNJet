...
ngx_http_image_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, r->connection->log, 0, "image filter");
...
ngx_http_image_process(ngx_http_request_t *r)
{
    ...
    if (conf->filter == NJT_HTTP_IMAGE_SIZE) {
        return ngx_http_image_json(r, rc == NJT_OK ? ctx : NULL);
    }
...
ngx_http_image_source(ngx_http_request_t *r, ngx_http_image_filter_ctx_t *ctx)
{
    ...
    if (img == NULL) {
        ngx_log_error(NJT_LOG_ERR, r->connection->log, 0, failed);
    }
...
failed:
    ...
    ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "invalid parameter \"%V\"",
                       &value[i]);
