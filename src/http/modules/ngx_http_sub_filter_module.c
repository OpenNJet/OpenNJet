...
ngx_http_sub_header_filter(ngx_http_request_t *r)
{
    ...
    if (slcf->dynamic == 0) {
        ...
        for (i = 0; i < n; i++) {
            ...
            m = &matches[j].match;
            if (ngx_http_complex_value(r, &pairs[i].match, m) != NJET_OK) {
                return NJET_ERROR;
            }
...
ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ctx->once && (ctx->buf == NULL || ctx->in == NULL)) {
        ...
        if (ctx->busy) {
            if (ngx_http_sub_output(r, ctx) == NJET_ERROR) {
                return NJET_ERROR;
            }
        }
...
ngx_http_sub_output(ngx_http_request_t *r, ngx_http_sub_ctx_t *ctx)
{
    ...
    if (ctx->in || ctx->buf) {
        r->buffered |= NJET_HTTP_SUB_BUFFERED;
    ...
    if (flush) {
        for ( ;; ) {
            ...
            for (i = 0; i < ctx->matches->nelts; i++) {
                ...
                if (ngx_http_sub_match(ctx, start, m) == NJET_AGAIN) {
                    goto again;
                }
            }
...
ngx_http_sub_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (value[1].len == 0) {
        ngx_conf_log_error(NJET_LOG_EMERG, cf, 0, "empty search pattern");
