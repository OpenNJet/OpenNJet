...
ngx_http_sub_header_filter(ngx_http_request_t *r)
{
    ...
    if (slcf->dynamic == 0) {
        ...
        for (i = 0; i < n; i++) {
            ...
            m = &matches[j].match;
            if (ngx_http_complex_value(r, &pairs[i].match, m) != NJT_OK) {
                return NJT_ERROR;
            }
...
ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ctx->once && (ctx->buf == NULL || ctx->in == NULL)) {
        ...
        if (ctx->busy) {
            if (ngx_http_sub_output(r, ctx) == NJT_ERROR) {
                return NJT_ERROR;
            }
        }
    ...
    if (flush) {
        for ( ;; ) {
            ...
            for (i = 0; i < ctx->matches->nelts; i++) {
                ...
                if (ngx_http_sub_match(ctx, start, m) == NJT_AGAIN) {
                    goto again;
                }
            }
...
ngx_http_sub_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (value[1].len == 0) {
        ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "empty search pattern");
