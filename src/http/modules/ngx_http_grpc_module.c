...
ngx_http_grpc_handler(ngx_http_request_t *r)
{
    ...
    if (glcf->grpc_lengths == NULL) {
        ...
        if (ngx_http_grpc_eval(r, ctx, glcf) != NJT_OK) {
            return NJT_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
...
ngx_http_grpc_create_request(ngx_http_request_t *r)
{
    ...
    if (r->method == NJT_HTTP_GET) {
        *b->last++ = ngx_http_v2_indexed(NJT_HTTP_V2_METHOD_GET_INDEX);

        ngx_log_debug0(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "grpc header: \":method: GET\"");

    } else if (r->method == NJT_HTTP_POST) {
        *b->last++ = ngx_http_v2_indexed(NJT_HTTP_V2_METHOD_POST_INDEX);

        ngx_log_debug0(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "grpc header: \":method: POST\"");

    } else {
        *b->last++ = ngx_http_v2_inc_indexed(NJT_HTTP_V2_METHOD_INDEX);
...
#if (NJT_HTTP_SSL)
    if (u->ssl) {
        *b->last++ = ngx_http_v2_indexed(NJT_HTTP_V2_SCHEME_HTTPS_INDEX);
...
#endif
    {
        *b->last++ = ngx_http_v2_indexed(NJT_HTTP_V2_SCHEME_HTTP_INDEX);
    ...
    if (r->valid_unparsed_uri) {
        ...
        if (r->unparsed_uri.len == 1 && r->unparsed_uri.data[0] == '/') {
            *b->last++ = ngx_http_v2_indexed(NJT_HTTP_V2_PATH_ROOT_INDEX);

        } else {
            *b->last++ = ngx_http_v2_inc_indexed(NJT_HTTP_V2_PATH_INDEX);
    ...
    if (!glcf->host_set) {
        *b->last++ = ngx_http_v2_inc_indexed(NJT_HTTP_V2_AUTHORITY_INDEX);
    ...
    while (next) {
        p += sizeof(ngx_http_grpc_frame_t) + NJT_HTTP_V2_DEFAULT_FRAME_SIZE;
        len = b->last - p;
...
ngx_http_grpc_body_output_filter(void *data, ngx_chain_t *in)
{
    ...
    while (in && limit > 0) {
        ...
        ngx_log_debug7(NJT_LOG_DEBUG_EVENT, r->connection->log, 0,
        ...
        do {
            ...
            *ll = cl;
            ...
            if (in->buf->in_file) {
                b->file_pos = file_pos;
                file_pos += ngx_min(NJT_HTTP_V2_DEFAULT_FRAME_SIZE, limit);
                ...
                b->pos = pos;
                pos += ngx_min(NJT_HTTP_V2_DEFAULT_FRAME_SIZE, limit);
    ...
    for ( ;; ) {
        ...
        if (ctx->state < ngx_http_grpc_st_payload) {
            ...
            if (ctx->type == NJT_HTTP_V2_DATA_FRAME) {
                ...
                {
                    if (ngx_http_grpc_send_window_update(r, ctx) != NJT_OK) {
                        return NJT_ERROR;
                    }
    ...
    ngx_buf_t *b)
{
    ...
    for (p = b->pos; p < last; p++) {
        ...
        if (!ctx->index) {
            if (ngx_http_grpc_validate_header_name(r, &ctx->name) != NJT_OK) {
                ngx_log_error(NJT_LOG_ERR, r->connection->log, 0,
                              "upstream sent invalid header: \"%V: %V\"",
                              &ctx->name, &ctx->value);
                return NJT_ERROR;
        ...
        if (!ctx->index || ctx->literal) {
            if (ngx_http_grpc_validate_header_value(r, &ctx->value) != NJT_OK) {
                ngx_log_error(NJT_LOG_ERR, r->connection->log, 0,
                              "upstream sent invalid header: \"%V: %V\"",
                              &ctx->name, &ctx->value);
                return NJT_ERROR;
...
ngx_http_grpc_get_ctx(ngx_http_request_t *r)
{
    ...
    if (ctx->connection == NULL) {
        ...
        if (ngx_http_grpc_get_connection_data(r, ctx, &u->peer) != NJT_OK) {
            return NULL;
        }
    }
...
ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (ngx_http_grpc_merge_ssl(cf, conf, prev) != NJT_OK) {
    ...
    if (conf->ssl && ngx_http_grpc_set_ssl(cf, conf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
...
ngx_http_grpc_set_ssl(ngx_conf_t *cf, ngx_http_grpc_loc_conf_t *glcf)
{
    ...
    if (glcf->upstream.ssl_verify) {
        ...
        if (ngx_ssl_crl(cf, glcf->upstream.ssl, &glcf->ssl_crl) != NJT_OK) {
            return NJT_ERROR;
        }
    }
