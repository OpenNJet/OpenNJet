...
ngx_http_headers_filter(ngx_http_request_t *r)
{
    ...
    if (conf->expires != NJT_HTTP_EXPIRES_OFF && safe_status) {
        if (ngx_http_set_expires(r, conf) != NJT_OK) {
            return NJT_ERROR;
        }
    }
    ...
    if (conf->headers) {
        ...
        for (i = 0; i < conf->headers->nelts; i++) {
            ...
            if (ngx_http_complex_value(r, &h[i].value, &value) != NJT_OK) {
                return NJT_ERROR;
            }
...
ngx_http_set_expires(ngx_http_request_t *r, ngx_http_headers_conf_t *conf)
{
    ...
    if (conf->expires_value != NULL) {
        ...
        if (ngx_http_complex_value(r, conf->expires_value, &value) != NJT_OK) {
            return NJT_ERROR;
        }
...
ngx_http_set_last_modified(ngx_http_request_t *r, ngx_http_header_val_t *hv,
    ngx_str_t *value)
{
    if (ngx_http_set_response_header(r, hv, value) != NJT_OK) {
        return NJT_ERROR;
    }
