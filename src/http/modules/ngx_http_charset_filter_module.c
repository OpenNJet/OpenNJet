...
ngx_http_destination_charset(ngx_http_request_t *r, ngx_str_t *name)
{
    ...
    vv = ngx_http_get_indexed_variable(r, charset - NJT_HTTP_CHARSET_VAR);
    ...
    ngx_http_charset_ctx_t *ctx)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_HTTP, pool->log, 0,
    ...
    if (c) {
        ...
        *dst++ = '?';
        ...
        ngx_log_debug0(NJT_LOG_DEBUG_HTTP, pool->log, 0,
...
ngx_http_charset_map_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_strcasecmp(value[2].data, (u_char *) "utf-8") == 0) {
        table->src2dst = ngx_pcalloc(cf->pool, 256 * NJT_UTF_LEN);
...
ngx_http_charset_map(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    ...
    if (cf->args->nelts != 2) {
        ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "invalid parameters number");
        return NJT_CONF_ERROR;
    }
...
ngx_http_charset_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->charset, prev->charset, NJT_HTTP_CHARSET_OFF);
