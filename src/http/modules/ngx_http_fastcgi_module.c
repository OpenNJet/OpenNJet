...
static ngx_path_init_t  ngx_http_fastcgi_temp_path = {
    ngx_string(NJT_HTTP_FASTCGI_TEMP_PATH), { 1, 2, 0 }
};
...
ngx_http_fastcgi_handler(ngx_http_request_t *r)
{
    ...
    if (flcf->fastcgi_lengths) {
        if (ngx_http_fastcgi_eval(r, flcf) != NJT_OK) {
            return NJT_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
...
ngx_http_fastcgi_create_key(ngx_http_request_t *r)
{
    ...
    if (ngx_http_complex_value(r, &flcf->cache_key, key) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_http_fastcgi_create_request(ngx_http_request_t *r)
{
    ...
    size = sizeof(ngx_http_fastcgi_header_t)
           ...
           + sizeof(ngx_http_fastcgi_header_t)  /* NJT_HTTP_FASTCGI_PARAMS */
           + len + padding
           + sizeof(ngx_http_fastcgi_header_t)  /* NJT_HTTP_FASTCGI_PARAMS */

           + sizeof(ngx_http_fastcgi_header_t); /* NJT_HTTP_FASTCGI_STDIN */
