...
static ngx_int_t ngx_http_perl_call_handler(pTHX_ ngx_http_request_t *r,
    ngx_http_perl_ctx_t *ctx, HV *njet, SV *sub, SV **args,
...
#if (NJET_HAVE_PERL_MULTIPLICITY)
...
static ngx_command_t  ngx_http_perl_commands[] = {
    ...
    { ngx_string("perl"),
      NJET_HTTP_LOC_CONF|NJET_HTTP_LMT_CONF|NJET_CONF_TAKE1,
      ngx_http_perl,
    ...
    { ngx_string("perl_set"),
      NJET_HTTP_MAIN_CONF|NJET_CONF_TAKE2,
      ngx_http_perl_set,
      NJET_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },
...
#define NJET_HTTP_PERL_SSI_SUB  0
#define NJET_HTTP_PERL_SSI_ARG  1


static ngx_http_ssi_param_t  ngx_http_perl_ssi_params[] = {
    { ngx_string("sub"), NJET_HTTP_PERL_SSI_SUB, 1, 0 },
    { ngx_string("arg"), NJET_HTTP_PERL_SSI_ARG, 0, 1 },
...
ngx_http_perl_handle_request(ngx_http_request_t *r)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_HTTP, r->connection->log, 0, "perl handler");
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->njet, sub, NULL,
                                    handler, NULL);
    ...
    if (rc == NJET_OK || rc == NJET_HTTP_OK) {
        ngx_http_send_special(r, NJET_HTTP_LAST);
        ctx->done = 1;
    }
    ...
    uintptr_t data)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->njet, pv->sub, NULL,
                                    &pv->handler, &value);
    ...
    ngx_str_t **params)
{
    ...
    handler = params[NJET_HTTP_PERL_SSI_SUB];
    ...
    args = &params[NJET_HTTP_PERL_SSI_ARG];
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->njet, sv, asv,
                                    handler, NULL);
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_HTTP, r->connection->log, 0, "perl ssi done");
...
ngx_http_perl_init_interpreter(ngx_conf_t *cf, ngx_http_perl_main_conf_t *pmcf)
{
...
#ifdef NJET_PERL_MODULES
    if (pmcf->modules == NJET_CONF_UNSET_PTR) {
        ...
        ngx_str_set(m, NJET_PERL_MODULES);
    }
#endif
    ...
    if (pmcf->modules != NJET_CONF_UNSET_PTR) {
        ...
        for (i = 0; i < pmcf->modules->nelts; i++) {
            if (ngx_conf_full_name(cf->cycle, &m[i], 0) != NJET_OK) {
                return NJET_CONF_ERROR;
            }
        }
    }
    ...
    ngx_http_perl_main_conf_t *pmcf)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_HTTP, cf->log, 0, "create perl interpreter");
    ...
    perl = perl_alloc();
    if (perl == NULL) {
        ngx_log_error(NJET_LOG_ALERT, cf->log, 0, "perl_alloc() failed");
        return NULL;
    }
    ...
    n = (pmcf->modules != NJET_CONF_UNSET_PTR) ? pmcf->modules->nelts * 2 : 0;
    ...
    if (n != 0) {
        ngx_log_error(NJET_LOG_ALERT, cf->log, 0, "perl_parse() failed: %d", n);
        goto fail;
    }
    ...
    if (ngx_http_perl_run_requires(aTHX_ pmcf->requires, cf->log) != NJET_OK) {
        goto fail;
    }
...
ngx_http_perl_run_requires(pTHX_ ngx_array_t *requires, ngx_log_t *log)
{
    ...
    if (requires == NJET_CONF_UNSET_PTR) {
        return NJET_OK;
    }
...
ngx_http_perl_create_main_conf(ngx_conf_t *cf)
{
    ...
    pmcf->modules = NJET_CONF_UNSET_PTR;
    pmcf->requires = NJET_CONF_UNSET_PTR;
...
ngx_http_perl_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    if (pmcf->perl == NULL) {
        if (ngx_http_perl_init_interpreter(cf, pmcf) != NJET_CONF_OK) {
            return NJET_CONF_ERROR;
        }
    }
...
ngx_http_perl_cleanup_perl(void *data)
{
    ...
    if (ngx_perl_term) {
        ngx_log_debug0(NJET_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "perl term");
...
ngx_http_perl_preconfiguration(ngx_conf_t *cf)
{
    ...
    rc = ngx_hash_add_key(&smcf->commands, &ngx_http_perl_ssi_command.name,
                          &ngx_http_perl_ssi_command, NJET_HASH_READONLY_KEY);
...
ngx_http_perl(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (plcf->sub == &PL_sv_undef) {
        ngx_conf_log_error(NJET_LOG_ERR, cf, 0,
...
ngx_http_perl_exit(ngx_cycle_t *cycle)
{
    ...
    if (njet_stash) {
        ngx_log_debug0(NJET_LOG_DEBUG_HTTP, cycle->log, 0, "perl term");
