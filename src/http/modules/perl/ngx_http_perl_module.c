...
static ngx_http_ssi_param_t  ngx_http_perl_ssi_params[] = {
    { ngx_string("sub"), NJT_HTTP_PERL_SSI_SUB, 1, 0 },
    { ngx_string("arg"), NJT_HTTP_PERL_SSI_ARG, 0, 1 },
...
ngx_http_perl_handle_request(ngx_http_request_t *r)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, r->connection->log, 0, "perl handler");
    ...
    if (rc == NJT_OK || rc == NJT_HTTP_OK) {
        ngx_http_send_special(r, NJT_HTTP_LAST);
        ctx->done = 1;
    }
    ...
    ngx_str_t **params)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, r->connection->log, 0, "perl ssi done");
...
ngx_http_perl_init_interpreter(ngx_conf_t *cf, ngx_http_perl_main_conf_t *pmcf)
{
    ...
    if (pmcf->modules == NJT_CONF_UNSET_PTR) {
        ...
        ngx_str_set(m, NJT_PERL_MODULES);
    }
#endif
    ...
    if (pmcf->modules != NJT_CONF_UNSET_PTR) {
        ...
        for (i = 0; i < pmcf->modules->nelts; i++) {
            if (ngx_conf_full_name(cf->cycle, &m[i], 0) != NJT_OK) {
                return NJT_CONF_ERROR;
            }
        }
    }
    ...
    ngx_http_perl_main_conf_t *pmcf)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, cf->log, 0, "create perl interpreter");
    ...
    perl = perl_alloc();
    if (perl == NULL) {
        ngx_log_error(NJT_LOG_ALERT, cf->log, 0, "perl_alloc() failed");
        return NULL;
    }
    ...
    if (n != 0) {
        ngx_log_error(NJT_LOG_ALERT, cf->log, 0, "perl_parse() failed: %d", n);
        goto fail;
    }
    ...
    if (ngx_strcmp(ver, NJT_VERSION) != 0) {
    ...
    if (ngx_http_perl_run_requires(aTHX_ pmcf->requires, cf->log) != NJT_OK) {
        goto fail;
    }
...
ngx_http_perl_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    if (pmcf->perl == NULL) {
        if (ngx_http_perl_init_interpreter(cf, pmcf) != NJT_CONF_OK) {
            return NJT_CONF_ERROR;
        }
    }
...
ngx_http_perl_cleanup_perl(void *data)
{
    ...
    if (ngx_perl_term) {
        ngx_log_debug0(NJT_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "perl term");
...
ngx_http_perl_preconfiguration(ngx_conf_t *cf)
{
    ...
    rc = ngx_hash_add_key(&smcf->commands, &ngx_http_perl_ssi_command.name,
                          &ngx_http_perl_ssi_command, NJT_HASH_READONLY_KEY);
...
ngx_http_perl(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (plcf->sub == &PL_sv_undef) {
        ngx_conf_log_error(NJT_LOG_ERR, cf, 0,
...
ngx_http_perl_exit(ngx_cycle_t *cycle)
{
    ...
    if (njet_stash) {
        ngx_log_debug0(NJT_LOG_DEBUG_HTTP, cycle->log, 0, "perl term");
