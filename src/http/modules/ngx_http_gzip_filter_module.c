...
ngx_http_gzip_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ctx->preallocated == NULL) {
        if (ngx_http_gzip_filter_deflate_start(r, ctx) != NJT_OK) {
            goto failed;
        }
    }
    ...
    if (ctx->nomem) {
        ...
        if (ngx_http_next_body_filter(r, NULL) == NJT_ERROR) {
            goto failed;
        }
...
ngx_http_gzip_filter_deflate(ngx_http_request_t *r, ngx_http_gzip_ctx_t *ctx)
{
    ...
    ngx_log_debug6(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
    ...
    ngx_log_debug5(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
    ...
    if (rc == Z_STREAM_END) {
        ...
        if (ngx_http_gzip_filter_deflate_end(r, ctx) != NJT_OK) {
            return NJT_ERROR;
        }
...
ngx_http_gzip_filter_alloc(void *opaque, u_int items, u_int size)
{
    ...
    if (ctx->zlib_ng) {
        ngx_log_error(NJT_LOG_ALERT, ctx->request->connection->log, 0,
...
ngx_http_gzip_filter_free(void *opaque, void *address)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_HTTP, ctx->request->connection->log, 0,
                   "gzip free: %p", address);
#endif
}
...
ngx_http_gzip_add_variables(ngx_conf_t *cf)
{
    ...
    var = ngx_http_add_variable(cf, &ngx_http_gzip_ratio, NJT_HTTP_VAR_NOHASH);
    if (var == NULL) {
        return NJT_ERROR;
    }
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    v->data = ngx_pnalloc(r->pool, NJT_INT32_LEN + 3);
    if (v->data == NULL) {
        return NJT_ERROR;
    }
