...
typedef struct {
    ...
    ngx_chain_t           out[NJT_HTTP_MP4_LAST_ATOM + 1];
...
ngx_http_mp4_handler(ngx_http_request_t *r)
{
    ...
    if (r->args.len) {
        ...
        if (ngx_http_arg(r, (u_char *) "start", 5, &value) == NJT_OK) {
        ...
        if (ngx_http_arg(r, (u_char *) "end", 3, &value) == NJT_OK) {
    ...
    if (clcf->directio <= of.size) {
        ...
        if (ngx_directio_on(of.fd) == NJT_FILE_ERROR) {
...
ngx_http_mp4_process(ngx_http_mp4_file_t *mp4)
{
    ...
    ngx_log_debug2(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0,
    ...
    if (mp4->trak.nelts == 0) {
        ngx_log_error(NJT_LOG_ERR, mp4->file.log, 0,
    ...
    *prev = &mp4->moov_atom;
    ...
    for (i = 0; i < mp4->trak.nelts; i++) {
        ...
        if (ngx_http_mp4_update_stts_atom(mp4, &trak[i]) != NJT_OK) {
            return NJT_ERROR;
        }

        if (ngx_http_mp4_update_stss_atom(mp4, &trak[i]) != NJT_OK) {
            return NJT_ERROR;
        }

        ngx_http_mp4_update_ctts_atom(mp4, &trak[i]);

        if (ngx_http_mp4_update_stsc_atom(mp4, &trak[i]) != NJT_OK) {
            return NJT_ERROR;
        }

        if (ngx_http_mp4_update_stsz_atom(mp4, &trak[i]) != NJT_OK) {
            return NJT_ERROR;
        }

        if (trak[i].out[NJT_HTTP_MP4_CO64_DATA].buf) {
            if (ngx_http_mp4_update_co64_atom(mp4, &trak[i]) != NJT_OK) {
                return NJT_ERROR;
            }

        } else {
            if (ngx_http_mp4_update_stco_atom(mp4, &trak[i]) != NJT_OK) {
                return NJT_ERROR;
            }
        }
    ...
    *prev = &mp4->mdat_atom;
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0,
    ...
    ngx_http_mp4_atom_handler_t *atom, uint64_t atom_data_size)
{
    ...
    while (mp4->offset < end) {
        ...
        if (ngx_http_mp4_read(mp4, sizeof(uint32_t)) != NJT_OK) {
            return NJT_ERROR;
        }
        ...
        if (atom_size == 0) {
            ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0,
                           "mp4 atom end");
            return NJT_OK;
        }
        ...
        if (ngx_http_mp4_read(mp4, sizeof(ngx_mp4_atom_header_t)) != NJT_OK) {
            return NJT_ERROR;
        }
        ...
        ngx_log_debug4(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "mp4 atom: %*s @%O:%uL",
...
ngx_http_mp4_read(ngx_http_mp4_file_t *mp4, size_t size)
{
    ...
    if ((size_t) n != mp4->buffer_size) {
        ngx_log_error(NJT_LOG_CRIT, mp4->file.log, 0,
...
ngx_http_mp4_read_ftyp_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 ftyp atom");
...
ngx_http_mp4_read_moov_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 moov atom");
    ...
    if (ngx_http_mp4_read(mp4, (size_t) atom_data_size) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 moov atom done");
...
ngx_http_mp4_read_mdat_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 mdat atom");
...
ngx_http_mp4_read_mvhd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 mvhd atom");
    ...
    ngx_log_debug3(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0,
...
ngx_http_mp4_read_trak_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 trak atom");
...
ngx_http_mp4_read_tkhd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 tkhd atom");
...
ngx_http_mp4_read_mdia_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "process mdia atom");
...
ngx_http_mp4_read_mdhd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 mdhd atom");
...
ngx_http_mp4_read_hdlr_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 hdlr atom");
...
ngx_http_mp4_read_minf_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "process minf atom");
...
ngx_http_mp4_read_vmhd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 vmhd atom");
...
ngx_http_mp4_read_smhd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 smhd atom");
...
ngx_http_mp4_read_dinf_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 dinf atom");
...
ngx_http_mp4_read_stbl_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "process stbl atom");
...
ngx_http_mp4_read_stsd_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 stsd atom");
...
ngx_http_mp4_read_stts_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 stts atom");
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (ngx_http_mp4_crop_stts_data(mp4, trak, 1) != NJT_OK) {
        return NJT_ERROR;
    }

    if (ngx_http_mp4_crop_stts_data(mp4, trak, 0) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_http_mp4_read_stss_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 stss atom");
...
ngx_http_mp4_read_ctts_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 ctts atom");
...
ngx_http_mp4_read_stsc_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 stsc atom");
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (ngx_http_mp4_crop_stsc_data(mp4, trak, 1) != NJT_OK) {
        return NJT_ERROR;
    }

    if (ngx_http_mp4_crop_stsc_data(mp4, trak, 0) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    while (entry < end) {
        ...
        ngx_log_debug5(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0,
...
ngx_http_mp4_read_stsz_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 stsz atom");
...
ngx_http_mp4_read_stco_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 stco atom");
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "chunks:%uD", entries);
...
ngx_http_mp4_read_co64_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 co64 atom");
