...
static ngx_str_t  ngx_http_access_log = ngx_string(NJT_HTTP_LOG_PATH);
...
static ngx_http_log_var_t  ngx_http_log_vars[] = {
    ...
    { ngx_string("msec"), NJT_TIME_T_LEN + 4, ngx_http_log_msec },
    { ngx_string("request_time"), NJT_TIME_T_LEN + 4,
                          ngx_http_log_request_time },
    { ngx_string("status"), NJT_INT_T_LEN, ngx_http_log_status },
    { ngx_string("bytes_sent"), NJT_OFF_T_LEN, ngx_http_log_bytes_sent },
    { ngx_string("body_bytes_sent"), NJT_OFF_T_LEN,
                          ngx_http_log_body_bytes_sent },
    { ngx_string("request_length"), NJT_SIZE_T_LEN,
...
ngx_http_log_handler(ngx_http_request_t *r)
{
    ...
    for (l = 0; l < lcf->logs->nelts; l++) {
        ...
        if (log[l].filter) {
            if (ngx_http_complex_value(r, log[l].filter, &val) != NJT_OK) {
                return NJT_ERROR;
            }
    ...
    size_t len)
{
    ...
    if (n == -1) {
        ...
        if (now - log->error_log_time > 59) {
            ngx_log_error(NJT_LOG_ALERT, r->connection->log, err,
    ...
    u_char **name, u_char *buf, size_t len)
{
    ...
    if (!r->root_tested) {
            ...
            != NJT_OK)
        {
            ...
            ngx_log_error(NJT_LOG_ERR, r->connection->log, of.err,
        ...
        if (!of.is_dir) {
            ngx_log_error(NJT_LOG_ERR, r->connection->log, NJT_ENOTDIR,
                          "testing \"%s\" existence failed", path.data);
    ...
    *name = log.data;
    ...
    if (ngx_http_set_disable_symlinks(r, clcf, &log, &of) != NJT_OK) {
        /* simulate successful logging */
        return len;
    }
    ...
    ngx_log_t *log)
{
    ...
    if (rc != Z_OK) {
        ngx_log_error(NJT_LOG_ALERT, log, 0, "deflateInit2() failed: %d", rc);
        goto done;
    }

    ngx_log_debug4(NJT_LOG_DEBUG_HTTP, log, 0,
    ...
    ngx_log_debug5(NJT_LOG_DEBUG_HTTP, log, 0,
    ...
    if (rc != Z_OK) {
        ngx_log_error(NJT_LOG_ALERT, log, 0, "deflateEnd() failed: %d", rc);
        goto done;
    }
...
ngx_http_log_gzip_alloc(void *opaque, u_int items, u_int size)
{
    ...
    ngx_log_debug2(NJT_LOG_DEBUG_HTTP, pool->log, 0,
...
ngx_http_log_gzip_free(void *opaque, void *address)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_HTTP, pool->log, 0, "gzip free: %p", address);
#endif
}
...
ngx_http_log_set_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_strncmp(value[1].data, "syslog:", 7) == 0) {
        ...
        if (ngx_syslog_process_conf(cf, peer) != NJT_CONF_OK) {
            return NJT_CONF_ERROR;
        }
    ...
    if (n == 0) {
        ...
        if (ngx_conf_full_name(cf->cycle, &value[1], 0) != NJT_OK) {
            return NJT_CONF_ERROR;
        }
...
process_formats:
    ...
    for (i = 3; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "flush=", 6) == 0) {
            ...
            if (flush == (ngx_msec_t) NJT_ERROR || flush == 0) {
                ngx_conf_log_error(NJT_LOG_EMERG, cf, 0,
...
invalid:
    ...
    ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "invalid parameter \"%s\"", data);
...
ngx_http_log_init(ngx_conf_t *cf)
{
    ...
    if (lmcf->combined_used) {
        if (ngx_array_init(&a, cf->pool, 1, sizeof(ngx_str_t)) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    h = ngx_array_push(&cmcf->phases[NJT_HTTP_LOG_PHASE].handlers);
    if (h == NULL) {
        return NJT_ERROR;
    }
