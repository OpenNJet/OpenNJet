...
typedef struct {
    ...
    u_char         index;
    njt_uint_t     offset;
} njt_http_v2_push_header_t;


static njt_http_v2_push_header_t  njt_http_v2_push_headers[] = {
    { njt_string(":authority"), NJT_HTTP_V2_AUTHORITY_INDEX,
      offsetof(njt_http_headers_in_t, host) },

    { njt_string("accept-encoding"), NJT_HTTP_V2_ACCEPT_ENCODING_INDEX,
      offsetof(njt_http_headers_in_t, accept_encoding) },

    { njt_string("accept-language"), NJT_HTTP_V2_ACCEPT_LANGUAGE_INDEX,
      offsetof(njt_http_headers_in_t, accept_language) },

    { njt_string("user-agent"), NJT_HTTP_V2_USER_AGENT_INDEX,
      offsetof(njt_http_headers_in_t, user_agent) },
};

#define NJT_HTTP_V2_PUSH_HEADERS                                              \
    (sizeof(njt_http_v2_push_headers) / sizeof(njt_http_v2_push_header_t))


static njt_int_t njt_http_v2_push_resources(njt_http_request_t *r);
static njt_int_t njt_http_v2_push_resource(njt_http_request_t *r,
    njt_str_t *path, njt_str_t *binary);

static njt_http_v2_out_frame_t *njt_http_v2_create_headers_frame(
    njt_http_request_t *r, u_char *pos, u_char *end, njt_uint_t fin);
static njt_http_v2_out_frame_t *njt_http_v2_create_push_frame(
    njt_http_request_t *r, u_char *pos, u_char *end);
...
static njt_int_t njt_http_v2_push_frame_handler(
    njt_http_v2_connection_t *h2c, njt_http_v2_out_frame_t *frame);
...
njt_http_v2_header_filter(njt_http_request_t *r)
{
    ...
    {
        if (njt_http_v2_push_resources(r) != NJT_OK) {
            return NJT_ERROR;
        }
    }
...
static njt_int_t
njt_http_v2_push_resources(njt_http_request_t *r)
{
    ...
    njt_str_t                  binary[NJT_HTTP_V2_PUSH_HEADERS];

    njt_log_debug0(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http2 push resources");

    njt_memzero(binary, NJT_HTTP_V2_PUSH_HEADERS * sizeof(njt_str_t));
    ...
    if (h2lcf->pushes) {
        ...
        for (i = 0; i < h2lcf->pushes->nelts; i++) {
            ...
            rc = njt_http_v2_push_resource(r, &path, binary);
...
static njt_int_t
njt_http_v2_push_resource(njt_http_request_t *r, njt_str_t *path,
    njt_str_t *binary)
{
    ...
    njt_http_v2_out_frame_t     *frame;
    njt_http_v2_connection_t    *h2c;
    njt_http_v2_push_header_t   *ph;

    fc = r->connection;

    njt_log_debug0(NJT_LOG_DEBUG_HTTP, fc->log, 0, "http2 push resource");

    stream = r->stream;
    h2c = stream->connection;

    if (!njt_path_separator(path->data[0])) {
        njt_log_error(NJT_LOG_WARN, fc->log, 0,
    ...
    ph = njt_http_v2_push_headers;

    len = njt_max(r->schema.len, path->len);

    if (binary[0].len) {
        tmp = njt_palloc(r->pool, len);
        if (tmp == NULL) {
            return NJT_ERROR;
        }
        ...
        for (i = 0; i < NJT_HTTP_V2_PUSH_HEADERS; i++) {
            h = (njt_table_elt_t **) ((char *) &r->headers_in + ph[i].offset);

            if (*h) {
                len = njt_max(len, (*h)->value.len);
            }
        }
        ...
        for (i = 0; i < NJT_HTTP_V2_PUSH_HEADERS; i++) {
            ...
            *pos++ = njt_http_v2_inc_indexed(ph[i].index);
            pos = njt_http_v2_write_value(pos, value->data, value->len, tmp);
    ...
    *pos++ = njt_http_v2_indexed(NJT_HTTP_V2_METHOD_GET_INDEX);

    njt_log_debug1(NJT_LOG_DEBUG_HTTP, fc->log, 0,
                   "http2 push header: \":path: %V\"", path);

    *pos++ = njt_http_v2_inc_indexed(NJT_HTTP_V2_PATH_INDEX);
    pos = njt_http_v2_write_value(pos, path->data, path->len, tmp);

    njt_log_debug1(NJT_LOG_DEBUG_HTTP, fc->log, 0,
                   "http2 push header: \":scheme: %V\"", &r->schema);

    if (r->schema.len == 5 && njt_strncmp(r->schema.data, "https", 5) == 0) {
        *pos++ = njt_http_v2_indexed(NJT_HTTP_V2_SCHEME_HTTPS_INDEX);

    } else if (r->schema.len == 4
               && njt_strncmp(r->schema.data, "http", 4) == 0)
    {
        *pos++ = njt_http_v2_indexed(NJT_HTTP_V2_SCHEME_HTTP_INDEX);

    } else {
        *pos++ = njt_http_v2_inc_indexed(NJT_HTTP_V2_SCHEME_HTTP_INDEX);
        pos = njt_http_v2_write_value(pos, r->schema.data, r->schema.len, tmp);
    }
    ...
    for (i = 0; i < NJT_HTTP_V2_PUSH_HEADERS; i++) {
        ...
        pos = njt_cpymem(pos, binary[i].data, binary[i].len);
    }

    frame = njt_http_v2_create_push_frame(r, start, pos);
    if (frame == NULL) {
        return NJT_ERROR;
    }
    ...
    stream = njt_http_v2_push_stream(stream, path);
...
static njt_http_v2_out_frame_t *
njt_http_v2_create_push_frame(njt_http_request_t *r, u_char *pos, u_char *end)
{
    u_char                     type, flags;
    ...
    frame->handler = njt_http_v2_push_frame_handler;
    frame->stream = stream;
    frame->length = rest;
    frame->blocked = 1;
    frame->fin = 0;
    ...
    for ( ;; ) {
        ...
        *b->last++ = flags;
        ...
        if (type == NJT_HTTP_V2_PUSH_PROMISE_FRAME) {
            ...
            b->last = njt_http_v2_write_sid(b->last, h2c->last_push);
...
static njt_int_t
njt_http_v2_push_frame_handler(njt_http_v2_connection_t *h2c,
    njt_http_v2_out_frame_t *frame)
{
    njt_chain_t           *cl, *ln;
    njt_http_v2_stream_t  *stream;
