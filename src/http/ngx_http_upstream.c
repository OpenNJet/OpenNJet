...
ngx_conf_bitmask_t  ngx_http_upstream_cache_method_mask[] = {
    { ngx_string("GET"), NJT_HTTP_GET },
    { ngx_string("HEAD"), NJT_HTTP_HEAD },
    { ngx_string("POST"), NJT_HTTP_POST },
    { ngx_null_string, 0 }
};


ngx_conf_bitmask_t  ngx_http_upstream_ignore_headers_masks[] = {
    { ngx_string("X-Accel-Redirect"), NJT_HTTP_UPSTREAM_IGN_XA_REDIRECT },
    { ngx_string("X-Accel-Expires"), NJT_HTTP_UPSTREAM_IGN_XA_EXPIRES },
    { ngx_string("X-Accel-Limit-Rate"), NJT_HTTP_UPSTREAM_IGN_XA_LIMIT_RATE },
    { ngx_string("X-Accel-Buffering"), NJT_HTTP_UPSTREAM_IGN_XA_BUFFERING },
    { ngx_string("X-Accel-Charset"), NJT_HTTP_UPSTREAM_IGN_XA_CHARSET },
    { ngx_string("Expires"), NJT_HTTP_UPSTREAM_IGN_EXPIRES },
    { ngx_string("Cache-Control"), NJT_HTTP_UPSTREAM_IGN_CACHE_CONTROL },
    { ngx_string("Set-Cookie"), NJT_HTTP_UPSTREAM_IGN_SET_COOKIE },
    { ngx_string("Vary"), NJT_HTTP_UPSTREAM_IGN_VARY },
    { ngx_null_string, 0 }
};
...
ngx_http_upstream_init(ngx_http_request_t *r)
{
    ...
    if (ngx_event_flags & NJT_USE_CLEAR_EVENT) {
        ...
        if (!c->write->active) {
            if (ngx_add_event(c->write, NJT_WRITE_EVENT, NJT_CLEAR_EVENT)
                == NJT_ERROR)
            {
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    if (ngx_http_upstream_set_local(r, u, u->conf->local) != NJT_OK) {
        ngx_http_finalize_request(r, NJT_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    if (u->resolved == NULL) {
        ...
        if (ctx == NJT_NO_RESOLVER) {
            ...
            ngx_http_upstream_finalize_request(r, u, NJT_HTTP_BAD_GATEWAY);
            return;
        }
        ...
        if (ngx_resolve_name(ctx) != NJT_OK) {
            u->resolved->ctx = NULL;
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return;
...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (c == NULL) {
        ...
        if (ngx_http_file_cache_new(r) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    case NJT_HTTP_CACHE_STALE:
        ...
        {
            if (ngx_http_upstream_cache_background_update(r, u) == NJT_OK) {
    ...
    if (ngx_http_upstream_cache_check_range(r, u) == NJT_DECLINED) {
        u->cacheable = 0;
    }
    ...
    ngx_http_file_cache_t **cache)
{
    ...
    if (ngx_http_complex_value(r, u->conf->cache_value, &val) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_http_upstream_cache_send(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NJT_OK) {
        ...
        if (ngx_http_upstream_process_headers(r, u) != NJT_OK) {
            return NJT_DONE;
        }
...
ngx_http_upstream_resolve_handler(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ngx_http_upstream_create_round_robin_peer(r, ur) != NJT_OK) {
        ngx_http_upstream_finalize_request(r, u,
                                           NJT_HTTP_INTERNAL_SERVER_ERROR);
        goto failed;
    }
    ...
    ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NJT_LOG_DEBUG_HTTP, ev->log, 0,
    ...
    if (c->error) {
        if ((ngx_event_flags & NJT_USE_LEVEL_EVENT) && ev->active) {

            event = ev->write ? NJT_WRITE_EVENT : NJT_READ_EVENT;

            if (ngx_del_event(ev, event, 0) != NJT_OK) {
                ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
                return;
            }
    ...
    if (ngx_event_flags & NJT_USE_KQUEUE_EVENT) {
        ...
        if (!u->cacheable && u->peer.connection) {
            ngx_log_error(NJT_LOG_INFO, ev->log, ev->kq_errno,
    ...
    if ((ngx_event_flags & NJT_USE_EPOLL_EVENT) && ngx_use_epoll_rdhup) {
        socklen_t  len;
        ...
        if (!u->cacheable && u->peer.connection) {
            ngx_log_error(NJT_LOG_INFO, ev->log, err,
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_HTTP, ev->log, err,
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NJT_BUSY) {
        ngx_log_error(NJT_LOG_ERR, r->connection->log, 0, "no live upstreams");
        ngx_http_upstream_next(r, u, NJT_HTTP_UPSTREAM_FT_NOLIVE);
        return;
    }

    if (rc == NJT_DECLINED) {
        ngx_http_upstream_next(r, u, NJT_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
    ...
    if (u->request_sent) {
        if (ngx_http_upstream_reinit(r, u) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    ngx_http_upstream_t *u, ngx_connection_t *c)
{
    ...
    if (ngx_http_upstream_test_connect(c) != NJT_OK) {
        ngx_http_upstream_next(r, u, NJT_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
    ...
    if (u->conf->ssl_server_name || u->conf->ssl_verify) {
        if (ngx_http_upstream_ssl_name(r, u, c) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    {
        if (ngx_http_upstream_ssl_certificate(r, u, c) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    if (u->conf->ssl_session_reuse) {
        ...
        if (clcf->tcp_nodelay && ngx_tcp_nodelay(c) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    ngx_connection_t *c)
{
    ...
    if (c->ssl->handshaked) {
        ...
        if (u->conf->ssl_verify) {
            ...
            if (ngx_ssl_check_host(c, &u->ssl_name) != NJT_OK) {
                ngx_log_error(NJT_LOG_ERR, c->log, 0,
    ...
    if (c->write->timedout) {
        ngx_http_upstream_next(r, u, NJT_HTTP_UPSTREAM_FT_TIMEOUT);
        return;
    }
    ...
    ngx_connection_t *c)
{
    ...
    if (u->conf->ssl_name) {
        if (ngx_http_complex_value(r, u->conf->ssl_name, &name) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    if (SSL_set_tlsext_host_name(c->ssl->connection,
        ...
        == 0)
    {
        ngx_ssl_error(NJT_LOG_ERR, r->connection->log, 0,
    ...
    ngx_uint_t do_write)
{
    ...
    if (!u->request_sent && ngx_http_upstream_test_connect(c) != NJT_OK) {
        ngx_http_upstream_next(r, u, NJT_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
    ...
    if (rc == NJT_AGAIN) {
        ...
        if (ngx_handle_write_event(c->write, u->conf->send_lowat) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
        ...
        if (c->write->ready && c->tcp_nopush == NJT_TCP_NOPUSH_SET) {
            if (ngx_tcp_push(c->fd) == -1) {
                ngx_log_error(NJT_LOG_CRIT, c->log, ngx_socket_errno,
...
ngx_http_upstream_read_request_handler(ngx_http_request_t *r)
{
    ...
    if (c->read->timedout) {
        c->timedout = 1;
        ngx_http_upstream_finalize_request(r, u, NJT_HTTP_REQUEST_TIME_OUT);
        return;
    }
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NJT_HTTP_UPSTREAM_INVALID_HEADER) {
        ngx_http_upstream_next(r, u, NJT_HTTP_UPSTREAM_FT_INVALID_HEADER);
        return;
    }
    ...
    if (u->headers_in.status_n >= NJT_HTTP_SPECIAL_RESPONSE) {
        ...
        if (ngx_http_upstream_test_next(r, u) == NJT_OK) {
            return;
        }

        if (ngx_http_upstream_intercept_errors(r, u) == NJT_OK) {
            return;
        }
    }
    ...
    ngx_http_upstream_t *u)
{
    ...
    if (status == NJT_HTTP_NOT_FOUND && u->conf->intercept_404) {
        ngx_http_upstream_finalize_request(r, u, NJT_HTTP_NOT_FOUND);
        return NJT_OK;
    }
...
ngx_http_upstream_process_headers(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    {
        ngx_http_upstream_finalize_request(r, u, NJT_DECLINED);
        ...
        if (uri.data[0] == '@') {
            ...
            if (ngx_http_parse_unsafe_uri(r, &uri, &args, &flags) != NJT_OK) {
                ngx_http_finalize_request(r, NJT_HTTP_NOT_FOUND);
                return NJT_DONE;
            }
    ...
    for (i = 0; /* void */; i++) {
        ...
        if (ngx_http_upstream_copy_header_line(r, &h[i], 0) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u,
                                               NJT_HTTP_INTERNAL_SERVER_ERROR);
            return NJT_DONE;
        }
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (!u->buffering) {
        ...
        if (u->input_filter_init(u->input_filter_ctx) == NJT_ERROR) {
            ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
            return;
        }
        ...
        if (n) {
            ...
            if (ngx_http_send_special(r, NJT_HTTP_FLUSH) == NJT_ERROR) {
                ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
                return;
            }
    ...
    default: /* NJT_OK */
        ...
        if (u->cache_status == NJT_HTTP_CACHE_BYPASS) {
            ...
            if (ngx_http_file_cache_create(r) != NJT_OK) {
                ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
                return;
            }
        }
    ...
    if (u->cacheable) {
        ...
        if (valid) {
            ...
            if (ngx_http_file_cache_set_header(r, u->buffer.start) != NJT_OK) {
                ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
                return;
            }
...
ngx_http_upstream_upgrade(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (clcf->tcp_nodelay) {
        ...
        if (ngx_tcp_nodelay(c) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
            return;
        }

        if (ngx_tcp_nodelay(u->peer.connection) != NJT_OK) {
            ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
            return;
        }
    }
    ...
    ngx_uint_t from_upstream, ngx_uint_t do_write)
{
    ...
    if (downstream->write->timedout) {
        c->timedout = 1;
        ngx_connection_error(c, NJT_ETIMEDOUT, "client timed out");
        ngx_http_upstream_finalize_request(r, u, NJT_HTTP_REQUEST_TIME_OUT);
        return;
    }

    if (upstream->read->timedout || upstream->write->timedout) {
        ngx_connection_error(c, NJT_ETIMEDOUT, "upstream timed out");
        ngx_http_upstream_finalize_request(r, u, NJT_HTTP_GATEWAY_TIME_OUT);
        return;
    }
    ...
    if (ngx_handle_read_event(upstream->read, flags) != NJT_OK) {
        ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
        return;
    }
    ...
    if (ngx_handle_read_event(downstream->read, flags) != NJT_OK) {
        ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
        return;
    }
        ...
        if (do_write) {
            ...
            if (u->busy_bufs == NULL) {
                ...
                if (upstream->read->eof) {
                    ngx_log_error(NJT_LOG_ERR, upstream->log, 0,
                                  "upstream prematurely closed connection");
...
ngx_http_upstream_non_buffered_filter(void *data, ssize_t bytes)
{
    ...
    if (u->length == 0) {
        ngx_log_error(NJT_LOG_WARN, r->connection->log, 0,
...
ngx_http_upstream_process_downstream(ngx_http_request_t *r)
{
    ...
    if (wev->timedout) {
        ...
        if (ngx_event_pipe(p, 1) == NJT_ABORT) {
            ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
            return;
        }
    }
    ...
    ngx_http_upstream_t *u)
{
    ...
    if (rev->timedout) {
        ...
        if (ngx_event_pipe(p, 0) == NJT_ABORT) {
            ngx_http_upstream_finalize_request(r, u, NJT_ERROR);
            return;
        }
    }
    ...
    ngx_uint_t ft_type)
{
    ...
    if (ft_type == NJT_HTTP_UPSTREAM_FT_TIMEOUT) {
        ngx_log_error(NJT_LOG_ERR, r->connection->log, NJT_ETIMEDOUT,
                      "upstream timed out");
    }
...
ngx_http_upstream_cleanup(void *data)
{
    ...
    ngx_http_upstream_finalize_request(r, r->upstream, NJT_DONE);
}
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    *u->cleanup = NULL;
    ...
    if (rc == 0) {
        ...
        if (ngx_http_upstream_process_trailers(r, u) != NJT_OK) {
            ngx_http_finalize_request(r, NJT_ERROR);
            return;
        }

        rc = ngx_http_send_special(r, NJT_HTTP_LAST);

    } else if (flush) {
        r->keepalive = 0;
        rc = ngx_http_send_special(r, NJT_HTTP_FLUSH);
    }
    ...
    ngx_uint_t offset)
{
    ...
    if (expires == NJT_ERROR || expires < ngx_time()) {
...
ngx_http_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    uscf = ngx_http_upstream_add(cf, &u, NJT_HTTP_UPSTREAM_CREATE
    ...
    ngx_http_upstream_local_t *local)
{
    ...
    if (ngx_http_complex_value(r, local->value, &val) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    ngx_str_t *default_hide_headers, ngx_hash_init_t *hash)
{
    ...
    if (ngx_hash_init(hash, hide_headers.elts, hide_headers.nelts) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_http_upstream_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    if (ngx_hash_init(&hash, headers_in.elts, headers_in.nelts) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
