...
ngx_http_special_response_handler(ngx_http_request_t *r, ngx_int_t error)
{
    ...
    if (ngx_http_discard_request_body(r) != NJT_OK) {
        r->keepalive = 0;
    }
...
ngx_http_send_error_page(ngx_http_request_t *r, ngx_http_err_page_t *err_page)
{
    ...
    if (ngx_http_complex_value(r, &err_page->value, &uri) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_http_send_refresh(ngx_http_request_t *r)
{
    ...
    escape = 2 * ngx_escape_uri(NULL, location, len, NJT_ESCAPE_REFRESH);
    ...
    if (escape == 0) {
        ...
        p = (u_char *) ngx_escape_uri(p, location, len, NJT_ESCAPE_REFRESH);
    }
