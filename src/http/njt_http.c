...
njt_str_t njt_http_html_default_types[] = {njt_string("text/html"),
                                           njt_null_string};

static njt_command_t njt_http_commands[] = {

    {njt_string("http"), NJT_MAIN_CONF | NJT_CONF_BLOCK | NJT_CONF_NOARGS,
     njt_http_block, 0, 0, NULL},

    njt_null_command};

static njt_core_module_t njt_http_module_ctx = {njt_string("http"), NULL, NULL};

njt_module_t njt_http_module = {NJT_MODULE_V1,
...
static char *njt_http_block(njt_conf_t *cf, njt_command_t *cmd, void *conf) {
  ...
  *(njt_http_conf_ctx_t **)conf = ctx;
  ...
  ctx->main_conf = njt_pcalloc(cf->pool, sizeof(void *) * njt_http_max_module);
...
static njt_int_t njt_http_init_phases(njt_conf_t *cf,
                                      njt_http_core_main_conf_t *cmcf) {
  if (njt_array_init(&cmcf->phases[NJT_HTTP_POST_READ_PHASE].handlers, cf->pool,
                     1, sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_SERVER_REWRITE_PHASE].handlers,
                     cf->pool, 1, sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_REWRITE_PHASE].handlers, cf->pool,
                     1, sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_PREACCESS_PHASE].handlers, cf->pool,
                     1, sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_ACCESS_PHASE].handlers, cf->pool, 2,
                     sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_PRECONTENT_PHASE].handlers,
                     cf->pool, 2, sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_CONTENT_PHASE].handlers, cf->pool,
                     4, sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }

  if (njt_array_init(&cmcf->phases[NJT_HTTP_LOG_PHASE].handlers, cf->pool, 1,
                     sizeof(njt_http_handler_pt)) != NJT_OK) {
    return NJT_ERROR;
  }
...
// static char *
char *njt_http_merge_servers(njt_conf_t *cf, njt_http_core_main_conf_t *cmcf,
...
char *njt_http_merge_locations(njt_conf_t *cf, njt_queue_t *locations,
                               void **loc_conf, njt_http_module_t *module,
                               njt_uint_t ctx_index) {
  char *rv;
  njt_queue_t *q;
  njt_http_conf_ctx_t *ctx, saved;
  ...
  for (q = njt_queue_head(locations); q != njt_queue_sentinel(locations);
       q = njt_queue_next(q)) {
    ...
    rv = njt_http_merge_locations(cf, clcf->locations, clcf->loc_conf, module,
                                  ctx_index);
    if (rv != NJT_CONF_OK) {
      return rv;
    }
...
njt_int_t njt_http_init_locations(njt_conf_t *cf,
                                  ...
                                  njt_http_core_loc_conf_t *pclcf) {
  njt_uint_t n;
  njt_queue_t *q, *locations, *named, tail;
  njt_http_core_loc_conf_t *clcf;
  njt_http_location_queue_t *lq;
  ...
  if (named) {
...
#else
    clcfp = njt_palloc(cf->pool, (n + 1) * sizeof(njt_http_core_loc_conf_t *));
#endif
    // end
    clcfp = njt_palloc(cf->pool, (n + 1) * sizeof(njt_http_core_loc_conf_t *));
    if (clcfp == NULL) {
    ...
    for (q = named; q != njt_queue_sentinel(locations); q = njt_queue_next(q)) {
      lq = (njt_http_location_queue_t *)q;
  ...
  if (regex) {
...
#if (NJT_HTTP_DYNAMIC_LOC)
    clcfp =
        njt_palloc(pclcf->pool, (r + 1) * sizeof(njt_http_core_loc_conf_t *));
#else
    clcfp = njt_palloc(cf->pool, (r + 1) * sizeof(njt_http_core_loc_conf_t *));
#endif
    // end
    ...
    for (q = regex; q != njt_queue_sentinel(locations); q = njt_queue_next(q)) {
      lq = (njt_http_location_queue_t *)q;
...
njt_int_t njt_http_init_new_locations(njt_conf_t *cf,
  ...
  for (q = njt_queue_head(locations); q != njt_queue_sentinel(locations);
    ...
    if (njt_http_init_new_locations(cf, NULL, clcf) != NJT_OK) {
      return NJT_ERROR;
    }
  ...
  if (named) {
    ...
    // cscf->named_locations = clcfp;
    cscf->new_named_locations = clcfp;
  ...
  if (regex) {
    ...
    // pclcf->regex_locations = clcfp;
    pclcf->new_regex_locations = clcfp;
...
njt_http_init_new_static_location_trees(njt_conf_t *cf,
  ...
  // pclcf->static_locations = njt_http_create_locations_tree(cf, locations, 0);
  pclcf->new_static_locations =
      njt_http_create_locations_tree(cf, locations, 0);
...
njt_int_t njt_http_add_location(njt_conf_t *cf, njt_queue_t **locations,
  ...
  if (*locations == NULL) {
    *locations = njt_palloc(cf->temp_pool, sizeof(njt_http_location_queue_t));
    if (*locations == NULL) {
      return NJT_ERROR;
    }
...
static njt_int_t njt_http_escape_location_name(njt_conf_t *cf,
  ...
  escape =
      2 * njt_escape_uri(NULL, clcf->name.data, clcf->name.len, NJT_ESCAPE_URI);
...
static njt_int_t njt_http_join_exact_locations(njt_conf_t *cf,
  ...
  while (q != njt_queue_last(locations)) {
    ...
    if (lq->name->len == lx->name->len &&
        njt_filename_cmp(lq->name->data, lx->name->data, lx->name->len) == 0) {
...
static void njt_http_create_locations_list(njt_queue_t *locations,
  ...
  for (x = njt_queue_next(q); x != njt_queue_sentinel(locations);
       x = njt_queue_next(x)) {
    lx = (njt_http_location_queue_t *)x;
...
njt_http_create_locations_tree(njt_conf_t *cf, njt_queue_t *locations,
  ...
  node =
      njt_palloc(cf->pool, offsetof(njt_http_location_tree_node_t, name) + len);
  if (node == NULL) {
    return NULL;
  }
...
njt_int_t njt_http_add_listen(njt_conf_t *cf, njt_http_core_srv_conf_t *cscf,
  ...
  if (cmcf->ports == NULL) {
    cmcf->ports =
        njt_array_create(cf->temp_pool, 2, sizeof(njt_http_conf_port_t));
    if (cmcf->ports == NULL) {
      return NJT_ERROR;
    }
  }
...
static njt_int_t njt_http_add_addresses(njt_conf_t *cf,
  ...
  for (i = 0; i < port->addrs.nelts; i++) {
    ...
    if (njt_cmp_sockaddr(lsopt->sockaddr, lsopt->socklen, addr[i].opt.sockaddr,
                         addr[i].opt.socklen, 0) != NJT_OK) {
      continue;
    }
...
static njt_int_t njt_http_add_address(njt_conf_t *cf,
  ...
  if (port->addrs.elts == NULL) {
    if (njt_array_init(&port->addrs, cf->temp_pool, 4,
                       sizeof(njt_http_conf_addr_t)) != NJT_OK) {
      return NJT_ERROR;
    }
  }
...
static njt_int_t njt_http_add_server(njt_conf_t *cf,
  ...
  if (addr->servers.elts == NULL) {
    if (njt_array_init(&addr->servers, cf->temp_pool, 4,
                       sizeof(njt_http_core_srv_conf_t *)) != NJT_OK) {
      return NJT_ERROR;
    }
    ...
    for (i = 0; i < addr->servers.nelts; i++) {
      if (server[i] == cscf) {
        njt_conf_log_error(NJT_LOG_EMERG, cf, 0, "a duplicate listen %V",
                           &addr->opt.addr_text);
        return NJT_ERROR;
      }
    }
...
static njt_int_t njt_http_add_addrs(njt_conf_t *cf, njt_http_port_t *hport,
  ...
  hport->addrs =
      njt_pcalloc(cf->pool, hport->naddrs * sizeof(njt_http_in_addr_t));
  if (hport->addrs == NULL) {
    return NJT_ERROR;
  }
...
static njt_int_t njt_http_add_addrs6(njt_conf_t *cf, njt_http_port_t *hport,
  ...
  hport->addrs =
      njt_pcalloc(cf->pool, hport->naddrs * sizeof(njt_http_in6_addr_t));
  if (hport->addrs == NULL) {
    return NJT_ERROR;
  }
  ...
  if (*types == NULL) {
    *types = njt_array_create(cf->temp_pool, 1, sizeof(njt_hash_key_t));
    ...
    if (default_type) {
      ...
      type->key_hash = njt_hash_key(default_type->data, default_type->len);
      type->value = (void *)4;
    }
  }
  ...
  for (i = 1; i < cf->args->nelts; i++) {
    ...
    for (n = 0; n < (*types)->nelts; n++) {
      ...
      if (njt_strcmp(value[i].data, type[n].key.data) == 0) {
        njt_conf_log_error(NJT_LOG_WARN, cf, 0, "duplicate MIME type \"%V\"",
                           &value[i]);
        goto next;
      }
    }
...
char *njt_http_merge_types(njt_conf_t *cf, njt_array_t **keys,
  ...
  if (prev_types_hash->buckets == NULL) {
    ...
    if (*prev_keys == NULL) {
      ...
      if (njt_http_set_default_types(cf, prev_keys, default_types) != NJT_OK) {
        return NJT_CONF_ERROR;
      }
