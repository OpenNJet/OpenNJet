...
typedef struct {
    ...
    u_char  sym;
    u_char  ending;
} njt_http_huff_decode_code_t;


static njt_inline njt_int_t njt_http_huff_decode_bits(u_char *state,
    u_char *ending, njt_uint_t bits, u_char **dst);


static njt_http_huff_decode_code_t  njt_http_huff_decode_codes[256][16] =
{
    /* 0 */
    {
...
njt_int_t
njt_http_huff_decode(u_char *state, u_char *src, size_t len, u_char **dst,
    njt_uint_t last, njt_log_t *log)
{
    ...
    while (src != end) {
        ...
        if (njt_http_huff_decode_bits(state, &ending, ch >> 4, dst)
            != NJT_OK)
        {
        ...
        if (njt_http_huff_decode_bits(state, &ending, ch & 0xf, dst)
            != NJT_OK)
        {
            njt_log_debug2(NJT_LOG_DEBUG_HTTP, log, 0,
                           "http2 huffman decoding error at state %d: "
...
njt_http_huff_decode_bits(u_char *state, u_char *ending, njt_uint_t bits,
    u_char **dst)
{
    njt_http_huff_decode_code_t  code;

    code = njt_http_huff_decode_codes[*state][bits];
