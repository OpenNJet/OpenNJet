...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    ngx_http_max_module = ngx_count_modules(cf->cycle, NJT_HTTP_MODULE);
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        if (ngx_http_init_locations(cf, cscfp[s], clcf) != NJT_OK) {
            return NJT_CONF_ERROR;
        }

        if (ngx_http_init_static_location_trees(cf, clcf) != NJT_OK) {
            return NJT_CONF_ERROR;
        }
    }


    if (ngx_http_init_phases(cf, cmcf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }

    if (ngx_http_init_headers_in_hash(cf, cmcf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
    ...
    if (ngx_http_variables_init_vars(cf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
    ...
    *cf = pcf;
    ...
    if (ngx_http_init_phase_handlers(cf, cmcf) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
    ...
    if (ngx_http_optimize_servers(cf, cmcf, cmcf->ports) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
...
{
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_POST_READ_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_SERVER_REWRITE_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != NJT_OK)
    {
        return NJT_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_REWRITE_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != NJT_OK)
    {
        return NJT_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_PREACCESS_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != NJT_OK)
    {
        return NJT_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_ACCESS_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_PRECONTENT_PHASE].handlers,
                       cf->pool, 2, sizeof(ngx_http_handler_pt))
        != NJT_OK)
    {
        return NJT_ERROR;
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_CONTENT_PHASE].handlers,
    ...
    if (ngx_array_init(&cmcf->phases[NJT_HTTP_LOG_PHASE].handlers,
                       cf->pool, 1, sizeof(ngx_http_handler_pt))
        != NJT_OK)
    {
        return NJT_ERROR;
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
         ...
         q = ngx_queue_next(q))
    {
        ...
        if (ngx_http_init_locations(cf, NULL, clcf) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
    ...
    if (ngx_http_join_exact_locations(cf, locations) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    ngx_http_core_loc_conf_t *clcf)
{
    ...
    if (ngx_http_escape_location_name(cf, clcf) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_http_escape_location_name(ngx_conf_t *cf, ngx_http_core_loc_conf_t *clcf)
{
    ...
    if (escape) {
        ...
        ngx_escape_uri(p, clcf->name.data, clcf->name.len, NJT_ESCAPE_URI);
    ...
    ngx_http_conf_port_t *port, ngx_http_listen_opt_t *lsopt)
{
    ...
    for (i = 0; i < port->addrs.nelts; i++) {
        ...
        if (ngx_http_add_server(cf, cscf, &addr[i]) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    ngx_array_t *ports)
{
    ...
    for (p = 0; p < ports->nelts; p++) {
        ...
        for (a = 0; a < port[p].addrs.nelts; a++) {
            ...
            {
                if (ngx_http_server_names(cf, cmcf, &addr[a]) != NJT_OK) {
                    return NJT_ERROR;
                }
            }
        }

        if (ngx_http_init_listening(cf, &port[p]) != NJT_OK) {
            return NJT_ERROR;
        }
    }
    ...
    ngx_http_conf_addr_t *addr)
{
    ...
    ha.temp_pool = ngx_create_pool(NJT_DEFAULT_POOL_SIZE, cf->log);
    ...
    if (ngx_hash_keys_array_init(&ha, NJT_HASH_LARGE) != NJT_OK) {
        goto failed;
    }
    ...
    if (ha.keys.nelts) {
        ...
        if (ngx_hash_init(&hash, ha.keys.elts, ha.keys.nelts) != NJT_OK) {
            goto failed;
        }
    }
...
#if (NJT_HAVE_INET6)
        case AF_INET6:
            if (ngx_http_add_addrs6(cf, hport, addr) != NJT_OK) {
                return NJT_ERROR;
            }
            break;
#endif
        default: /* AF_INET */
            if (ngx_http_add_addrs(cf, hport, addr) != NJT_OK) {
                return NJT_ERROR;
            }
            break;
        }
    ...
    ngx_str_t *default_types)
{
    ...
    if (*keys) {
        ...
        if (ngx_hash_init(&hash, (*keys)->elts, (*keys)->nelts) != NJT_OK) {
            return NJT_CONF_ERROR;
        }
