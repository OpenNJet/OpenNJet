...
#define NJET_SENDFILE_MAXSIZE  2147483647L
...
ngx_linux_sendfile_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    if (limit == 0 || limit > (off_t) (NJET_SENDFILE_MAXSIZE - ngx_pagesize)) {
        limit = NJET_SENDFILE_MAXSIZE - ngx_pagesize;
    }
    ...
    for ( ;; ) {
        ...
        if (c->tcp_nopush == NJET_TCP_NOPUSH_UNSET
            && header.count != 0
            && cl
            && cl->buf->in_file)
        {
            ...
            if (c->tcp_nodelay == NJET_TCP_NODELAY_SET) {
                               ...
                               (const void *) &tcp_nodelay, sizeof(int)) == -1)
                {
                    ...
                    c->tcp_nodelay = NJET_TCP_NODELAY_UNSET;
...
eintr:
    ...
    ngx_log_debug3(NJET_LOG_DEBUG_EVENT, c->log, 0, "sendfile: %z of %uz @%O",
...
ngx_linux_sendfile_thread(ngx_connection_t *c, ngx_buf_t *file, size_t size)
{
    ...
    ngx_log_debug3(NJET_LOG_DEBUG_CORE, c->log, 0,
    ...
    if (file->file->thread_handler(task, file->file) != NJET_OK) {
        return NJET_ERROR;
    }
...
ngx_linux_sendfile_thread_handler(void *data, ngx_log_t *log)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_CORE, log, 0, "linux sendfile thread handler");
