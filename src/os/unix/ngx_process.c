...
ngx_process_t    ngx_processes[NJT_MAX_PROCESSES];


ngx_signal_t  signals[] = {
    { ngx_signal_value(NJT_RECONFIGURE_SIGNAL),
      "SIG" ngx_value(NJT_RECONFIGURE_SIGNAL),
      "reload",
      ngx_signal_handler },

    { ngx_signal_value(NJT_REOPEN_SIGNAL),
      "SIG" ngx_value(NJT_REOPEN_SIGNAL),
      "reopen",
      ngx_signal_handler },

    { ngx_signal_value(NJT_NOACCEPT_SIGNAL),
      "SIG" ngx_value(NJT_NOACCEPT_SIGNAL),
      "",
      ngx_signal_handler },

    { ngx_signal_value(NJT_TERMINATE_SIGNAL),
      "SIG" ngx_value(NJT_TERMINATE_SIGNAL),
      "stop",
      ngx_signal_handler },

    { ngx_signal_value(NJT_SHUTDOWN_SIGNAL),
      "SIG" ngx_value(NJT_SHUTDOWN_SIGNAL),
      "quit",
      ngx_signal_handler },

    { ngx_signal_value(NJT_CHANGEBIN_SIGNAL),
      "SIG" ngx_value(NJT_CHANGEBIN_SIGNAL),
      "",
      ngx_signal_handler },
    ...
    char *name, ngx_int_t respawn)
{
    ...
    ngx_log_error(NJT_LOG_NOTICE, cycle->log, 0, "start %s %P", name, pid);
...
ngx_signal_handler(int signo, siginfo_t *siginfo, void *ucontext)
{
    ...
    case NJT_PROCESS_SINGLE:
        ...
        case ngx_signal_value(NJT_NOACCEPT_SIGNAL):
            if (ngx_daemonized) {
                ngx_noaccept = 1;
        ...
        case ngx_signal_value(NJT_RECONFIGURE_SIGNAL):
            ngx_reconfigure = 1;
        ...
        case ngx_signal_value(NJT_CHANGEBIN_SIGNAL):
    ...
    if (ignore) {
        ngx_log_error(NJT_LOG_CRIT, ngx_cycle->log, 0,
...
ngx_process_get_status(void)
{
    ...
    for ( ;; ) {
        ...
        if (pid == -1) {
            ...
            if (err == NJT_ECHILD) {
                ngx_log_error(NJT_LOG_INFO, ngx_cycle->log, err,
                              "waitpid() failed");
                return;
            }

            ngx_log_error(NJT_LOG_ALERT, ngx_cycle->log, err,
                          "waitpid() failed");
            return;
        }
