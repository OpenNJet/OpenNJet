    ...
    ngx_pool_t *pool)
{
    ...
    if (n == -1) {
        ...
        ngx_log_error(NJT_LOG_CRIT, file->log, n,
    ...
    ngx_log_debug2(NJT_LOG_DEBUG_CORE, file->log, 0,
...
ngx_file_aio_result(ngx_file_t *file, ngx_event_aio_t *aio, ngx_event_t *ev)
{
    ...
    if (n == -1) {
        ...
        ngx_log_error(NJT_LOG_ALERT, file->log, err,
    ...
    if (n == NJT_EINPROGRESS) {
        if (ev->ready) {
            ev->ready = 0;
            ngx_log_error(NJT_LOG_ALERT, file->log, n,
...
ngx_file_aio_event_handler(ngx_event_t *ev)
{
    ...
    if (ngx_file_aio_result(aio->file, aio, ev) != NJT_AGAIN) {
        aio->handler(ev);
    }
}
