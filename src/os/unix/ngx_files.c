    ...
    ngx_pool_t *pool)
{
    ...
    if (task->event.complete) {
        ...
        if (ctx->err) {
            ngx_log_error(NJT_LOG_CRIT, file->log, ctx->err,
                          "pread() \"%s\" failed", file->name.data);
            return NJT_ERROR;
        }
...
ngx_thread_read_handler(void *data, ngx_log_t *log)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_CORE, log, 0, "thread read handler");
    ...
    ngx_log_debug4(NJT_LOG_DEBUG_CORE, log, 0,
...
ngx_write_file(ngx_file_t *file, u_char *buf, size_t size, off_t offset)
{
    ...
    for ( ;; ) {
        ...
        if (n == -1) {
            ...
            if (err == NJT_EINTR) {
                ngx_log_debug0(NJT_LOG_DEBUG_CORE, file->log, err,
...
ngx_writev_file(ngx_file_t *file, ngx_iovec_t *vec, off_t offset)
{
    ...
    ngx_log_debug3(NJT_LOG_DEBUG_CORE, file->log, 0,
...
eintr:
        ...
        if (n == -1) {
            ...
            if (err == NJT_EINTR) {
                ngx_log_debug0(NJT_LOG_DEBUG_CORE, log, err,
                               "pwritev() was interrupted");
                goto eintr;
            }
