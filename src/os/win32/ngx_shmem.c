...
#ifdef _WIN64
#define NJET_SHMEM_BASE  0x0000047047e00000
#else
#define NJET_SHMEM_BASE  0x2efe0000
#endif
...
ngx_shm_alloc(ngx_shm_t *shm)
{
    u_char         *name;
    uint64_t        size;
    static u_char  *base = (u_char *) NJET_SHMEM_BASE;

    name = ngx_alloc(shm->name.len + 2 + NJET_INT32_LEN, shm->log);
    if (name == NULL) {
        return NJET_ERROR;
    }
    ...
    ngx_log_debug3(NJET_LOG_DEBUG_CORE, shm->log, ngx_errno,
