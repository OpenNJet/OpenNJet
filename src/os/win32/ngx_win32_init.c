...
char        ngx_unique[NJT_INT32_LEN + 1];
...
#pragma warning(default:4996)
    ...
    if (ngx_win32_version < NJT_WIN_NT) {
        ngx_max_wsabufs = 16;
        return NJT_OK;
    }
    ...
    if (WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &ax_guid, sizeof(GUID),
        ...
        == -1)
    {
        ngx_log_error(NJT_LOG_NOTICE, log, ngx_socket_errno,
    ...
    if (hmod == NULL) {
        ngx_log_error(NJT_LOG_NOTICE, log, ngx_errno,
...
nopoll:
    ...
    if (GetEnvironmentVariable("ngx_unique", ngx_unique, NJT_INT32_LEN + 1)
        != 0)
    {
...
ngx_os_status(ngx_log_t *log)
{
    ...
    if (osviex) {
        ...
        ngx_log_error(NJT_LOG_INFO, log, 0,
        ...
        if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
            ...
            ngx_log_error(NJT_LOG_INFO, log, 0, "OS: %ui build:%ud, \"%s\"",
