    ...
    ngx_stream_access_srv_conf_t *ascf, in_addr_t addr)
{
    ...
    for (i = 0; i < ascf->rules->nelts; i++) {
        ...
        ngx_log_debug3(NJT_LOG_DEBUG_STREAM, s->connection->log, 0,
    ...
    ngx_stream_access_srv_conf_t *ascf, u_char *p)
{
    ...
    for (i = 0; i < ascf->rules6->nelts; i++) {
        ...
        cl = ngx_inet6_ntop(p, ct, NJT_INET6_ADDRSTRLEN);
        ml = ngx_inet6_ntop(rule6[i].mask.s6_addr, mt, NJT_INET6_ADDRSTRLEN);
        al = ngx_inet6_ntop(rule6[i].addr.s6_addr, at, NJT_INET6_ADDRSTRLEN);

        ngx_log_debug6(NJT_LOG_DEBUG_STREAM, s->connection->log, 0,
...
ngx_stream_access_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[NJT_STREAM_ACCESS_PHASE].handlers);
    if (h == NULL) {
        return NJT_ERROR;
    }
