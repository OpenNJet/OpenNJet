...
#define njt_stream_upstream_tries(p) ((p)->tries + ((p)->next ? (p)->next->tries : 0))
...
njt_stream_upstream_get_peer(njt_stream_upstream_rr_peer_data_t *rrp)
{
         ...
         peer = peer->next, i++)
    {
        ...
        if (peer != NULL)
        {
            njt_log_error(NJT_LOG_DEBUG, njt_cycle->log, 0, "peer ip=%V,rr_current_weight=%d,rr_effective_weight=%d,peer_slow_weight=%d", &peer->name, peer->rr_current_weight, peer->rr_effective_weight, peer_slow_weight);
        }
...
void njt_stream_upstream_free_round_robin_peer(njt_peer_connection_t *pc, void *data,
                                               njt_uint_t state)
{
    njt_stream_upstream_rr_peer_data_t *rrp = data;
                                            ...
                                            void *data, njt_uint_t type)
{
    ...
    if (type == NJT_STREAM_UPSTREAM_NOTIFY_CONNECT && pc->connection->type == SOCK_STREAM)
    {
        njt_stream_upstream_rr_peers_rlock(rrp->peers);
        njt_stream_upstream_rr_peer_lock(rrp->peers, peer);
...
void njt_stream_upstream_del_round_robin_peer(njt_slab_pool_t *pool, njt_stream_upstream_rr_peer_t *peer)
{
    if (peer->server.data)
    {
