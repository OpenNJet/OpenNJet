...
static njt_stream_upstream_rr_peer_t *njt_stream_upstream_zone_copy_peer(
    njt_stream_upstream_rr_peers_t *peers, njt_stream_upstream_rr_peer_t *src);
static njt_int_t
njt_stream_upstream_merge_zone(njt_shm_zone_t *shm_zone, void *data);
...
static njt_command_t  njt_stream_upstream_zone_commands[] = {

    { njt_string("zone"),
      NJT_STREAM_UPS_CONF|NJT_CONF_TAKE12,
      njt_stream_upstream_zone,
      0,
      0,
      NULL },
...
static njt_stream_module_t  njt_stream_upstream_zone_module_ctx = {
...
njt_module_t  njt_stream_upstream_zone_module = {
    NJT_MODULE_V1,
    &njt_stream_upstream_zone_module_ctx,  /* module context */
    njt_stream_upstream_zone_commands,     /* module directives */
...
static char *
njt_stream_upstream_zone(njt_conf_t *cf, njt_command_t *cmd, void *conf)
{
    ssize_t                           size;
    njt_str_t                        *value;
    ...
    if(cf->dynamic == 1) {
        if(uscf->shm_zone == NULL) {
            ...
            uscf->shm_zone->tag = &njt_stream_upstream_module;
    ...
    uscf->shm_zone->init = njt_stream_upstream_init_zone;
    uscf->shm_zone->merge = njt_stream_upstream_merge_zone;
...
njt_stream_upstream_merge_zone(njt_shm_zone_t *shm_zone, void *data)
{
    ...
    njt_stream_upstream_srv_conf_t   *uscf, **uscfp,*old_uscf,**old_uscfp;;
    njt_stream_upstream_main_conf_t  *umcf,*old_umcf;
...
njt_stream_upstream_init_zone(njt_shm_zone_t *shm_zone, void *data)
{
    ...
    peersp = (njt_stream_upstream_rr_peers_t **) (void *) &shpool->data;
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        peers = njt_stream_upstream_zone_copy_peers(shpool, uscf);
        if (peers == NULL) {
            return NJT_ERROR;
        }
...
static njt_stream_upstream_rr_peers_t *
njt_stream_upstream_zone_copy_peers(njt_slab_pool_t *shpool,
    njt_stream_upstream_srv_conf_t *uscf)
{
    njt_str_t                       *name;
    ...
    njt_memcpy(peers, uscf->peer.data, sizeof(njt_stream_upstream_rr_peers_t));
    ...
    for (peerp = &peers->peer; *peerp; peerp = &peer->next) {
        /* pool is unlocked */
        peer = njt_stream_upstream_zone_copy_peer(peers, *peerp);
        if (peer == NULL) {
            return NULL;
        }
    ...
    backup = njt_slab_alloc(shpool, sizeof(njt_stream_upstream_rr_peers_t));
    if (backup == NULL) {
        return NULL;
    }
    if(peers->next != NULL) {
       njt_memcpy(backup, peers->next, sizeof(njt_stream_upstream_rr_peers_t));
    }
    backup->name = name;
    ...
    for (peerp = &backup->peer; *peerp; peerp = &peer->next) {
        /* pool is unlocked */
        peer = njt_stream_upstream_zone_copy_peer(backup, *peerp);
        if (peer == NULL) {
            return NULL;
        }
	if(uscf->mandatory == 1 && uscf->persistent == 0) {
...
njt_stream_upstream_zone_copy_peer(njt_stream_upstream_rr_peers_t *peers,
    njt_stream_upstream_rr_peer_t *src)
{
    ...
    if (src) {
        njt_memcpy(dst, src, sizeof(njt_stream_upstream_rr_peer_t));
        dst->sockaddr = NULL;
        dst->name.data = NULL;
...
njt_stream_upstream_zone_inherit_peer_status (njt_stream_upstream_rr_peers_t *peers,
        ...
        njt_stream_upstream_rr_peer_t    *peer,*old_peer;
        ...
        for (peer = peers->peer; peer;peer = peer->next) {
                if(peer->parent_id == -1) {
                        for (old_peer = src_peers->peer; old_peer;old_peer = old_peer->next) {
                                if(peer->server.len == old_peer->server.len && njt_strncmp(peer->server.data,old_peer->server.data,peer->server.len) == 0){
