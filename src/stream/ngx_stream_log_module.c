...
ngx_stream_log_handler(ngx_stream_session_t *s)
{
    ...
    for (l = 0; l < lscf->logs->nelts; l++) {
        ...
        if (log[l].filter) {
            if (ngx_stream_complex_value(s, log[l].filter, &val) != NJT_OK) {
                return NJT_ERROR;
            }
    ...
    alloc_line:
        ...
        if (log[l].syslog_peer) {
            ...
            if (n < 0) {
                ngx_log_error(NJT_LOG_WARN, s->connection->log, 0,
    ...
    u_char *buf, size_t len)
{
    ...
    if (n == -1) {
        ...
        if (now - log->error_log_time > 59) {
            ngx_log_error(NJT_LOG_ALERT, s->connection->log, err,
    ...
    ngx_stream_log_script_t *script, u_char **name, u_char *buf, size_t len)
{
    ...
    *name = log.data;
        ...
        != NJT_OK)
    {
        ...
        ngx_log_error(NJT_LOG_CRIT, s->connection->log, ngx_errno,
    ...
    ngx_log_t *log)
{
    ...
    ngx_log_debug4(NJT_LOG_DEBUG_STREAM, log, 0,
    ...
    ngx_log_debug5(NJT_LOG_DEBUG_STREAM, log, 0,
...
ngx_stream_log_gzip_alloc(void *opaque, u_int items, u_int size)
{
    ...
    ngx_log_debug2(NJT_LOG_DEBUG_STREAM, pool->log, 0,
...
ngx_stream_log_gzip_free(void *opaque, void *address)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_STREAM, pool->log, 0,
                   "gzip free: %p", address);
#endif
}
...
ngx_stream_log_init(ngx_conf_t *cf)
{
    ...
    h = ngx_array_push(&cmcf->phases[NJT_STREAM_LOG_PHASE].handlers);
    if (h == NULL) {
        return NJT_ERROR;
    }
