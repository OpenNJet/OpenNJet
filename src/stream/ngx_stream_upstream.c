...
static ngx_command_t  ngx_stream_upstream_commands[] = {
    ...
    { ngx_string("upstream"),
      NJET_STREAM_MAIN_CONF|NJET_CONF_BLOCK|NJET_CONF_TAKE1,
      ngx_stream_upstream,
      0,
      0,
      NULL },
    ...
    { ngx_string("server"),
      NJET_STREAM_UPS_CONF|NJET_CONF_1MORE,
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    len = s->upstream_states->nelts * (NJET_OFF_T_LEN + 2);
    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    len = s->upstream_states->nelts * (NJET_TIME_T_LEN + 4 + 2);
...
ngx_stream_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    uscf = ngx_stream_upstream_add(cf, &u, NJET_STREAM_UPSTREAM_CREATE
    ...
    pcf = *cf;
    cf->ctx = ctx;
    cf->cmd_type = NJET_STREAM_UPS_CONF;
...
ngx_stream_upstream_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "weight=", 7) == 0) {
            ...
            if (!(uscf->flags & NJET_STREAM_UPSTREAM_WEIGHT)) {
                goto not_supported;
            }
        ...
        if (ngx_strncmp(value[i].data, "max_conns=", 10) == 0) {
            ...
            if (!(uscf->flags & NJET_STREAM_UPSTREAM_MAX_CONNS)) {
                goto not_supported;
            }
        ...
        if (ngx_strncmp(value[i].data, "max_fails=", 10) == 0) {
            ...
            if (!(uscf->flags & NJET_STREAM_UPSTREAM_MAX_FAILS)) {
                goto not_supported;
            }
        ...
        if (ngx_strncmp(value[i].data, "fail_timeout=", 13) == 0) {
            ...
            if (!(uscf->flags & NJET_STREAM_UPSTREAM_FAIL_TIMEOUT)) {
                goto not_supported;
            }
        ...
        if (ngx_strcmp(value[i].data, "backup") == 0) {
            ...
            if (!(uscf->flags & NJET_STREAM_UPSTREAM_BACKUP)) {
                goto not_supported;
            }
        ...
        if (ngx_strcmp(value[i].data, "down") == 0) {
            ...
            if (!(uscf->flags & NJET_STREAM_UPSTREAM_DOWN)) {
                goto not_supported;
            }
...
ngx_stream_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
    ...
    if (!(flags & NJET_STREAM_UPSTREAM_CREATE)) {
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        if ((flags & NJET_STREAM_UPSTREAM_CREATE)
             && (uscfp[i]->flags & NJET_STREAM_UPSTREAM_CREATE))
        {
            ngx_conf_log_error(NJET_LOG_EMERG, cf, 0,
        ...
        if ((uscfp[i]->flags & NJET_STREAM_UPSTREAM_CREATE) && !u->no_port) {
            ngx_conf_log_error(NJET_LOG_EMERG, cf, 0,
        ...
        if ((flags & NJET_STREAM_UPSTREAM_CREATE) && !uscfp[i]->no_port) {
