...
ngx_stream_init_connection(ngx_connection_t *c)
{
    ...
    ngx_log_error(NJT_LOG_INFO, c->log, 0, "*%uA %sclient %*s connected to %V",
...
ngx_stream_proxy_protocol_handler(ngx_event_t *rev)
{
    ...
    if (rev->timedout) {
        ...
        ngx_stream_finalize_session(s, NJT_STREAM_OK);
        return;
    }
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_STREAM, c->log, 0, "recv(): %z", n);
    ...
    if (p == NULL) {
        ngx_stream_finalize_session(s, NJT_STREAM_BAD_REQUEST);
        return;
    }
