    ...
    uintptr_t data)
{
    ...
    if (ngx_stream_complex_value(s, &map->value, &val) != NJT_OK) {
        return NJT_ERROR;
    }
    ...
    if (!value->valid) {
        ...
        if (ngx_stream_complex_value(s, cv, &str) != NJT_OK) {
            return NJT_ERROR;
        }
...
ngx_stream_map_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_hash_keys_array_init(&ctx.keys, NJT_HASH_LARGE) != NJT_OK) {
