    ...
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_stream_geo_addr(s, ctx, &addr) != NJT_OK) {
    ...
    ngx_addr_t *addr)
{
    ...
    if (ngx_parse_addr(s->connection->pool, addr, v->data, v->len) == NJT_OK) {
        return NJT_OK;
    }
...
ngx_stream_geo_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    var = ngx_stream_add_variable(cf, &name, NJT_STREAM_VAR_CHANGEABLE);
    if (var == NULL) {
        return NJT_CONF_ERROR;
    }
    ...
    ngx_str_t *value)
{
    ...
    if (ngx_stream_geo_cidr_value(cf, net, &cidr) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
