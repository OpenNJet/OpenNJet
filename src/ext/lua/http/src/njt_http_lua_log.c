...
static int njt_http_lua_print(lua_State *L);
static int njt_http_lua_njt_log(lua_State *L);
static int log_wrapper(njt_log_t *log, const char *ident,
    njt_uint_t level, lua_State *L);
static void njt_http_lua_inject_log_consts(lua_State *L);
 ...
 * */
int
njt_http_lua_njt_log(lua_State *L)
{
    njt_log_t                   *log;
    ...
    return log_wrapper(log, "[lua] ", (njt_uint_t) level, L);
}
 ...
 * */
int
njt_http_lua_print(lua_State *L)
{
    njt_log_t                   *log;
    njt_http_request_t          *r;
    ...
    return log_wrapper(log, "[lua] ", NJT_LOG_NOTICE, L);
}
    ...
    lua_State *L)
{
    ...
    *p++ = ':'; *p++ = ' ';
    ...
    for (i = 1; i <= nargs; i++) {
        ...
        switch (type) {
            case LUA_TNUMBER:
                p = njt_http_lua_write_num(L, i, p);
                break;
...
void
njt_http_lua_inject_log_api(lua_State *L)
{
    njt_http_lua_inject_log_consts(L);

    lua_pushcfunction(L, njt_http_lua_njt_log);
    lua_setfield(L, -2, "log");

    lua_pushcfunction(L, njt_http_lua_print);
...
njt_int_t
njt_http_lua_capture_log_handler(njt_log_t *log,
    njt_uint_t level, u_char *buf, size_t n)
{
    ...
    njt_http_lua_log_ringbuf_write(ringbuf, level, buf, n);
...
int
njt_http_lua_ffi_errlog_set_filter_level(int level, u_char *err, size_t *errlen)
{
#ifdef HAVE_INTERCEPT_ERROR_LOG_PATCH
    njt_http_lua_log_ringbuf_t     *ringbuf;
...
int
njt_http_lua_ffi_errlog_get_msg(char **log, int *loglevel, u_char *err,
    size_t *errlen, double *log_time)
{
    ...
    njt_http_lua_log_ringbuf_read(ringbuf, loglevel, (void **) log, &loglen,
                                  log_time);
    return loglen;
#else
    *errlen = njt_snprintf(err, *errlen,
...
int
njt_http_lua_ffi_errlog_get_sys_filter_level(njt_http_request_t *r)
{
    njt_log_t                   *log;
    int                          log_level;
...
int
njt_http_lua_ffi_raw_log(njt_http_request_t *r, int level, u_char *s,
    size_t s_len)
{
    njt_log_t           *log;
