...
typedef enum {
    ...
    NJT_LUA_BT_LUA,
    NJT_LUA_BT_LJ,
} njt_http_lua_clfactory_file_type_e;
...
typedef struct {
    njt_http_lua_clfactory_file_type_e file_type;
...
} njt_http_lua_clfactory_file_ctx_t;
...
typedef struct {
    ...
    const char *s;
    size_t      size;
} njt_http_lua_clfactory_buffer_ctx_t;


static const char *njt_http_lua_clfactory_getF(lua_State *L, void *ud,
    size_t *size);
static int njt_http_lua_clfactory_errfile(lua_State *L, const char *what,
    int fname_index);
static const char *njt_http_lua_clfactory_getS(lua_State *L, void *ud,
    size_t *size);
#ifndef OPENRESTY_LUAJIT
static long njt_http_lua_clfactory_file_size(FILE *f);
#endif


#ifndef OPENRESTY_LUAJIT
int
njt_http_lua_clfactory_bytecode_prepare(lua_State *L,
    njt_http_lua_clfactory_file_ctx_t *lf, int fname_index)
{
    ...
    *lf->begin_code.str = LUA_SIGNATURE[0];
    ...
    if (lf->file_type == NJT_LUA_BT_LJ) {
        ...
        fsize = njt_http_lua_clfactory_file_size(lf->f);
        if (fsize < 0) {
            serr = strerror(errno);
...
njt_int_t
njt_http_lua_clfactory_loadfile(lua_State *L, const char *filename)
{
    int                         c, status, readstatus;
    njt_flag_t                  sharp;

    njt_http_lua_clfactory_file_ctx_t        lf;
    ...
    if (lf.f == NULL) {
        return njt_http_lua_clfactory_errfile(L, "open", fname_index);
    }
    ...
    if (c == LUA_SIGNATURE[0] && filename) {  /* binary file? */
        ...
        if (lf.f == NULL) {
            return njt_http_lua_clfactory_errfile(L, "reopen", fname_index);
        }
...
#ifndef OPENRESTY_LUAJIT
        status = njt_http_lua_clfactory_bytecode_prepare(L, &lf, fname_index);
    ...
    ungetc(c, lf.f);
#endif
    status = lua_load(L, njt_http_lua_clfactory_getF, &lf,
                      lua_tostring(L, -1));
    ...
    if (readstatus) {
        ...
        return njt_http_lua_clfactory_errfile(L, "read", fname_index);
    }
...
njt_int_t
njt_http_lua_clfactory_loadbuffer(lua_State *L, const char *buff,
    size_t size, const char *name)
{
    njt_http_lua_clfactory_buffer_ctx_t     ls;
    ...
    return lua_load(L, njt_http_lua_clfactory_getS, &ls, name);
}


static const char *
njt_http_lua_clfactory_getF(lua_State *L, void *ud, size_t *size)
{
#ifndef OPENRESTY_LUAJIT
    char                        *buf;
#endif
    ...
    lf = (njt_http_lua_clfactory_file_ctx_t *) ud;
...
static int
njt_http_lua_clfactory_errfile(lua_State *L, const char *what, int fname_index)
{
    const char      *serr;
    const char      *filename;
...
static const char *
njt_http_lua_clfactory_getS(lua_State *L, void *ud, size_t *size)
{
    njt_http_lua_clfactory_buffer_ctx_t      *ls = ud;
...
#ifndef OPENRESTY_LUAJIT
static long
njt_http_lua_clfactory_file_size(FILE *f)
{
    long              cur_pos, len;
