...
static int njt_http_lua_uthread_spawn(lua_State *L);
static int njt_http_lua_uthread_wait(lua_State *L);
static int njt_http_lua_uthread_kill(lua_State *L);


void
njt_http_lua_inject_uthread_api(njt_log_t *log, lua_State *L)
{
    /* new thread table */
    lua_createtable(L, 0 /* narr */, 3 /* nrec */);

    lua_pushcfunction(L, njt_http_lua_uthread_spawn);
    lua_setfield(L, -2, "spawn");

    lua_pushcfunction(L, njt_http_lua_uthread_wait);
    lua_setfield(L, -2, "wait");

    lua_pushcfunction(L, njt_http_lua_uthread_kill);
...
static int
njt_http_lua_uthread_spawn(lua_State *L)
{
    int                           n, co_ref;
    ...
    ctx->co_op = NJT_HTTP_LUA_USER_THREAD_RESUME;

    ctx->cur_co_ctx->thread_spawn_yielded = 1;

    if (njt_http_lua_post_thread(r, ctx, ctx->cur_co_ctx) != NJT_OK) {
    ...
    njt_http_lua_attach_co_ctx_to_L(coctx->co, coctx);

    njt_http_lua_probe_user_thread_spawn(r, L, coctx->co);
...
static int
njt_http_lua_uthread_wait(lua_State *L)
{
    int                          i, nargs, nrets;
    lua_State                   *sub_co;
    njt_http_request_t          *r;
    njt_http_lua_ctx_t          *ctx;
    njt_http_lua_co_ctx_t       *coctx, *sub_coctx;
    ...
    for (i = 1; i <= nargs; i++) {
        ...
        sub_coctx = njt_http_lua_get_co_ctx(sub_co, ctx);
        if (sub_coctx == NULL) {
        ...
        case NJT_HTTP_LUA_CO_ZOMBIE:

            njt_http_lua_probe_info("found zombie child");
...
#if 1
            njt_http_lua_del_thread(r, L, ctx, sub_coctx);
            ctx->uthreads--;
#endif

            return nrets;

        case NJT_HTTP_LUA_CO_DEAD:
        ...
        njt_http_lua_probe_user_thread_wait(L, sub_coctx->co);
...
static int
njt_http_lua_uthread_kill(lua_State *L)
{
    lua_State                   *sub_co;
    njt_http_request_t          *r;
    njt_http_lua_ctx_t          *ctx;
    ...
    default:
        njt_http_lua_cleanup_pending_operation(sub_coctx);
        njt_http_lua_del_thread(r, L, ctx, sub_coctx);
        ctx->uthreads--;
