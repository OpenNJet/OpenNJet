...
typedef struct {
    ...
    int                      captures_len;
    uint8_t                  flags;
} njt_http_lua_regex_ctx_t;


static njt_int_t njt_http_lua_regex_compile(njt_http_lua_regex_compile_t *rc);


#define njt_http_lua_regex_exec(re, e, s, start, captures, size, opts)       \
    pcre_exec(re, e, (const char *) (s)->data, (s)->len, start, opts,        \
              captures, size)


#define njt_http_lua_regex_dfa_exec(re, e, s, start, captures, size, ws,     \
                                    wscount, opts)                           \
...
static void
njt_http_lua_regex_free_study_data(njt_pool_t *pool, njt_http_lua_regex_t *re)
{
    njt_pool_t  *old_pool;

#if (NJT_PCRE2)
    if (re && re->regex) {
        old_pool = njt_http_lua_pcre_malloc_init(pool);
...
#if (NJT_PCRE2)
static njt_int_t
njt_http_lua_regex_compile(njt_http_lua_regex_compile_t *rc)
{
    int                     n, errcode;
    char                   *p;
    size_t                  erroff;
    ...
    if (njt_regex_compile_context == NULL) {
        ...
        old_pool = njt_http_lua_pcre_malloc_init(NULL);

        gctx = pcre2_general_context_create(njt_http_lua_pcre_malloc,
                                            njt_http_lua_pcre_free,
                                            NULL);
        if (gctx == NULL) {
            njt_http_lua_pcre_malloc_done(old_pool);
            goto nomem;
    ...
    old_pool = njt_http_lua_pcre_malloc_init(rc->pool);
...
njt_int_t
njt_http_lua_ffi_set_jit_stack_size(int size, u_char *errstr,
    size_t *errstr_size)
{
#if (LUA_HAVE_PCRE_JIT)
...
#if (NJT_PCRE2)
static void
njt_http_lua_regex_jit_compile(njt_http_lua_regex_t *re, int flags,
    njt_pool_t *pool, njt_http_lua_main_conf_t *lmcf,
    njt_http_lua_regex_compile_t *re_comp)
{
    njt_int_t    ret;
    njt_pool_t  *old_pool;
...
#if (NJT_PCRE2)
void
njt_http_lua_regex_cleanup(void *data)
{
    njt_pool_t                *old_pool;
    njt_http_lua_main_conf_t  *lmcf;
...
njt_http_lua_regex_t *
njt_http_lua_ffi_compile_regex(const unsigned char *pat, size_t pat_len,
    ...
    size_t errstr_size)
{
    ...
    re = njt_palloc(pool, sizeof(njt_http_lua_regex_t));
    if (re == NULL) {
    ...
    old_pool = njt_http_lua_pcre_malloc_init(pool);
    rc = njt_http_lua_regex_compile(&re_comp);
    njt_http_lua_pcre_malloc_done(old_pool);
    ...
    njt_http_lua_regex_jit_compile(re, flags, pool, lmcf, &re_comp);
...
error:
    ...
    *p = '\0';
    ...
    njt_http_lua_regex_free_study_data(pool, re);
...
#if (NJT_PCRE2)
int
njt_http_lua_ffi_exec_regex(njt_http_lua_regex_t *re, int flags,
    ...
    const u_char *s, size_t len, int pos)
{
    ...
    if (flags & NJT_LUA_RE_MODE_DFA) {
        ...
        int ws[NJT_LUA_RE_DFA_MODE_WORKSPACE_COUNT];
        rc = njt_http_lua_regex_dfa_exec(re->regex, sd, &subj,
        ...
        rc = njt_http_lua_regex_exec(re->regex, sd, &subj, (int) pos, cap,
                                     ovecsize, exec_opts);
    }
...
void
njt_http_lua_ffi_destroy_regex(njt_http_lua_regex_t *re)
{
    ...
    njt_http_lua_regex_free_study_data(re->pool, re);
...
int
njt_http_lua_ffi_compile_replace_template(njt_http_lua_regex_t *re,
    const u_char *replace_data, size_t replace_len)
{
    njt_int_t                                rc;
    njt_str_t                                tpl;
    njt_http_lua_complex_value_t            *ctpl;
    njt_http_lua_compile_complex_value_t     ccv;

    ctpl = njt_palloc(re->pool, sizeof(njt_http_lua_complex_value_t));
    if (ctpl == NULL) {
        return NJT_ERROR;
    }
    ...
    njt_memzero(&ccv, sizeof(njt_http_lua_compile_complex_value_t));
    ccv.pool = re->pool;
    ccv.log = njt_cycle->log;
    ccv.value = &tpl;
    ccv.complex_value = ctpl;

    rc = njt_http_lua_compile_complex_value(&ccv);
...
njt_http_lua_ffi_create_script_engine(void)
{
    return njt_calloc(sizeof(njt_http_lua_script_engine_t), njt_cycle->log);
}


void
njt_http_lua_ffi_init_script_engine(njt_http_lua_script_engine_t *e,
    const unsigned char *subj, njt_http_lua_regex_t *compiled, int count)
{
    e->log = njt_cycle->log;
    e->ncaptures = count * 2;
...
void
njt_http_lua_ffi_destroy_script_engine(njt_http_lua_script_engine_t *e)
{
    njt_free(e);
}
...
size_t
njt_http_lua_ffi_script_eval_len(njt_http_lua_script_engine_t *e,
    njt_http_lua_complex_value_t *val)
{
    size_t          len;
...
void
njt_http_lua_ffi_script_eval_data(njt_http_lua_script_engine_t *e,
    njt_http_lua_complex_value_t *val, u_char *dst)
{
...
uint32_t
njt_http_lua_ffi_max_regex_cache_size(void)
{
    njt_http_lua_main_conf_t    *lmcf;
    lmcf = njt_http_cycle_get_module_main_conf(njt_cycle,
                                               njt_http_lua_module);
