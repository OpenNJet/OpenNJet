...
static void njt_http_lua_regex_free_study_data(njt_pool_t *pool,
    pcre_extra *sd);
...
static void
njt_http_lua_regex_free_study_data(njt_pool_t *pool, pcre_extra *sd)
{
    njt_pool_t              *old_pool;
...
error:
    ...
    *p = '\0';
    ...
    if (sd) {
        njt_http_lua_regex_free_study_data(pool, sd);
    }
...
njt_http_lua_ffi_destroy_regex(njt_http_lua_regex_t *re)
{
    ...
    if (re->regex_sd) {
        old_pool = njt_http_lua_pcre_malloc_init(re->pool);
#if LUA_HAVE_PCRE_JIT
