...
--- config
    ...
    location /t {
        ...
        content_by_lua '
            ...
            local port1 = njt.var.port1
            local port2 = njt.var.port2
...
--- config
    ...
    location /t {
        ...
        content_by_lua '
            ...
            local resp = njt.location.capture("/memc")
            if type(resp) ~= "table" then
                njt.say("bad resp: type ", type(resp), ": ", resp)
                return
            end

            njt.print("subrequest: ", resp.status, ", ", resp.body)
        ';
    }
...
--- config
    ...
    location = /sub {
        content_by_lua '
            ...
            if not ok then
                njt.say("failed to connect to xxx: ", err)
                return
            end
            njt.say("successfully connected to xxx!")
            sock:close()
        ';
    }
...
--- config
    ...
    location = /sub {
        content_by_lua '
            ...
            if not ok then
                njt.say("failed to connect to ", server, ": ", err)
                return
            end
            njt.say("successfully connected to xxx!")
            sock:close()
...
F(njt_http_lua_socket_tcp_conn_retval_handler) {
    println("lua socket tcp connect retval handler")
}

F(njt_http_run_posted_requests) {
...
--- config
    ...
    location /t {
        content_by_lua '
            local sock = njt.socket.tcp()
            local ok, err = sock:connect("127.0.0.1", njt.var.server_port)
            if not ok then
                njt.say("failed to connect: ", err)
                return
            end
...
--- config
    ...
    location /foo {
        content_by_lua 'njt.sleep(0.1) njt.say("foo")';
        more_clear_headers Date;
    }
...
--- config
    ...
    location /t {
        ...
        content_by_lua '
            local test = require "test"
            if njt.var.arg_reset then
                test.new_sock()
            end
            local sock = test.get_sock()
            local ok, err = sock:connect("127.0.0.1", njt.var.port)
            if not ok then
                njt.say("failed to connect: ", err)
            else
                njt.say("connected")
...
=== TEST 57: reuse cleanup in njt.timer (fake_request)
--- config
    server_tokens off;
    location /t {
        #set $port 5000;
...
=== TEST 58: free cleanup in njt.timer (without sock:close)
--- config
    server_tokens off;
    location /t {
        #set $port 5000;
...
--- config
    location /t {
        ...
        content_by_lua_block {
            ...
            local ok, err = sock:close()
            if not ok then
                njt.log(njt.ERR, "failed to close socket: ", err)
                return
            end
...
--- config
    ...
    location /t {
        ...
        content_by_lua_block {
            ...
            njt.say("request sent: ", total_bytes)
