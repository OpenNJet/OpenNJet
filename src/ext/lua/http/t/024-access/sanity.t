...
--- config
    location /lua {
        ...
        access_by_lua 'njt.print("Hello, Lua!\\n")';
...
--- config
    location /lua {
        access_by_lua 'njt.echo("Hello, Lua!\\n")';
        content_by_lua 'njt.exit(njt.OK)';
    }
--- request
GET /lua
...
--- config
    location /lua {
        # NOTE: the newline escape sequence must be double-escaped, as nginx config
        # parser will unescape first!
        access_by_lua 'local v = njt.var["request_uri"] njt.print("request_uri: ", v, "\\n")';
        content_by_lua 'njt.exit(njt.OK)';
    }
--- request
GET /lua?a=1&b=2
...
--- config
    location = /lua {
        access_by_lua 'local who = njt.var.arg_who
            njt.print("Hello, ", who, "!")';
        content_by_lua 'njt.exit(njt.OK)';
    }
--- request
...
--- config
    location /lua {
        access_by_lua 'local res = njt.location.capture("/other"); njt.print("status=", res.status)';
        content_by_lua 'njt.exit(njt.OK)';
    }
--- request
GET /lua
...
--- config
    location /lua {
        access_by_lua 'local res = njt.location.capture("*(#*"); njt.say("res=", res.status)';
        content_by_lua 'njt.exit(njt.OK)';
    }
--- request
GET /lua
...
--- config
    location /lua {
        access_by_lua 'njt.say(true, " ", false)';
        content_by_lua return;
    }
--- request
GET /lua
...
--- config
    location /lua {
        access_by_lua 'njt.location.capture(nil)';
        content_by_lua 'njt.exit(njt.OK)';
    }
--- request
GET /lua
...
--- config
 location /set {
       set $a "";
       access_by_lua 'njt.var.a = 32; njt.say(njt.var.a)';
       content_by_lua 'njt.exit(njt.OK)';
       add_header Foo $a;
   }
--- request
...
--- config
 location /set {
       ...
       access_by_lua 'njt.say(njt.quote_sql_str(njt.var.a))';
       content_by_lua 'njt.exit(njt.OK)';
   }
--- request
GET /set
...
location /set {
    #set $a "hello\n\r'\"\\";
    access_by_lua 'njt.say(njt.quote_sql_str("hello\\n\\r\'\\"\\\\"))';
    content_by_lua 'njt.exit(njt.OK)';
}
--- request
GET /set
...
location /parent {
    set $a 12;
    access_by_lua 'local res = njt.location.capture("/sub"); njt.print(res.body)';
    content_by_lua 'njt.exit(njt.OK)';
}
--- request
GET /parent
...
=== TEST 27: access_by_lua runs after njt_access
--- config
    location /lua {
        deny all;
...
=== TEST 28: auth_request runs before njt_access
--- config
    location /lua {
        deny all;
...
--- config
    location ~ '^/lua/(.+)' {
        set $path $1;
        access_by_lua 'njt.say(njt.var.path)';
        content_by_lua return;
    }
    location ~ '^/lua2/(.+)' {
        set $path $1;
...
--- config
    access_by_lua 'njt.header["X-Foo"] = "bar" -- njt.send_headers()';
--- request
GET /
--- response_body chop
...
=== TEST 37: use of njt.say() in access_by_lua without exiting with 200+.
--- config
    location /t {
        access_by_lua "njt.say('test')";
        echo_exec /t2;
    }
--- request
    GET /t
...
=== TEST 38: use of njt.say() in access_by_lua without exiting with 200+. (with explicit njt.eof())
--- config
    location /t {
        access_by_lua "njt.say('test') njt.eof()";
        echo_exec /t2;
    }
--- request
    GET /t
...
=== TEST 39: use of njt.say() in access_by_lua without exiting with 200+. (with IO)
--- config
    location /t {
        access_by_lua "njt.say('test') njt.sleep(0.001)";
        echo_exec /t2;
    }
--- request
    GET /t
