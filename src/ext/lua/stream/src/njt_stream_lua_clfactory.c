 ...
 *
 * src/subsys/njt_subsys_lua_clfactory.c.tt2
 */
...
typedef enum {
    ...
    NJT_LUA_BT_LUA,
    NJT_LUA_BT_LJ
} njt_stream_lua_clfactory_file_type_e;


enum {
    NJT_LUA_READER_BUFSIZE = 4096
};


typedef struct {
    njt_stream_lua_clfactory_file_type_e       file_type;
...
#endif /* OPENRESTY_LUAJIT */
    char        buff[NJT_LUA_READER_BUFSIZE];
} njt_stream_lua_clfactory_file_ctx_t;
...
typedef struct {
    ...
    const char *s;
    size_t      size;
} njt_stream_lua_clfactory_buffer_ctx_t;


static const char *njt_stream_lua_clfactory_getF(lua_State *L, void *ud,
    size_t *size);
static int njt_stream_lua_clfactory_errfile(lua_State *L, const char *what,
    int fname_index);
static const char *njt_stream_lua_clfactory_getS(lua_State *L, void *ud,
    size_t *size);
#ifndef OPENRESTY_LUAJIT
static long njt_stream_lua_clfactory_file_size(FILE *f);
#endif


#ifndef OPENRESTY_LUAJIT
int
njt_stream_lua_clfactory_bytecode_prepare(lua_State *L,
    njt_stream_lua_clfactory_file_ctx_t *lf, int fname_index)
{
    ...
    *lf->begin_code.str = LUA_SIGNATURE[0];
    ...
    if (lf->file_type == NJT_LUA_BT_LJ) {
        ...
        if (njt_memcmp(lf->begin_code.str, LJ_SIGNATURE,
        ...
        fsize = njt_stream_lua_clfactory_file_size(lf->f);
        if (fsize < 0) {
            serr = strerror(errno);
        ...
        if (njt_memcmp(lf->begin_code.str, LUA_SIGNATURE,
         ...
         * - last line
         */
        njt_memzero(lf->begin_code.str + POS_SOURCE_STR_LEN,
        ...
        *(lf->begin_code.str + POS_MAX_STACK_SIZE) = 2;
        ...
        njt_memcpy(lf->begin_code.str + POS_NUM_OF_INST, &num_of_inst,
                   sizeof(int));
        ...
        /* bytecode */
        njt_memcpy(lf->begin_code.str + POS_BYTECODE, bytecode, bytecode_len);

        /* number of consts */
        njt_memzero(lf->begin_code.str + POS_BYTECODE + bytecode_len,
                    sizeof(int));
        /* number of internal functions */
        njt_memcpy(lf->begin_code.str + POS_BYTECODE + bytecode_len
         ...
         * - number of upvalues
         */
        njt_memzero(lf->end_code.str, sizeof(int) * 3);
...
njt_int_t
njt_stream_lua_clfactory_loadfile(lua_State *L, const char *filename)
{
    int                         c, status, readstatus;
    njt_flag_t                  sharp;

    njt_stream_lua_clfactory_file_ctx_t              lf;
    ...
    lf.extraline = 0;
    lf.file_type = NJT_LUA_TEXT_FILE;
    ...
    if (lf.f == NULL) {
        return njt_stream_lua_clfactory_errfile(L, "open", fname_index);
    }
    ...
    if (c == LUA_SIGNATURE[0] && filename) {  /* binary file? */
        ...
        if (lf.f == NULL) {
            return njt_stream_lua_clfactory_errfile(L, "reopen", fname_index);
        }
        ...
        if (lua_istable(L, -1)) {
            lf.file_type = NJT_LUA_BT_LJ;

        } else {
            lf.file_type = NJT_LUA_BT_LUA;
        }
         ...
         */
        if (lf.file_type == NJT_LUA_BT_LJ && sharp) {
...
#ifndef OPENRESTY_LUAJIT
        status = njt_stream_lua_clfactory_bytecode_prepare(L, &lf, fname_index);
...
#ifndef OPENRESTY_LUAJIT
    if (lf.file_type == NJT_LUA_TEXT_FILE) {
        ungetc(c, lf.f);
    }
    ...
    ungetc(c, lf.f);
#endif
    status = lua_load(L, njt_stream_lua_clfactory_getF, &lf,
                      lua_tostring(L, -1));
    ...
    if (readstatus) {
        ...
        return njt_stream_lua_clfactory_errfile(L, "read", fname_index);
    }
...
njt_int_t
njt_stream_lua_clfactory_loadbuffer(lua_State *L, const char *buff,
    size_t size, const char *name)
{
    njt_stream_lua_clfactory_buffer_ctx_t           ls;
    ...
    return lua_load(L, njt_stream_lua_clfactory_getS, &ls, name);
}


static const char *
njt_stream_lua_clfactory_getF(lua_State *L, void *ud, size_t *size)
{
#ifndef OPENRESTY_LUAJIT
    char                        *buf;
#endif
    ...
    lf = (njt_stream_lua_clfactory_file_ctx_t *) ud;
    ...
    if (num == 0) {
        ...
        if (lf->sent_end == 0) {
            ...
            *size = lf->end_code_len;
            ...
            if (lf->file_type == NJT_LUA_BT_LUA) {
                buf = lf->end_code.str;
...
static int
njt_stream_lua_clfactory_errfile(lua_State *L, const char *what,
    int fname_index)
{
    const char      *serr;
    const char      *filename;
...
static const char *
njt_stream_lua_clfactory_getS(lua_State *L, void *ud, size_t *size)
{
    njt_stream_lua_clfactory_buffer_ctx_t            *ls = ud;
...
#ifndef OPENRESTY_LUAJIT
static long
njt_stream_lua_clfactory_file_size(FILE *f)
{
    long              cur_pos, len;
