 ...
 *
 * src/subsys/njt_subsys_lua_shdict.c.tt2
 */
...
static int njt_stream_lua_shdict_expire(njt_stream_lua_shdict_ctx_t *ctx,
    njt_uint_t n);
static njt_int_t njt_stream_lua_shdict_lookup(njt_shm_zone_t *shm_zone,
    njt_uint_t hash, u_char *kdata, size_t klen,
    njt_stream_lua_shdict_node_t **sdp);
static int njt_stream_lua_shdict_flush_expired(lua_State *L);
static int njt_stream_lua_shdict_get_keys(lua_State *L);
static int njt_stream_lua_shdict_lpush(lua_State *L);
static int njt_stream_lua_shdict_rpush(lua_State *L);
static int njt_stream_lua_shdict_push_helper(lua_State *L, int flags);
static int njt_stream_lua_shdict_lpop(lua_State *L);
static int njt_stream_lua_shdict_rpop(lua_State *L);
static int njt_stream_lua_shdict_pop_helper(lua_State *L, int flags);
static int njt_stream_lua_shdict_llen(lua_State *L);


static njt_inline njt_shm_zone_t *njt_stream_lua_shdict_get_zone(lua_State *L,
    int index);


#define NJT_STREAM_LUA_SHDICT_ADD         0x0001
#define NJT_STREAM_LUA_SHDICT_REPLACE     0x0002
#define NJT_STREAM_LUA_SHDICT_SAFE_STORE  0x0004


#define NJT_STREAM_LUA_SHDICT_LEFT        0x0001
#define NJT_STREAM_LUA_SHDICT_RIGHT       0x0002
...
njt_stream_lua_shdict_get_list_head(njt_stream_lua_shdict_node_t *sd,
    size_t len)
{
...
njt_int_t
njt_stream_lua_shdict_init_zone(njt_shm_zone_t *shm_zone, void *data)
{
    njt_stream_lua_shdict_ctx_t        *octx = data;
    ...
    ctx->sh = njt_slab_alloc(ctx->shpool,
                             sizeof(njt_stream_lua_shdict_shctx_t));
    if (ctx->sh == NULL) {
        return NJT_ERROR;
    }
    ...
    njt_rbtree_init(&ctx->sh->rbtree, &ctx->sh->sentinel,
                    njt_stream_lua_shdict_rbtree_insert_value);
...
void
njt_stream_lua_shdict_rbtree_insert_value(njt_rbtree_node_t *temp,
    njt_rbtree_node_t *node, njt_rbtree_node_t *sentinel)
{
    njt_rbtree_node_t           **p;

    njt_stream_lua_shdict_node_t         *sdn, *sdnt;
    ...
    for ( ;; ) {
        ...
        if (node->key < temp->key) {
            ...
            sdn = (njt_stream_lua_shdict_node_t *) &node->color;
            sdnt = (njt_stream_lua_shdict_node_t *) &temp->color;
...
static njt_int_t
njt_stream_lua_shdict_lookup(njt_shm_zone_t *shm_zone, njt_uint_t hash,
    u_char *kdata, size_t klen, njt_stream_lua_shdict_node_t **sdp)
{
    njt_int_t                    rc;
    njt_time_t                  *tp;
    uint64_t                     now;
    ...
    njt_stream_lua_shdict_ctx_t         *ctx;
    njt_stream_lua_shdict_node_t        *sd;
    ...
    while (node != sentinel) {
        ...
        sd = (njt_stream_lua_shdict_node_t *) &node->color;
...
static int
njt_stream_lua_shdict_expire(njt_stream_lua_shdict_ctx_t *ctx, njt_uint_t n)
{
    njt_time_t                      *tp;
    uint64_t                         now;
    ...
    njt_stream_lua_shdict_node_t            *sd;
    njt_stream_lua_shdict_list_node_t       *lnode;
    ...
    while (n < 3) {
        ...
        sd = njt_queue_data(q, njt_stream_lua_shdict_node_t, queue);
        ...
        if (sd->value_type == SHDICT_TLIST) {
            list_queue = njt_stream_lua_shdict_get_list_head(sd,
                                                             sd->key_len);
            ...
            {
                lnode = njt_queue_data(lq, njt_stream_lua_shdict_list_node_t,
                                       queue);
...
void
njt_stream_lua_inject_shdict_api(njt_stream_lua_main_conf_t *lmcf, lua_State *L)
{
    njt_stream_lua_shdict_ctx_t         *ctx;
    ...
    if (lmcf->shdict_zones != NULL) {
        ...
        lua_pushcfunction(L, njt_stream_lua_shdict_lpush);
        lua_setfield(L, -2, "lpush");

        lua_pushcfunction(L, njt_stream_lua_shdict_rpush);
        lua_setfield(L, -2, "rpush");

        lua_pushcfunction(L, njt_stream_lua_shdict_lpop);
        lua_setfield(L, -2, "lpop");

        lua_pushcfunction(L, njt_stream_lua_shdict_rpop);
        lua_setfield(L, -2, "rpop");

        lua_pushcfunction(L, njt_stream_lua_shdict_llen);
        lua_setfield(L, -2, "llen");

        lua_pushcfunction(L, njt_stream_lua_shdict_flush_expired);
        lua_setfield(L, -2, "flush_expired");

        lua_pushcfunction(L, njt_stream_lua_shdict_get_keys);
...
njt_stream_lua_shdict_get_zone(lua_State *L, int index)
{
    njt_shm_zone_t      *zone;
    njt_shm_zone_t     **zone_udata;
...
static int
njt_stream_lua_shdict_flush_expired(lua_State *L)
{
    ...
    zone = njt_stream_lua_shdict_get_zone(L, 1);
    if (zone == NULL) {
...
static int
njt_stream_lua_shdict_get_keys(lua_State *L)
{
    njt_queue_t                 *q, *prev;
    njt_shm_zone_t              *zone;
    njt_time_t                  *tp;
...
static int
njt_stream_lua_shdict_lpush(lua_State *L)
{
    return njt_stream_lua_shdict_push_helper(L, NJT_STREAM_LUA_SHDICT_LEFT);
}


static int
njt_stream_lua_shdict_rpush(lua_State *L)
{
    return njt_stream_lua_shdict_push_helper(L, NJT_STREAM_LUA_SHDICT_RIGHT);
}


static int
njt_stream_lua_shdict_push_helper(lua_State *L, int flags)
{
    int                              n;
    njt_str_t                        key;
    uint32_t                         hash;
...
#if 1
    njt_stream_lua_shdict_expire(ctx, 1);
#endif

    rc = njt_stream_lua_shdict_lookup(zone, hash, key.data, key.len, &sd);
    ...
    if (rc == NJT_DONE) {
        ...
        queue = njt_stream_lua_shdict_get_list_head(sd, key.len);
             ...
             q = njt_queue_next(q))
        {
            ...
            lnode = njt_queue_data(q, njt_stream_lua_shdict_list_node_t, queue);
            njt_slab_free_locked(ctx->shpool, lnode);
        }
...
init_list:
    ...
    n = offsetof(njt_rbtree_node_t, color)
        + offsetof(njt_stream_lua_shdict_node_t, data)
        + key.len
        + sizeof(njt_queue_t);
...
push_node:
    ...
    n = offsetof(njt_stream_lua_shdict_list_node_t, data)
        + value.len;
    ...
    if (flags == NJT_STREAM_LUA_SHDICT_LEFT) {
...
static int
njt_stream_lua_shdict_lpop(lua_State *L)
{
    return njt_stream_lua_shdict_pop_helper(L, NJT_STREAM_LUA_SHDICT_LEFT);
}


static int
njt_stream_lua_shdict_rpop(lua_State *L)
{
    return njt_stream_lua_shdict_pop_helper(L, NJT_STREAM_LUA_SHDICT_RIGHT);
}


static int
njt_stream_lua_shdict_pop_helper(lua_State *L, int flags)
{
    int                              n;
    njt_str_t                        name;
    njt_str_t                        key;
    ...
    lnode = njt_queue_data(queue, njt_stream_lua_shdict_list_node_t, queue);
...
static int
njt_stream_lua_shdict_llen(lua_State *L)
{
    int                          n;
    njt_str_t                    key;
    uint32_t                     hash;
...
njt_shm_zone_t *
njt_stream_lua_find_zone(u_char *name_data, size_t name_len)
{
    njt_str_t                       *name;
    njt_uint_t                       i;
    njt_shm_zone_t                  *zone;
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        {
            ctx = (njt_stream_lua_shm_zone_ctx_t *) zone[i].data;
            return &ctx->zone;
        }
    }
...
njt_shm_zone_t *
njt_stream_lua_ffi_shdict_udata_to_zone(void *zone_udata)
{
    if (zone_udata == NULL) {
        return NULL;
    }
...
int
njt_stream_lua_ffi_shdict_store(njt_shm_zone_t *zone, int op, u_char *key,
    ...
    char **errmsg, int *forcible)
{
    ...
    *forcible = 0;
    ...
    case LUA_TNIL:
        if (op & (NJT_STREAM_LUA_SHDICT_ADD|NJT_STREAM_LUA_SHDICT_REPLACE)) {
    ...
    rc = njt_stream_lua_shdict_lookup(zone, hash, key, key_len, &sd);

    dd("lookup returns %d", (int) rc);

    if (op & NJT_STREAM_LUA_SHDICT_REPLACE) {
    ...
    if (op & NJT_STREAM_LUA_SHDICT_ADD) {
...
remove:
        ...
        if (sd->value_type == SHDICT_TLIST) {
            queue = njt_stream_lua_shdict_get_list_head(sd, key_len);
...
insert:
    ...
    if (node == NULL) {
        ...
        if (op & NJT_STREAM_LUA_SHDICT_SAFE_STORE) {
            njt_shmtx_unlock(&ctx->shpool->mutex);
        ...
        for (i = 0; i < 30; i++) {
            if (njt_stream_lua_shdict_expire(ctx, 0) == 0) {
                break;
            }
...
int
njt_stream_lua_ffi_shdict_get(njt_shm_zone_t *zone, u_char *key,
    size_t key_len, int *value_type, u_char **str_value_buf,
...
#if (NJT_DEBUG)
    njt_log_debug3(NJT_LOG_DEBUG_STREAM, ctx->log, 0,
...
int
njt_stream_lua_ffi_shdict_incr(njt_shm_zone_t *zone, u_char *key,
...
remove:
    ...
    if (sd->value_type == SHDICT_TLIST) {
        ...
        for (q = njt_queue_head(queue);
             ...
             q = njt_queue_next(q))
        {
            p = (u_char *) njt_queue_data(q, njt_stream_lua_shdict_list_node_t,
                                          queue);
...
int
njt_stream_lua_ffi_shdict_flush_all(njt_shm_zone_t *zone)
{
    njt_queue_t                 *q;
    ...
    njt_stream_lua_shdict_expire(ctx, 0);
...
static njt_int_t
njt_stream_lua_shdict_peek(njt_shm_zone_t *shm_zone, njt_uint_t hash,
    u_char *kdata, size_t klen, njt_stream_lua_shdict_node_t **sdp)
{
    njt_int_t                    rc;
    njt_rbtree_node_t           *node, *sentinel;
...
long
njt_stream_lua_ffi_shdict_get_ttl(njt_shm_zone_t *zone, u_char *key,
    size_t key_len)
{
    uint32_t                     hash;
    uint64_t                     now;
    ...
    rc = njt_stream_lua_shdict_peek(zone, hash, key, key_len, &sd);
...
int
njt_stream_lua_ffi_shdict_set_expire(njt_shm_zone_t *zone, u_char *key,
...
size_t
njt_stream_lua_ffi_shdict_capacity(njt_shm_zone_t *zone)
{
...
size_t
njt_stream_lua_ffi_shdict_free_space(njt_shm_zone_t *zone)
{
    size_t         bytes;
...
#if (NJT_DARWIN)
int
njt_stream_lua_ffi_shdict_get_macos(njt_stream_lua_shdict_get_params_t *p)
{
    return njt_stream_lua_ffi_shdict_get(p->zone,
...
int
njt_stream_lua_ffi_shdict_store_macos(njt_stream_lua_shdict_store_params_t *p)
{
    return njt_stream_lua_ffi_shdict_store(p->zone, p->op,
                                           (u_char *) p->key, p->key_len,
                                           p->value_type,
...
int
njt_stream_lua_ffi_shdict_incr_macos(njt_stream_lua_shdict_incr_params_t *p)
{
    return njt_stream_lua_ffi_shdict_incr(p->zone,
                                          (u_char *) p->key, p->key_len,
