 ...
 *
 * src/subsys/njt_subsys_lua_phase.c.tt2
 */
...
static int
njt_stream_lua_njt_get_phase(lua_State *L)
{
    ...
    switch (ctx->context) {
    case NJT_STREAM_LUA_CONTEXT_INIT_WORKER:
        lua_pushliteral(L, "init_worker");
        break;

    case NJT_STREAM_LUA_CONTEXT_SSL_CLIENT_HELLO:
        lua_pushliteral(L, "ssl_client_hello");
        break;

    case NJT_STREAM_LUA_CONTEXT_SSL_CERT:
        lua_pushliteral(L, "ssl_cert");
        break;

    case NJT_STREAM_LUA_CONTEXT_PREREAD:
        lua_pushliteral(L, "preread");
        break;

    case NJT_STREAM_LUA_CONTEXT_CONTENT:
        lua_pushliteral(L, "content");
        break;

    case NJT_STREAM_LUA_CONTEXT_LOG:
        lua_pushliteral(L, "log");
        break;

    case NJT_STREAM_LUA_CONTEXT_TIMER:
        lua_pushliteral(L, "timer");
        break;

    case NJT_STREAM_LUA_CONTEXT_BALANCER:
...
void
njt_stream_lua_inject_phase_api(lua_State *L)
{
    lua_pushcfunction(L, njt_stream_lua_njt_get_phase);
