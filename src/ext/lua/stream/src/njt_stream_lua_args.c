 ...
 *
 * src/subsys/njt_subsys_lua_args.c.tt2
 */
...
int
njt_stream_lua_parse_args(lua_State *L, u_char *buf, u_char *last, int max)
{
    u_char                      *p, *q;
    u_char                      *src, *dst;
    ...
    while (p != last) {
        if (*p == '=' && ! parsing_value) {
            ...
            njt_stream_lua_unescape_uri(&dst, &src, p - q,
            ...
            if (len == 0) {
                ...
                dd("setting table...");
                njt_stream_lua_set_multi_value_table(L, top);
            }
