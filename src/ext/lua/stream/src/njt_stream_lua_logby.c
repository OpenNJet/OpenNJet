 ...
 *
 * src/subsys/njt_subsys_lua_logby.c.tt2
 */
...
static njt_int_t njt_stream_lua_log_by_chunk(lua_State *L,
...
njt_int_t
njt_stream_lua_log_handler(njt_stream_session_t *r)
{
#if (NJT_STREAM_LUA_HAVE_MALLOC_TRIM)
...
#   endif
    ...
    ctx = njt_stream_get_module_ctx(r, njt_stream_lua_module);

    dd("ctx = %p", ctx);

    if (ctx == NULL) {
        ctx = njt_stream_lua_create_ctx(r);
        if (ctx == NULL) {
            return NJT_ERROR;
        }
    }

    ctx->context = NJT_STREAM_LUA_CONTEXT_LOG;
...
njt_int_t
njt_stream_lua_log_handler_inline(njt_stream_lua_request_t *r)
{
    lua_State                           *L;
    njt_int_t                            rc;
    njt_stream_lua_loc_conf_t           *llcf;
    ...
    return njt_stream_lua_log_by_chunk(L, r);
}


njt_int_t
njt_stream_lua_log_handler_file(njt_stream_lua_request_t *r)
{
    lua_State                               *L;
    njt_int_t                                rc;
    u_char                                  *script_path;
    ...
    if (njt_stream_complex_value(r->session, &llcf->log_src, &eval_src)
        != NJT_OK)
    {
        return NJT_ERROR;
    }
...
njt_int_t
njt_stream_lua_log_by_chunk(lua_State *L, njt_stream_lua_request_t *r)
{
    njt_int_t        rc;
    u_char          *err_msg;
    size_t           len;
