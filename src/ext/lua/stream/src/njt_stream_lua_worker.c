 ...
 *
 * src/subsys/njt_subsys_lua_worker.c.tt2
 */
...
int
njt_stream_lua_ffi_worker_pid(void)
{
    return (int) njt_pid;
}
...
int
njt_stream_lua_ffi_worker_exiting(void)
{
...
int
njt_stream_lua_ffi_worker_count(void)
{
    njt_core_conf_t   *ccf;
...
int
njt_stream_lua_ffi_master_pid(void)
{
#if defined(njet_version) && njet_version >= 1013008
...
int
njt_stream_lua_ffi_get_process_type(void)
{
    njt_core_conf_t  *ccf;
...
int
njt_stream_lua_ffi_enable_privileged_agent(char **err, unsigned int connections)
#else
int
njt_stream_lua_ffi_enable_privileged_agent(char **err)
#endif
{
...
void
njt_stream_lua_ffi_process_signal_graceful_exit(void)
{
    njt_quit = 1;
}
