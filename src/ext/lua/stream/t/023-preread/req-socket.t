...
server {
    ...
    content_by_lua_block {
        ...
        if data then
            njt.log(njt.DEBUG, "upstream received: ", data)
...
--- stream_server_config
    ssl_preread on;
    preread_by_lua_block {
        local rsock = assert(njt.req.socket())

        local data, err = rsock:peek(2)
        if not data then
            njt.log(njt.ERR, "failed to peek the request socket: ", err)
            return
        end

        local n = njt.var.ssl_preread_server_name
        if not n or n == '' then
            njt.log(njt.INFO, "$ssl_preread_server_name is empty")
