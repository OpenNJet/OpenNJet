...
void
ndk_clean_path (njt_str_t *path, njt_uint_t complex, size_t off)
{
    u_char         *s, *p, *m, *e, c, *l;
    njt_uint_t      root;
...
void
ndk_path_to_dir_safe (njt_str_t *path, njt_uint_t complex, size_t off)
{
    size_t   len;
    u_char  *p, *m;
...
njt_array_t *
ndk_split_path_create (njt_conf_t *cf, njt_str_t *path)
{
    njt_str_t         *str;
    int                n;
    u_char            *m, *s, *e;
    ...
    a = njt_array_create (cf->pool, n + 1, sizeof (njt_str_t));
    if (a == NULL) {
        return  NULL;
    }
    ...
    while (s < e) {
        ...
        if (njt_conf_full_name (cf->cycle, str, 0) == NJT_ERROR)
            return  NULL;
...
njt_array_t *
ndk_split_path_create_raw (njt_conf_t *cf, char *path)
{
    njt_str_t         *str;
    int                n;
    char              *m, *s;
...
char *
ndk_conf_set_full_path_slot (njt_conf_t *cf, njt_command_t *cmd, void *conf)
{
    char  *p = conf;
    ...
    *path = value[1];
    ...
    if (njt_conf_full_name (cf->cycle, path, 0) == NJT_ERROR)
...
char *
ndk_conf_set_split_path_slot (njt_conf_t *cf, njt_command_t *cmd, void *conf)     
{
    ...
    *a = njt_array_create (cf->pool, n, sizeof (njt_str_t));
    if (*a == NULL) {
        return  NJT_CONF_ERROR;
    }
...
char *
ndk_conf_set_full_path (njt_conf_t *cf, void *data, njt_str_t *path)
{
    if (njt_conf_full_name (cf->cycle, path, 0) == NJT_ERROR)
        return  NJT_CONF_ERROR;
...
char *
ndk_conf_set_full_conf_path (njt_conf_t *cf, void *data, njt_str_t *path)
{
    if (njt_conf_full_name (cf->cycle, path, 1) == NJT_ERROR)
        return  NJT_CONF_ERROR;
