...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    pool = ngx_create_pool(NJT_CYCLE_POOL_SIZE, log);
    if (pool == NULL) {
        return NULL;
    }
    pool->log = log;
    ...
    if (gethostname(hostname, NJT_MAXHOSTNAMELEN) == -1) {
        ngx_log_error(NJT_LOG_EMERG, log, ngx_errno, "gethostname() failed");
        ngx_destroy_pool(pool);
        return NULL;
    }
    ...
    if (ngx_cycle_modules(cycle) != NJT_OK) {
        ngx_destroy_pool(pool);
        return NULL;
    }
    ...
    conf.temp_pool = ngx_create_pool(NJT_CYCLE_POOL_SIZE, log);
    ...
    if (ngx_conf_param(&conf) != NJT_CONF_OK) {
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return NULL;
    }

    if (ngx_conf_parse(&conf, &cycle->conf_file) != NJT_CONF_OK) {
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return NULL;
    }
    ...
    if (ngx_process == NJT_PROCESS_SIGNALLER) {
        return cycle;
    }
    ...
    if (ngx_test_config) {
        ...
        if (ngx_create_pidfile(&ccf->pid, log) != NJT_OK) {
            goto failed;
        }
    ...
    if (ngx_test_lockfile(cycle->lock_file.data, log) != NJT_OK) {
        goto failed;
    }


    if (ngx_create_paths(cycle, ccf->user) != NJT_OK) {
        goto failed;
    }


    if (ngx_log_open_default(cycle) != NJT_OK) {
        goto failed;
    }
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        ngx_log_debug3(NJT_LOG_DEBUG_CORE, log, 0,
                       "log: %p %d \"%s\"",
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (shm_zone[i].shm.size == 0) {
            ngx_log_error(NJT_LOG_EMERG, log, 0,
        ...
        if (ngx_shm_alloc(&shm_zone[i].shm) != NJT_OK) {
            goto failed;
        }

        if (ngx_init_zone_pool(cycle, &shm_zone[i]) != NJT_OK) {
            goto failed;
        }
    ...
    if (ngx_open_listening_sockets(cycle) != NJT_OK) {
        goto failed;
    }
    ...
    if (ngx_init_modules(cycle) != NJT_OK) {
        /* fatal */
        exit(1);
    }
...
old_shm_zone_done:
    ...
    for (i = 0; i < old_cycle->listening.nelts; i++) {
        ...
        if (ngx_close_socket(ls[i].fd) == -1) {
            ngx_log_error(NJT_LOG_EMERG, log, ngx_socket_errno,
        ...
        if (ls[i].sockaddr->sa_family == AF_UNIX) {
            ...
            if (ngx_delete_file(name) == NJT_FILE_ERROR) {
                ngx_log_error(NJT_LOG_EMERG, cycle->log, ngx_socket_errno,
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (file[i].fd == NJT_INVALID_FILE || file[i].fd == ngx_stderr) {
            continue;
        }

        if (ngx_close_file(file[i].fd) == NJT_FILE_ERROR) {
            ngx_log_error(NJT_LOG_EMERG, log, ngx_errno,
    ...
    if (ngx_process == NJT_PROCESS_MASTER || ngx_is_init_cycle(old_cycle)) {
...
ngx_init_zone_pool(ngx_cycle_t *cycle, ngx_shm_zone_t *zn)
{
    ...
    if (zn->shm.exists) {
        ...
        if (ngx_shm_remap(&zn->shm, sp->addr) != NJT_OK) {
            return NJT_ERROR;
        }
    ...
    if (ngx_shmtx_create(&sp->mutex, &sp->lock, file) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_create_pidfile(ngx_str_t *name, ngx_log_t *log)
{
    ...
    if (ngx_process > NJT_PROCESS_MASTER) {
        return NJT_OK;
    }
    ...
    create = ngx_test_config ? NJT_FILE_CREATE_OR_OPEN : NJT_FILE_TRUNCATE;

    file.fd = ngx_open_file(file.name.data, NJT_FILE_RDWR,
    ...
    if (!ngx_test_config) {
        len = ngx_snprintf(pid, NJT_INT64_LEN + 2, "%P%N", ngx_pid) - pid;

        if (ngx_write_file(&file, pid, len, 0) == NJT_ERROR) {
            rc = NJT_ERROR;
        }
    }
...
ngx_signal_process(ngx_cycle_t *cycle, char *sig)
{
    ...
    ngx_log_error(NJT_LOG_NOTICE, cycle->log, 0, "signal process started");
    ...
    if (file.fd == NJT_INVALID_FILE) {
        ngx_log_error(NJT_LOG_ERR, cycle->log, ngx_errno,
                      ngx_open_file_n " \"%s\" failed", file.name.data);
        return 1;
    }

    n = ngx_read_file(&file, buf, NJT_INT64_LEN + 2, 0);
    ...
    if (pid == (ngx_pid_t) NJT_ERROR) {
        ngx_log_error(NJT_LOG_ERR, cycle->log, 0,
...
ngx_test_lockfile(u_char *file, ngx_log_t *log)
{
    ...
    fd = ngx_open_file(file, NJT_FILE_RDWR, NJT_FILE_CREATE_OR_OPEN,
                       NJT_FILE_DEFAULT_ACCESS);
    ...
    if (ngx_delete_file(file) == NJT_FILE_ERROR) {
        ngx_log_error(NJT_LOG_ALERT, log, ngx_errno,
...
ngx_reopen_files(ngx_cycle_t *cycle, ngx_uid_t user)
{
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        fd = ngx_open_file(file[i].name.data, NJT_FILE_APPEND,
                           NJT_FILE_CREATE_OR_OPEN, NJT_FILE_DEFAULT_ACCESS);

        ngx_log_debug3(NJT_LOG_DEBUG_EVENT, cycle->log, 0,
...
#if !(NJT_WIN32)
        if (user != (ngx_uid_t) NJT_CONF_UNSET_UINT) {
            ngx_file_info_t  fi;

            if (ngx_file_info(file[i].name.data, &fi) == NJT_FILE_ERROR) {
                ngx_log_error(NJT_LOG_EMERG, cycle->log, ngx_errno,
...
ngx_clean_old_cycles(ngx_event_t *ev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_CORE, log, 0, "clean old cycles");
    ...
    for (i = 0; i < ngx_old_cycles.nelts; i++) {
        ...
        for (n = 0; n < cycle[i]->connection_n; n++) {
            if (cycle[i]->connections[n].fd != (ngx_socket_t) -1) {
                ...
                ngx_log_debug1(NJT_LOG_DEBUG_CORE, log, 0, "live fd:%ui", n);
        ...
        ngx_log_debug1(NJT_LOG_DEBUG_CORE, log, 0, "clean old cycle: %ui", i);

        ngx_destroy_pool(cycle[i]->pool);
        cycle[i] = NULL;
    }

    ngx_log_debug1(NJT_LOG_DEBUG_CORE, log, 0, "old cycles status: %ui", live);
