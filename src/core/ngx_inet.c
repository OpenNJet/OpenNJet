...
#if (NJT_HAVE_INET6)
    } else if (ngx_inet6_addr(addr, len, cidr->u.in6.addr.s6_addr) == NJT_OK) {
...
#if (NJT_HAVE_INET6)
    } else if (ngx_inet6_addr(text, len, inaddr6.s6_addr) == NJT_OK) {
        family = AF_INET6;
...
no_port:
    ...
    if (ngx_inet_resolve_host(pool, u) != NJT_OK) {
        return NJT_ERROR;
    }
...
ngx_parse_inet6_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    if (ngx_inet6_addr(host, len, sin6->sin6_addr.s6_addr) != NJT_OK) {
