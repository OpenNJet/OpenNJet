    ...
    ngx_uint_t persistent, ngx_uint_t clean, ngx_uint_t access)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug1(NJT_LOG_DEBUG_CORE, file->log, 0,
        ...
        if (ngx_create_path(file, path) == NJT_ERROR) {
            return NJT_ERROR;
        }
    }
}
...
ngx_create_path(ngx_file_t *file, ngx_path_t *path)
{
    ...
    for (i = 0; i < NJT_MAX_PATH_LEVEL; i++) {
        ...
        if (ngx_create_dir(file->name.data, 0700) == NJT_FILE_ERROR) {
            err = ngx_errno;
            if (err != NJT_EEXIST) {
                ngx_log_error(NJT_LOG_CRIT, file->log, err,
...
ngx_create_full_path(u_char *dir, ngx_uint_t access)
{
    ...
    for ( /* void */ ; *p; p++) {
        ...
        *p = '\0';
        ...
        if (ngx_create_dir(dir, access) == NJT_FILE_ERROR) {
            err = ngx_errno;
...
ngx_conf_set_path_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_conf_full_name(cf->cycle, &path->name, 0) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
    ...
    *slot = path;
    ...
    if (ngx_add_path(cf, slot) == NJT_ERROR) {
        return NJT_CONF_ERROR;
    }
    ...
    ngx_path_init_t *init)
{
    ...
    *path = ngx_pcalloc(cf->pool, sizeof(ngx_path_t));
    ...
    if (ngx_add_path(cf, path) != NJT_OK) {
        return NJT_CONF_ERROR;
    }
...
invalid:
    ...
    ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "invalid value \"%V\"", &value[i]);
...
ngx_create_paths(ngx_cycle_t *cycle, ngx_uid_t user)
{
    ...
    for (i = 0; i < cycle->paths.nelts; i++) {
        ...
        if (ngx_create_dir(path[i]->name.data, 0700) == NJT_FILE_ERROR) {
            err = ngx_errno;
            if (err != NJT_EEXIST) {
                ngx_log_error(NJT_LOG_EMERG, cycle->log, err,
                              ngx_create_dir_n " \"%s\" failed",
                              path[i]->name.data);
                return NJT_ERROR;
            }
        ...
        if (ngx_file_info(path[i]->name.data, &fi) == NJT_FILE_ERROR) {
...
ngx_ext_rename_file(ngx_str_t *src, ngx_str_t *to, ngx_ext_rename_file_t *ext)
{
    ...
    if (ext->access) {
        if (ngx_change_file_access(src->data, ext->access) == NJT_FILE_ERROR) {
            ngx_log_error(NJT_LOG_CRIT, ext->log, ngx_errno,
    ...
    if (ext->time != -1) {
        if (ngx_set_file_time(src->data, ext->fd, ext->time) != NJT_OK) {
            ngx_log_error(NJT_LOG_CRIT, ext->log, ngx_errno,
    ...
    if (ngx_rename_file(src->data, to->data) != NJT_FILE_ERROR) {
        return NJT_OK;
    }
    ...
    if (err == NJT_ENOPATH) {
        ...
        if (err) {
            ngx_log_error(NJT_LOG_CRIT, ext->log, err,
    ...
    if (err == NJT_EXDEV) {
        ...
        if (ngx_copy_file(src->data, name, &cf) == NJT_OK) {

            if (ngx_rename_file(name, to->data) != NJT_FILE_ERROR) {
                ngx_free(name);

                if (ngx_delete_file(src->data) == NJT_FILE_ERROR) {
                    ngx_log_error(NJT_LOG_CRIT, ext->log, ngx_errno,
...
ngx_copy_file(u_char *from, u_char *to, ngx_copy_file_t *cf)
{
    ...
    fd = ngx_open_file(from, NJT_FILE_RDONLY, NJT_FILE_OPEN, 0);

    if (fd == NJT_INVALID_FILE) {
        ngx_log_error(NJT_LOG_CRIT, cf->log, ngx_errno,
    ...
    nfd = ngx_open_file(to, NJT_FILE_WRONLY, NJT_FILE_TRUNCATE, access);
    ...
    while (size > 0) {
        ...
        if ((size_t) n != len) {
            ngx_log_error(NJT_LOG_ALERT, cf->log, 0,
    ...
    if (ngx_set_file_time(to, nfd, time) != NJT_OK) {
        ngx_log_error(NJT_LOG_ALERT, cf->log, ngx_errno,
...
failed:
    ...
    if (nfd != NJT_INVALID_FILE) {
        if (ngx_close_file(nfd) == NJT_FILE_ERROR) {
            ngx_log_error(NJT_LOG_ALERT, cf->log, ngx_errno,
...
ngx_walk_tree(ngx_tree_ctx_t *ctx, ngx_str_t *tree)
{
    ...
    ngx_log_debug1(NJT_LOG_DEBUG_CORE, ctx->log, 0,
                   "walk tree \"%V\"", tree);

    if (ngx_open_dir(tree, &dir) == NJT_ERROR) {
    ...
    for ( ;; ) {
        ...
        if (ngx_read_dir(&dir) == NJT_ERROR) {
            ...
            if (err == NJT_ENOMOREFILES) {
                ...
                ngx_log_error(NJT_LOG_CRIT, ctx->log, err,
        ...
        ngx_log_debug2(NJT_LOG_DEBUG_CORE, ctx->log, 0,
        ...
        *p++ = '/';
        ...
        if (!dir.valid_info) {
            if (ngx_de_info(file.data, &dir) == NJT_FILE_ERROR) {
                ngx_log_error(NJT_LOG_CRIT, ctx->log, ngx_errno,
        ...
        if (ngx_de_is_file(&dir)) {
            ...
            if (ngx_walk_tree(ctx, &file) == NJT_ABORT) {
                goto failed;
            }
