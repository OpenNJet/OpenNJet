...
#if (NJET_THREADS && NJET_HAVE_PWRITEV)
    ...
    ngx_uint_t persistent, ngx_uint_t clean, ngx_uint_t access)
{
    ...
    for ( ;; ) {
        ...
        ngx_log_debug1(NJET_LOG_DEBUG_CORE, file->log, 0,
        ...
        if (err == NJET_EEXIST_FILE) {
            n = (uint32_t) ngx_next_temp_number(1);
            continue;
        }

        if ((path->level[0] == 0) || (err != NJET_ENOPATH)) {
        ...
        if (ngx_create_path(file, path) == NJET_ERROR) {
            return NJET_ERROR;
        }
    }
}
...
ngx_create_path(ngx_file_t *file, ngx_path_t *path)
{
    ...
    for (i = 0; i < NJET_MAX_PATH_LEVEL; i++) {
        if (path->level[i] == 0) {
            break;
        }
        ...
        if (ngx_create_dir(file->name.data, 0700) == NJET_FILE_ERROR) {
            err = ngx_errno;
            if (err != NJET_EEXIST) {
                ngx_log_error(NJET_LOG_CRIT, file->log, err,
...
ngx_create_full_path(u_char *dir, ngx_uint_t access)
{
    ...
    for ( /* void */ ; *p; p++) {
        ...
        *p = '\0';
        ...
        if (ngx_create_dir(dir, access) == NJET_FILE_ERROR) {
            err = ngx_errno;
...
ngx_conf_set_path_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_conf_full_name(cf->cycle, &path->name, 0) != NJET_OK) {
        return NJET_CONF_ERROR;
    }
    ...
    *slot = path;
    ...
    if (ngx_add_path(cf, slot) == NJET_ERROR) {
        return NJET_CONF_ERROR;
    }
    ...
    ngx_path_init_t *init)
{
    ...
    *path = ngx_pcalloc(cf->pool, sizeof(ngx_path_t));
    ...
    if (ngx_add_path(cf, path) != NJET_OK) {
        return NJET_CONF_ERROR;
    }
...
ngx_conf_set_access_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (*access != NJET_CONF_UNSET_UINT) {
        return "is duplicate";
    }
...
invalid:
    ...
    ngx_conf_log_error(NJET_LOG_EMERG, cf, 0, "invalid value \"%V\"", &value[i]);
...
ngx_create_paths(ngx_cycle_t *cycle, ngx_uid_t user)
{
    ...
    for (i = 0; i < cycle->paths.nelts; i++) {
        ...
        if (ngx_create_dir(path[i]->name.data, 0700) == NJET_FILE_ERROR) {
            err = ngx_errno;
            if (err != NJET_EEXIST) {
                ngx_log_error(NJET_LOG_EMERG, cycle->log, err,
                              ngx_create_dir_n " \"%s\" failed",
                              path[i]->name.data);
                return NJET_ERROR;
            }
        ...
        if (ngx_file_info(path[i]->name.data, &fi) == NJET_FILE_ERROR) {
...
ngx_ext_rename_file(ngx_str_t *src, ngx_str_t *to, ngx_ext_rename_file_t *ext)
{
    ...
    if (ext->access) {
        if (ngx_change_file_access(src->data, ext->access) == NJET_FILE_ERROR) {
            ngx_log_error(NJET_LOG_CRIT, ext->log, ngx_errno,
    ...
    if (ext->time != -1) {
        if (ngx_set_file_time(src->data, ext->fd, ext->time) != NJET_OK) {
            ngx_log_error(NJET_LOG_CRIT, ext->log, ngx_errno,
    ...
    if (ngx_rename_file(src->data, to->data) != NJET_FILE_ERROR) {
        return NJET_OK;
    }
    ...
    if (err == NJET_ENOPATH) {
        ...
        if (err) {
            ngx_log_error(NJET_LOG_CRIT, ext->log, err,
    ...
    if (err == NJET_EEXIST || err == NJET_EEXIST_FILE) {
    ...
    if (err == NJET_EXDEV) {
        ...
        if (ngx_copy_file(src->data, name, &cf) == NJET_OK) {

            if (ngx_rename_file(name, to->data) != NJET_FILE_ERROR) {
                ngx_free(name);

                if (ngx_delete_file(src->data) == NJET_FILE_ERROR) {
                    ngx_log_error(NJET_LOG_CRIT, ext->log, ngx_errno,
...
ngx_copy_file(u_char *from, u_char *to, ngx_copy_file_t *cf)
{
    ...
    rc = NJET_ERROR;
    buf = NULL;
    nfd = NJET_INVALID_FILE;

    fd = ngx_open_file(from, NJET_FILE_RDONLY, NJET_FILE_OPEN, 0);

    if (fd == NJET_INVALID_FILE) {
        ngx_log_error(NJET_LOG_CRIT, cf->log, ngx_errno,
    ...
    nfd = ngx_open_file(to, NJET_FILE_WRONLY, NJET_FILE_TRUNCATE, access);

    if (nfd == NJET_INVALID_FILE) {
        ngx_log_error(NJET_LOG_CRIT, cf->log, ngx_errno,
    ...
    while (size > 0) {
        ...
        if ((size_t) n != len) {
            ngx_log_error(NJET_LOG_ALERT, cf->log, 0,
    ...
    if (ngx_set_file_time(to, nfd, time) != NJET_OK) {
        ngx_log_error(NJET_LOG_ALERT, cf->log, ngx_errno,
...
failed:
    ...
    if (nfd != NJET_INVALID_FILE) {
        if (ngx_close_file(nfd) == NJET_FILE_ERROR) {
            ngx_log_error(NJET_LOG_ALERT, cf->log, ngx_errno,
 ...
 * ctx->log - a log
 *
 * on fatal (memory) error handler must return NJET_ABORT to stop walking tree
 */
...
ngx_walk_tree(ngx_tree_ctx_t *ctx, ngx_str_t *tree)
{
    ...
    ngx_log_debug1(NJET_LOG_DEBUG_CORE, ctx->log, 0,
                   "walk tree \"%V\"", tree);

    if (ngx_open_dir(tree, &dir) == NJET_ERROR) {
    ...
    if (ctx->alloc) {
        ...
        if (ctx->init_handler(data, prev) == NJET_ABORT) {
            goto failed;
        }
    ...
    for ( ;; ) {
        ...
        if (ngx_read_dir(&dir) == NJET_ERROR) {
            ...
            if (err == NJET_ENOMOREFILES) {
                ...
                ngx_log_error(NJET_LOG_CRIT, ctx->log, err,
        ...
        ngx_log_debug2(NJET_LOG_DEBUG_CORE, ctx->log, 0,
        ...
        *p++ = '/';
        ...
        if (!dir.valid_info) {
            if (ngx_de_info(file.data, &dir) == NJET_FILE_ERROR) {
                ngx_log_error(NJET_LOG_CRIT, ctx->log, ngx_errno,
        ...
        if (ngx_de_is_file(&dir)) {
            ...
            if (ctx->file_handler(ctx, &file) == NJET_ABORT) {
                goto failed;
            }
            ...
            if (ngx_walk_tree(ctx, &file) == NJET_ABORT) {
                goto failed;
            }
            ...
            if (ctx->post_tree_handler(ctx, &file) == NJET_ABORT) {
                goto failed;
            }
            ...
            if (ctx->spec_handler(ctx, &file) == NJET_ABORT) {
                goto failed;
            }
        }
    }
