...
static const u_char *njt_md5_body(njt_md5_t *ctx, const u_char *data,
    size_t size);


void
njt_md5_init(njt_md5_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
njt_md5_update(njt_md5_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        if (size < free) {
            njt_memcpy(&ctx->buffer[used], data, size);
            return;
        }

        njt_memcpy(&ctx->buffer[used], data, free);
        data = (u_char *) data + free;
        size -= free;
        (void) njt_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = njt_md5_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }

    njt_memcpy(ctx->buffer, data, size);
}


void
njt_md5_final(u_char result[16], njt_md5_t *ctx)
{
    size_t  used, free;
    ...
    if (free < 8) {
        njt_memzero(&ctx->buffer[used], free);
        (void) njt_md5_body(ctx, ctx->buffer, 64);
        used = 0;
        free = 64;
    }

    njt_memzero(&ctx->buffer[used], free - 8);
...
static const u_char *
njt_md5_body(njt_md5_t *ctx, const u_char *data, size_t size)
{
    uint32_t       a, b, c, d;
