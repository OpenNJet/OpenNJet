...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {
        ...
        fd = ngx_open_file(filename->data, NJT_FILE_RDONLY, NJT_FILE_OPEN, 0);
        ...
        if (ngx_fd_info(fd, &cf->conf_file->file.info) == NJT_FILE_ERROR) {
            ngx_log_error(NJT_LOG_EMERG, cf->log, ngx_errno,
                          ngx_fd_info_n " \"%s\" failed", filename->data);
        }

        cf->conf_file->buffer = &buf;

        buf.start = ngx_alloc(NJT_CONF_BUFFER, cf->log);
        if (buf.start == NULL) {
            goto failed;
        }
        ...
        {
            if (ngx_conf_add_dump(cf, filename) != NJT_OK) {
                goto failed;
            }
    ...
    for ( ;; ) {
        ...
        if (rc == NJT_CONF_BLOCK_DONE) {
            ...
            if (type != parse_block) {
                ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "unexpected \"}\"");
                goto failed;
            }
        ...
        if (cf->handler) {
            ...
            ngx_conf_log_error(NJT_LOG_EMERG, cf, 0, "%s", rv);
...
ngx_conf_open_file(ngx_cycle_t *cycle, ngx_str_t *name)
{
    ...
    if (name->len) {
        ...
        if (ngx_conf_full_name(cycle, &full, 0) != NJT_OK) {
            return NULL;
        }
...
ngx_conf_flush_files(ngx_cycle_t *cycle)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_CORE, cycle->log, 0, "flush files");
