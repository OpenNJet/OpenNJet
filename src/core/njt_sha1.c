...
static const u_char *njt_sha1_body(njt_sha1_t *ctx, const u_char *data,
    size_t size);
...
void
njt_sha1_update(njt_sha1_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        size -= free;
        (void) njt_sha1_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = njt_sha1_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
void
njt_sha1_final(u_char result[20], njt_sha1_t *ctx)
{
    size_t  used, free;
...
static const u_char *
njt_sha1_body(njt_sha1_t *ctx, const u_char *data, size_t size)
{
