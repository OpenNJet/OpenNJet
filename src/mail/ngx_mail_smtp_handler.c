...
ngx_mail_smtp_init_session(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_resolve_addr(ctx) != NJT_OK) {
        ngx_mail_close_connection(c);
    }
}
...
ngx_mail_smtp_block_reading(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "smtp reading blocked");
...
ngx_mail_smtp_invalid_pipelining(ngx_event_t *rev)
{
    ...
    if (rev->timedout) {
        ...
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "delay greeting");
        ...
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "invalid pipelining");

        if (s->buffer == NULL) {
            if (ngx_mail_smtp_create_buffer(s, c) != NJT_OK) {
                return;
            }
        }
...
ngx_mail_smtp_create_buffer(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    if (ngx_array_init(&s->args, c->pool, 2, sizeof(ngx_str_t)) == NJT_ERROR) {
...
ngx_mail_smtp_auth_state(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "smtp auth state");
    ...
    if (s->out.len) {
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "smtp send handler busy");
        s->blocked = 1;
...
ngx_mail_smtp_auth(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    case NJT_MAIL_AUTH_CRAM_MD5:
        ...
        if (ngx_mail_auth_cram_md5_salt(s, c, "334 ", 4) == NJT_OK) {
    ...
    char *err)
{
    ...
    ngx_log_error(NJT_LOG_INFO, c->log, 0, err, &cmd);
}
