...
static ngx_command_t  ngx_mail_proxy_commands[] = {
    ...
    { ngx_string("proxy"),
      NJET_MAIL_MAIN_CONF|NJET_MAIL_SRV_CONF|NJET_CONF_FLAG,
...
ngx_mail_proxy_init(ngx_mail_session_t *s, ngx_addr_t *peer)
{
    ...
    p->upstream.log_error = NJET_ERROR_ERR;

    rc = ngx_event_connect_peer(&p->upstream);

    if (rc == NJET_ERROR || rc == NJET_BUSY || rc == NJET_DECLINED) {
...
ngx_mail_proxy_block_read(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy block read");
...
ngx_mail_proxy_pop3_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_MAIL, rev->log, 0,
    ...
    if (s->proxy->proxy_protocol) {
        ngx_log_debug0(NJET_LOG_DEBUG_MAIL, c->log, 0, "mail proxy pop3 busy");
    ...
    case ngx_pop3_start:
        ngx_log_debug0(NJET_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send user");
    ...
    case ngx_pop3_user:
        ngx_log_debug0(NJET_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send pass");
    ...
    case ngx_pop3_passwd:
        ...
        c->log->action = NULL;
        ngx_log_error(NJET_LOG_INFO, c->log, 0, "client logged in");
    ...
    if (c->send(c, line.data, line.len) < (ssize_t) line.len) {
        /*
         * we treat the incomplete sending as NJET_ERROR
...
ngx_mail_proxy_imap_handler(ngx_event_t *rev)
{
    ...
    if (s->proxy->proxy_protocol) {
        ngx_log_debug0(NJET_LOG_DEBUG_MAIL, c->log, 0, "mail proxy imap busy");
    ...
    case ngx_imap_login:
        ...
        line.len = s->login.len + 1 + 1 + NJET_SIZE_T_LEN + 1 + 2;
        line.data = ngx_pnalloc(c->pool, line.len);
        if (line.data == NULL) {
            ngx_mail_proxy_internal_server_error(s);
            return;
...
ngx_mail_proxy_smtp_handler(ngx_event_t *rev)
{
    ...
    if (s->proxy->proxy_protocol) {
        ngx_log_debug0(NJET_LOG_DEBUG_MAIL, c->log, 0, "mail proxy smtp busy");
    ...
    case ngx_smtp_start:
        ngx_log_debug0(NJET_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send ehlo");
        ...
        *p++ = CR; *p = LF;
        ...
        } else if (s->auth_method == NJET_MAIL_AUTH_NONE) {
    ...
    case ngx_smtp_xclient_helo:
        ...
        if (s->auth_method == NJET_MAIL_AUTH_NONE) {
            s->mail_state = ngx_smtp_helo_from;
...
ngx_mail_proxy_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_MAIL, wev->log, 0, "mail proxy write handler");

    c = wev->data;
    s = c->data;

    if (s->proxy->proxy_protocol) {
        if (ngx_mail_proxy_send_proxy_protocol(s) != NJET_OK) {
            return;
        }
...
ngx_mail_proxy_send_proxy_protocol(ngx_mail_session_t *s)
{
    ...
    ngx_log_debug0(NJET_LOG_DEBUG_MAIL, s->connection->log, 0,
                   "mail proxy send PROXY protocol header");

    p = ngx_proxy_protocol_write(s->connection, buf,
                                 buf + NJET_PROXY_PROTOCOL_MAX_HEADER);
    if (p == NULL) {
        ngx_mail_proxy_internal_server_error(s);
        return NJET_ERROR;
    }
...
ngx_mail_proxy_read_response(ngx_mail_session_t *s, ngx_uint_t state)
{
    ...
    ngx_log_error(NJET_LOG_INFO, s->connection->log, 0,
...
ngx_mail_proxy_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug3(NJET_LOG_DEBUG_MAIL, ev->log, 0,
            ...
            && s->proxy->upstream.connection->read->eof))
    {
        action = c->log->action;
        c->log->action = NULL;
        ngx_log_error(NJET_LOG_INFO, c->log, 0, "proxied session done");
        c->log->action = action;
    ...
    if (ngx_handle_read_event(dst->read, 0) != NJET_OK) {
        ngx_mail_proxy_close_session(s);
        return;
    }

    if (ngx_handle_write_event(src->write, 0) != NJET_OK) {
        ngx_mail_proxy_close_session(s);
        return;
    }

    if (ngx_handle_read_event(src->read, 0) != NJET_OK) {
        ngx_mail_proxy_close_session(s);
        return;
    }
...
ngx_mail_proxy_upstream_error(ngx_mail_session_t *s)
{
    if (s->proxy->upstream.connection) {
        ngx_log_debug1(NJET_LOG_DEBUG_MAIL, s->connection->log, 0,
...
ngx_mail_proxy_create_conf(ngx_conf_t *cf)
{
    ...
    pcf->enable = NJET_CONF_UNSET;
    pcf->pass_error_message = NJET_CONF_UNSET;
    pcf->xclient = NJET_CONF_UNSET;
    pcf->smtp_auth = NJET_CONF_UNSET;
    pcf->proxy_protocol = NJET_CONF_UNSET;
    pcf->buffer_size = NJET_CONF_UNSET_SIZE;
    pcf->timeout = NJET_CONF_UNSET_MSEC;
