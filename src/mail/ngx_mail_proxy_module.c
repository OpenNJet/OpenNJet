...
ngx_mail_proxy_block_read(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy block read");
...
ngx_mail_proxy_pop3_handler(ngx_event_t *rev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_MAIL, rev->log, 0,
    ...
    if (s->proxy->proxy_protocol) {
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "mail proxy pop3 busy");
    ...
    case ngx_pop3_start:
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send user");
    ...
    case ngx_pop3_user:
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send pass");
    ...
    case ngx_pop3_passwd:
        ...
        c->log->action = NULL;
        ngx_log_error(NJT_LOG_INFO, c->log, 0, "client logged in");
...
ngx_mail_proxy_imap_handler(ngx_event_t *rev)
{
    ...
    if (s->proxy->proxy_protocol) {
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "mail proxy imap busy");
...
ngx_mail_proxy_smtp_handler(ngx_event_t *rev)
{
    ...
    if (s->proxy->proxy_protocol) {
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, c->log, 0, "mail proxy smtp busy");
    ...
    case ngx_smtp_start:
        ngx_log_debug0(NJT_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send ehlo");
...
ngx_mail_proxy_write_handler(ngx_event_t *wev)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_MAIL, wev->log, 0, "mail proxy write handler");

    c = wev->data;
    s = c->data;

    if (s->proxy->proxy_protocol) {
        if (ngx_mail_proxy_send_proxy_protocol(s) != NJT_OK) {
            return;
        }
...
ngx_mail_proxy_send_proxy_protocol(ngx_mail_session_t *s)
{
    ...
    ngx_log_debug0(NJT_LOG_DEBUG_MAIL, s->connection->log, 0,
...
ngx_mail_proxy_read_response(ngx_mail_session_t *s, ngx_uint_t state)
{
    ...
    ngx_log_error(NJT_LOG_INFO, s->connection->log, 0,
...
ngx_mail_proxy_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug3(NJT_LOG_DEBUG_MAIL, ev->log, 0,
            ...
            && s->proxy->upstream.connection->read->eof))
    {
        action = c->log->action;
        c->log->action = NULL;
        ngx_log_error(NJT_LOG_INFO, c->log, 0, "proxied session done");
        c->log->action = action;
    ...
    if (ngx_handle_read_event(dst->read, 0) != NJT_OK) {
        ngx_mail_proxy_close_session(s);
        return;
    }

    if (ngx_handle_write_event(src->write, 0) != NJT_OK) {
        ngx_mail_proxy_close_session(s);
        return;
    }

    if (ngx_handle_read_event(src->read, 0) != NJT_OK) {
        ngx_mail_proxy_close_session(s);
        return;
    }
...
ngx_mail_proxy_upstream_error(ngx_mail_session_t *s)
{
    if (s->proxy->upstream.connection) {
        ngx_log_debug1(NJT_LOG_DEBUG_MAIL, s->connection->log, 0,
