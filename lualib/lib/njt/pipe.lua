...
ffi.cdef[[
typedef int                         njt_pid_t;
typedef uintptr_t                   njt_msec_t;
...
int njt_http_lua_ffi_pipe_proc_read(njt_http_request_t *r,
    njt_http_lua_ffi_pipe_proc_t *proc, int from_stderr, int reader_type,
    size_t length, u_char **buf, size_t *buf_size, u_char *errbuf,
    size_t *errbuf_size);

int njt_http_lua_ffi_pipe_get_read_result(njt_http_request_t *r,
    njt_http_lua_ffi_pipe_proc_t *proc, int from_stderr, u_char **buf,
    size_t *buf_size, u_char *errbuf, size_t *errbuf_size);

ssize_t njt_http_lua_ffi_pipe_proc_write(njt_http_request_t *r,
    njt_http_lua_ffi_pipe_proc_t *proc, const u_char *data, size_t len,
    u_char *errbuf, size_t *errbuf_size);

ssize_t njt_http_lua_ffi_pipe_get_write_result(njt_http_request_t *r,
    njt_http_lua_ffi_pipe_proc_t *proc, u_char *errbuf, size_t *errbuf_size);

int njt_http_lua_ffi_pipe_proc_shutdown_stdin(
    njt_http_lua_ffi_pipe_proc_t *proc, u_char *errbuf, size_t *errbuf_size);

int njt_http_lua_ffi_pipe_proc_shutdown_stdout(
    njt_http_lua_ffi_pipe_proc_t *proc, u_char *errbuf, size_t *errbuf_size);

int njt_http_lua_ffi_pipe_proc_shutdown_stderr(
    njt_http_lua_ffi_pipe_proc_t *proc, u_char *errbuf, size_t *errbuf_size);

int njt_http_lua_ffi_pipe_proc_wait(njt_http_request_t *r,
    njt_http_lua_ffi_pipe_proc_t *proc, char **reason, int *status,
    u_char *errbuf, size_t *errbuf_size);

int njt_http_lua_ffi_pipe_proc_kill(njt_http_lua_ffi_pipe_proc_t *proc,
    int signal, u_char *errbuf, size_t *errbuf_size);

void njt_http_lua_ffi_pipe_proc_destroy(njt_http_lua_ffi_pipe_proc_t *proc);
]]


if not pcall(function() return C.njt_http_lua_ffi_pipe_spawn end) then
...
do
    ...
    function proc_read(proc, stderr, reader_type, len)
        ...
        local rc = C.njt_http_lua_ffi_pipe_proc_read(r, proc, stderr,
                                                     reader_type, len, buf,
                                                     buf_size, errbuf,
                                                     errbuf_size)
        ...
        while true do
            ...
            if rc == FFI_OK then
                ...
                if p ~= value_buf then
                    ...
                    buf[0] = p
                    C.njt_http_lua_ffi_pipe_get_read_result(r, proc, stderr,
                                                            buf, buf_size,
                                                            errbuf, errbuf_size)
                    assert(p == buf[0])
                end
            ...
            errbuf_size[0] = ERR_BUF_SIZE
            rc = C.njt_http_lua_ffi_pipe_get_read_result(r, proc, stderr, buf,
                                                         buf_size, errbuf,
                                                         errbuf_size)
        end
    end
...
local function proc_write(proc, data)
    ...
    local errbuf_size = get_size_ptr()
    errbuf_size[0] = ERR_BUF_SIZE
    local rc = C.njt_http_lua_ffi_pipe_proc_write(r, proc, data, #data, errbuf,
                                                  errbuf_size)
    if rc == FFI_NO_REQ_CTX then
    ...
    while true do
        ...
        errbuf_size = get_size_ptr()
        errbuf_size[0] = ERR_BUF_SIZE
        rc = C.njt_http_lua_ffi_pipe_get_write_result(r, proc, errbuf,
                                                      errbuf_size)
    end
end
...
local function proc_shutdown(proc, direction)
    ...
    if direction == "stdin" then
        rc = C.njt_http_lua_ffi_pipe_proc_shutdown_stdin(proc, errbuf,
                                                         errbuf_size)

    elseif direction == "stdout" then
        rc = C.njt_http_lua_ffi_pipe_proc_shutdown_stdout(proc, errbuf,
                                                          errbuf_size)

    elseif direction == "stderr" then
        rc = C.njt_http_lua_ffi_pipe_proc_shutdown_stderr(proc, errbuf,
                                                          errbuf_size)
...
do
    ...
    function proc_wait(proc)
        ...
        local errbuf_size = get_size_ptr()
        errbuf_size[0] = ERR_BUF_SIZE
        local rc = C.njt_http_lua_ffi_pipe_proc_wait(r, proc, reason, status,
                                                     errbuf, errbuf_size)
        if rc == FFI_NO_REQ_CTX then
            error("no request ctx found", 2)
        end
...
local function proc_kill(proc, signal)
    ...
    local rc = C.njt_http_lua_ffi_pipe_proc_kill(proc, signal, errbuf,
                                                 errbuf_size)
    if rc == FFI_ERROR then
        return nil, ffi_str(errbuf, errbuf_size[0])
    end
...
local mt = {
    __gc = C.njt_http_lua_ffi_pipe_proc_destroy,
...
local Proc = ffi.metatype("njt_http_lua_ffi_pipe_proc_t", mt)
