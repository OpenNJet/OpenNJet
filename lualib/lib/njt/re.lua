...
if core_regex.no_pcre then
    error("no support for 'njt.re' module: OpenResty was " ..
...
local njt_lua_ffi_set_jit_stack_size
local njt_lua_ffi_exec_regex


if subsystem == 'http' then
    ffi.cdef[[
int njt_http_lua_ffi_set_jit_stack_size(int size, unsigned char *errstr,
    size_t *errstr_size);
    ]]

    njt_lua_ffi_exec_regex = C.njt_http_lua_ffi_exec_regex
    njt_lua_ffi_set_jit_stack_size = C.njt_http_lua_ffi_set_jit_stack_size

elseif subsystem == 'stream' then
    ffi.cdef[[
int njt_stream_lua_ffi_set_jit_stack_size(int size, unsigned char *errstr,
    size_t *errstr_size);
    ]]

    njt_lua_ffi_exec_regex = C.njt_stream_lua_ffi_exec_regex
    njt_lua_ffi_set_jit_stack_size = C.njt_stream_lua_ffi_set_jit_stack_size
end
...
function _M.opt(option, value)
    if option == "jit_stack_size" then
        ...
        local rc = njt_lua_ffi_set_jit_stack_size(value, errbuf, sizep)
