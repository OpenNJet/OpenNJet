...
ffi.cdef[[
int njt_http_lua_ffi_get_phase(njt_http_request_t *r, char **err)
]]
...
function njt.get_phase()
    ...
    local context = C.njt_http_lua_ffi_get_phase(r, errmsg)
    if context == FFI_ERROR then -- NJT_ERROR
        error(errmsg, 2)
    end
