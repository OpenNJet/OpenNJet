...
int njt_http_lua_ffi_ndk_lookup_directive(const unsigned char *var_data,
    size_t var_len, ndk_set_var_value_pt *func);
int njt_http_lua_ffi_ndk_set_var_get(njt_http_request_t *r,
...
local function ndk_set_var_get(self, var)
    ...
    if C.njt_http_lua_ffi_ndk_lookup_directive(var, #var, func_p) ~= FFI_OK then
    ...
    return function (arg)
        ...
        local rc = C.njt_http_lua_ffi_ndk_set_var_get(r, func, arg, #arg, value)
        if rc ~= FFI_OK then
