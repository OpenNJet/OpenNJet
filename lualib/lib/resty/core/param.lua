...
ffi.cdef[[
    ...
    void njt_http_lua_ffi_get_setby_param(njt_http_request_t *r, int idx,
        u_char **data, size_t *len);
    int njt_http_lua_ffi_get_body_filter_param_eof(njt_http_request_t *r);
    int njt_http_lua_ffi_get_body_filter_param_body(njt_http_request_t *r,
        u_char **data_p, size_t *len_p);
    int njt_http_lua_ffi_copy_body_filter_param_body(njt_http_request_t *r,
        u_char *data);
]]
...
local function get_setby_param(r, idx)
    C.njt_http_lua_ffi_get_setby_param(r, idx, data_p, len_p)
    if len_p[0] == 0 then
        return nil
    end
...
local function get_body_filter_param(r, idx)
    if idx == 1 then
        data_p[0] = nil
        local rc = C.njt_http_lua_ffi_get_body_filter_param_body(r, data_p,
                                                                 len_p)
        if rc == FFI_AGAIN then
            local buf = get_string_buf(len_p[0])
            assert(C.njt_http_lua_ffi_copy_body_filter_param_body(r, buf)
                   == FFI_OK)
    ...
    elseif idx == 2 then
        local rc = C.njt_http_lua_ffi_get_body_filter_param_eof(r)
        return rc == 1
