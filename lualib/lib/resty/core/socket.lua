...
njt_http_lua_ffi_socket_tcp_sslhandshake(njt_http_request_t *r,
    njt_http_lua_socket_tcp_upstream_t *u, void *sess,
...
njt_http_lua_ffi_socket_tcp_get_sslhandshake_result(njt_http_request_t *r,
    njt_http_lua_socket_tcp_upstream_t *u, void **sess, char **errmsg,
    int *openssl_error_code);

void
njt_http_lua_ffi_ssl_free_session(void *sess);
]]
...
local function getoption(cosocket, option)
    ...
    local rc = C.njt_http_lua_ffi_socket_tcp_getoption(tcp_socket,
                                                       option_index[option],
                                                       output_value_buf,
                                                       err,
                                                       errlen)
...
local function setoption(cosocket, option, value)
    ...
    local rc = C.njt_http_lua_ffi_socket_tcp_setoption(tcp_socket,
                                                       option_index[option],
                                                       value,
                                                       err,
                                                       errlen)
...
local server_name_str    = ffi_new("njt_str_t[1]")
...
local function sslhandshake(cosocket, reused_session, server_name, ssl_verify,
    ...
    if not cosocket or n > 0 then
        error("njt.socket sslhandshake: expecting 1 ~ 5 arguments " ..
    ...
    local rc = C.njt_http_lua_ffi_socket_tcp_sslhandshake(r, u,
                   session_ptr[0],
    ...
    while true do
        ...
        if rc == FFI_OK then
            ...
            rc = C.njt_http_lua_ffi_socket_tcp_get_sslhandshake_result(r, u,
            ...
            return ffi_gc(session_ptr[0], C.njt_http_lua_ffi_ssl_free_session)
        end
