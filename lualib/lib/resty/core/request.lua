...
local njt_lua_ffi_req_start_time


if subsystem == "stream" then
    ffi.cdef[[
    double njt_stream_lua_ffi_req_start_time(njt_stream_lua_request_t *r);
    ]]

    njt_lua_ffi_req_start_time = C.njt_stream_lua_ffi_req_start_time

elseif subsystem == "http" then
    ffi.cdef[[
    double njt_http_lua_ffi_req_start_time(njt_http_request_t *r);
    ]]

    njt_lua_ffi_req_start_time = C.njt_http_lua_ffi_req_start_time
end


function njt.req.start_time()
    local r = get_request()
    if not r then
        error("no request found")
    end

    return tonumber(njt_lua_ffi_req_start_time(r))
end
...
local ffi_str_type = ffi.typeof("njt_http_lua_ffi_str_t*")
local ffi_str_size = ffi.sizeof("njt_http_lua_ffi_str_t")
...
ffi.cdef[[
    ...
    int njt_http_lua_ffi_req_get_headers_count(njt_http_request_t *r,
        int max, int *truncated);

    int njt_http_lua_ffi_req_get_headers(njt_http_request_t *r,
        njt_http_lua_ffi_table_elt_t *out, int count, int raw);

    int njt_http_lua_ffi_req_get_uri_args_count(njt_http_request_t *r,
        int max, int *truncated);

    size_t njt_http_lua_ffi_req_get_querystring_len(njt_http_request_t *r);

    int njt_http_lua_ffi_req_get_uri_args(njt_http_request_t *r,
        unsigned char *buf, njt_http_lua_ffi_table_elt_t *out, int count);

    int njt_http_lua_ffi_req_get_method(njt_http_request_t *r);

    int njt_http_lua_ffi_req_get_method_name(njt_http_request_t *r,
        unsigned char **name, size_t *len);

    int njt_http_lua_ffi_req_set_method(njt_http_request_t *r, int method);

    int njt_http_lua_ffi_req_set_header(njt_http_request_t *r,
        const unsigned char *key, size_t key_len, const unsigned char *value,
        size_t value_len, njt_http_lua_ffi_str_t *mvals, size_t mvals_len,
        int override, char **errmsg);
]]


local table_elt_type = ffi.typeof("njt_http_lua_ffi_table_elt_t*")
local table_elt_size = ffi.sizeof("njt_http_lua_ffi_table_elt_t")
...
function njt.req.get_headers(max_headers, raw)
    local r = get_request()
    if not r then
        error("no request found")
    end
    ...
    local n = C.njt_http_lua_ffi_req_get_headers_count(r, max_headers,
                                                       truncated)
    ...
    local rc = C.njt_http_lua_ffi_req_get_headers(r, buf, n, raw)
    if rc == 0 then
...
do
    ...
    function njt.req.get_method()
        ...
        do
            local id = C.njt_http_lua_ffi_req_get_method(r)
        ...
        local rc = C.njt_http_lua_ffi_req_get_method_name(r, namep, sizep)
        if rc ~= 0 then
            return nil
        end
...
function njt.req.set_method(method)
    local r = get_request()
    if not r then
        error("no request found")
    end
    ...
    local rc = C.njt_http_lua_ffi_req_set_method(r, method)
    if rc == FFI_OK then
        return
    end
...
do
    local function set_req_header(name, value, override)
        ...
        if value == nil then
            ...
            rc = C.njt_http_lua_ffi_req_set_header(r, name, #name, nil, 0, nil,
                                                   0, 1, errmsg)
            ...
            rc = C.njt_http_lua_ffi_req_set_header(r, name, #name, sval,
    ...
    function njt.req.set_header(name, value)
        set_req_header(name, value, true) -- override
    end
end  -- do


function njt.req.clear_header(name)
    local r = get_request()
    if not r then
        error("no request found")
    end
    ...
    local rc = C.njt_http_lua_ffi_req_set_header(r, name, #name, nil, 0, nil, 0,
                                                 1, errmsg)
