...
ffi.cdef[[
    int njt_http_lua_ffi_set_resp_header(njt_http_request_t *r,
        const char *key_data, size_t key_len, int is_nil,
        const char *sval, size_t sval_len, njt_http_lua_ffi_str_t *mvals,
        size_t mvals_len, int override, char **errmsg);

    int njt_http_lua_ffi_get_resp_header(njt_http_request_t *r,
        const unsigned char *key, size_t key_len,
        unsigned char *key_buf, njt_http_lua_ffi_str_t *values,
        int max_nvalues, char **errmsg);
]]


local njt_lua_ffi_set_resp_header
...
if MACOS then
    ffi.cdef[[
        ...
        int njt_http_lua_ffi_set_resp_header_macos(
            njt_http_lua_set_resp_header_params_t *p);
    ]]

    local set_params = ffi.new("njt_http_lua_set_resp_header_params_t")

    njt_lua_ffi_set_resp_header = function(r, key, key_len, is_nil,
                                           sval, sval_len, mvals,
        ...
        return C.njt_http_lua_ffi_set_resp_header_macos(set_params)
    end
    ...
    njt_lua_ffi_set_resp_header = function(r, key, key_len, is_nil,
        ...
        return C.njt_http_lua_ffi_set_resp_header(r, key, key_len, is_nil,
                                                  sval, sval_len, mvals,
                                                  mvals_len, override, err)
    end
end
...
local function set_resp_header(tb, key, value, no_override)
    ...
    if value == nil then
        ...
        rc = njt_lua_ffi_set_resp_header(r, key, #key, true, nil, 0, nil,
                                         0, 1, errmsg)
    else
        ...
        rc = njt_lua_ffi_set_resp_header(r, key, #key, false, sval,
...
local function get_resp_header(tb, key)
    ...
    local n = C.njt_http_lua_ffi_get_resp_header(r, key, key_len, key_buf,
...
do
    ...
    njt.header = setmetatable(new_tab(0, 0), mt)
end
