...
if string.find(jit.version, " 2.0", 1, true) then
    njt.log(njt.ALERT, "use of lua-resty-lrucache with LuaJIT 2.0 is ",
...
-- the implementation is mostly borrowed from nginx's njt_queue_t data
-- structure.
...
function _M.get(self, key)
    ...
    if node.expire >= 0 and node.expire < njt_now() then
        -- print("expired: ", node.expire, " > ", njt_now())
