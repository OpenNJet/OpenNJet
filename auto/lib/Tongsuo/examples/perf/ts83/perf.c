...
int main(void)
{
    ...
    for (; i < ITR_NUM; i++) {
        ...
        sm3_drbg = RAND_DRBG_new(NID_sm3, 0, NULL);
        if (sm3_drbg == NULL) {
            goto err;
        }
        if (RAND_DRBG_instantiate(sm3_drbg, NULL, 0) != 1) {
            goto err;
        }
        if (RAND_DRBG_bytes(sm3_drbg, rnd_data, RND_DATA_SIZE) != 1) {
            goto err;
        }
        if (RAND_DRBG_uninstantiate(sm3_drbg) != 1) {
            goto err;
        }
        RAND_DRBG_free(sm3_drbg);
        sm3_drbg = NULL;
        ...
        if (!EVP_DigestUpdate(mdctx, rnd_data, inlen)) {
            goto err;
        }
        if (!EVP_DigestFinal_ex(mdctx, md, (unsigned int *)&mdlen)) {
            goto err;
        }
        ...
        if (!EVP_EncryptInit_ex(sm4_ctx, EVP_sm4_ecb(), NULL, key, iv)) {
            goto err;
        }
        /* SM4 ECB encrypt */
        start = get_time();
        ...
        if (!EVP_DecryptInit_ex(sm4_ctx, EVP_sm4_ecb(), NULL, key, iv)) {
            goto err;
        }
        /* SM4 ECB decrypt */
        start = get_time();
        ...
        if (!EVP_DecryptInit_ex(sm4_ctx, EVP_sm4_cbc(), NULL, key, iv)) {
            goto err;
        }
        /* SM4 CBC decrypt */
        start = get_time();
