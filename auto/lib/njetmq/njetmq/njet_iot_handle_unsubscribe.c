...
int iot_handle__unsubscribe(struct mosq_iot *context)
{
	...
	iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Received UNSUBSCRIBE from %s", context->id);
	...
	while (context->in_packet.pos < context->in_packet.remaining_length)
	{
		...
		iot_log__printf(NULL, MOSQ_LOG_UNSUBSCRIBE, "%s %s", context->id, sub);
		mosquitto__free(sub);
		if (rc)
		{
			mosquitto__free(reason_codes);
	...
	iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Sending UNSUBACK to %s", context->id);

	/* We don't use Reason String or User Property yet. */
	rc = iot_send__unsuback(context, mid, reason_code_count, reason_codes, NULL);
	mosquitto__free(reason_codes);
	return rc;
}
