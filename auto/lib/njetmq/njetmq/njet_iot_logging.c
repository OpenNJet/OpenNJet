...
int log__init(struct mosquitto__config *config)
{
	...
	if (log_destinations & MQTT3_LOG_FILE)
	{
		...
		else
		{
			...
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to open log file %s for writing.", config->log_file);
		}
	}
#ifdef WITH_DLT
	dlt_fifo_check();
...
int iot_log__printf(struct mosq_iot *mosq, unsigned int priority, const char *fmt, ...)
{
	va_list va;
	int rc;
...
void log__internal(const char *fmt, ...)
{
	...
	if (len >= 200)
	{
		iot_log__printf(NULL, MOSQ_LOG_INTERNAL, "Internal log buffer too short (%d)", len);
		return;
	}

#ifdef WIN32
	iot_log__printf(NULL, MOSQ_LOG_INTERNAL, "%s", buf);
#else
	iot_log__printf(NULL, MOSQ_LOG_INTERNAL, "%s%s%s", "\e[32m", buf, "\e[0m");
#endif
}
