...
static struct mosq_iot *persist__find_or_add_context(const char *client_id, uint16_t last_mid)
{
...
static int persist__client_msg_restore(struct P_client_msg *chunk)
{
	...
	if (!context)
	{
		iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Persistence file contains client message with no matching client. File may be corrupt.");
		return 0;
	}
...
static int persist__client_chunk_restore(FILE *db_fptr)
{
	...
	else if (rc < 0)
	{
		...
		iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Empty client entry found in persistence database, it may be corrupt.");
		return MOSQ_ERR_SUCCESS;
	}
...
static int persist__retain_chunk_restore(FILE *db_fptr)
{
	...
	if (load)
	{
		if (sub__topic_tokenise(load->store->topic, &local_topic, &split_topics, NULL))
			return 1;
...
int persist__restore(void)
{
	...
	{
		fclose(fptr);
		iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Persistence file is empty.");
		return 0;
	}
	else if (rlen != 15)
	{
	...
	if (!memcmp(header, magic, 15))
	{
		...
		if (db_version != MOSQ_DB_VERSION)
		{
			...
			{
				fclose(fptr);
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unsupported persistent database format version %d (need version %d).", db_version, MOSQ_DB_VERSION);
				return 1;
			}
		}
		...
		while (persist__chunk_header_read(fptr, &chunk, &length) == MOSQ_ERR_SUCCESS)
		{
			...
			case DB_CHUNK_CFG:
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Incompatible database configuration (dbid size is %d bytes, expected %lu)",
			...
			default:
				iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Unsupported chunk \"%d\" in persistent database file. Ignoring.", chunk);
	...
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to restore persistent database. Unrecognised file format.");
		rc = 1;
	}
...
error:
	err = strerror(errno);
	iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: %s.", err);
	if (fptr)
		fclose(fptr);
	return 1;
}
