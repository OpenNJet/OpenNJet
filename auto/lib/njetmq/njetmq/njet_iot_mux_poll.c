...
int mux_poll__add_out(struct mosq_iot *context)
{
	size_t i;
...
int mux_poll__remove_out(struct mosq_iot *context)
{
	if (context->events & POLLOUT)
	{
...
int mux_poll__add_in(struct mosq_iot *context)
{
	size_t i;
...
int mux_poll__delete(struct mosq_iot *context)
{
	size_t pollfd_index;
	...
	if (context->pollfd_index != -1)
	{
		...
		while (pollfd_index == pollfd_current_max && pollfd_index > 0 && pollfds[pollfd_index].fd == INVALID_SOCKET)
		{
...
#endif
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error in poll: %s.", strerror(errno));
		}
	}
	else
	{
...
int iot_mux_poll__cleanup(void)
{
