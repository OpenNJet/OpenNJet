...
static int persist__client_messages_save(FILE *db_fptr, struct mosq_iot *context, struct mosquitto_client_msg *queue)
{
	...
	while (cmsg)
	{
		if (!strncmp(cmsg->store->topic, "$SYS", 4) && cmsg->store->ref_count <= 1 && cmsg->store->dest_id_count == 0)
		{
...
int persist__backup(bool shutdown)
{
	...
	iot_log__printf(NULL, MOSQ_LOG_INFO, "Saving in-memory database to %s.", db.config->persistence_filepath);

	len = strlen(db.config->persistence_filepath) + 5;
	outfile = mosquitto__malloc(len + 1);
	if (!outfile)
	{
		iot_log__printf(NULL, MOSQ_LOG_INFO, "Error saving in-memory database, out of memory.");
		return MOSQ_ERR_NOMEM;
	}
	...
	if (rc != 0)
	{
		...
		if (errno != ENOENT)
		{
			iot_log__printf(NULL, MOSQ_LOG_INFO, "Error saving in-memory database, unable to remove %s.", outfile);
			goto error;
		}
	}
#endif
	...
	if (db_fptr == NULL)
	{
		iot_log__printf(NULL, MOSQ_LOG_INFO, "Error saving in-memory database, unable to open %s for writing.", outfile);
		goto error;
	}
