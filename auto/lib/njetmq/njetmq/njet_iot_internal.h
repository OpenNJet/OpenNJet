...
struct iot_session_expiry_list
{
	struct mosq_iot *context;
	struct iot_session_expiry_list *prev;
	struct iot_session_expiry_list *next;
};
...
struct mosq_iot
{
	...
	struct mosq_iot *for_free_next;
	struct iot_session_expiry_list *expiry_list_item;
	uint16_t remote_port;

	uint32_t events;
};
struct iot_will_delay_list
{
	struct mosq_iot *context;
	struct iot_will_delay_list *prev;
	struct iot_will_delay_list *next;
};

struct iot_mosquitto_evt_control
{
	void *future;
	struct mosq_iot *client;
	const char *topic;
	const void *payload;
...
struct iot_mosquitto_evt_message
{
	void *future;
	struct mosq_iot *client;
	char *topic;
...
struct iot_mosquitto_evt_disconnect
{
	void *future;
	struct mosq_iot *client;
	int reason;
...
struct iot_mosquitto_evt_psk_key
{
	void *future;
	struct mosq_iot *client;
	const char *hint;
...
struct iot_mosquitto_evt_extended_auth
{
	void *future;
	struct mosq_iot *client;
	const void *data_in;
...
struct iot_mosquitto_evt_acl_check
{
	void *future;
	struct mosq_iot *client;
	const char *topic;
...
struct iot_mosquitto_evt_basic_auth
{
	void *future;
	struct mosq_iot *client;
	char *username;
...
typedef int (*FUNC_auth_plugin_acl_check_v4)(void *, int, struct mosq_iot *, struct mosquitto_acl_msg *);
typedef int (*FUNC_auth_plugin_unpwd_check_v4)(void *, struct mosq_iot *, const char *, const char *);
typedef int (*FUNC_auth_plugin_psk_key_get_v4)(void *, struct mosq_iot *, const char *, const char *, char *, int);
typedef int (*FUNC_auth_plugin_auth_start_v4)(void *, struct mosq_iot *, const char *, bool, const void *, uint16_t, void **, uint16_t *);
typedef int (*FUNC_auth_plugin_auth_continue_v4)(void *, struct mosq_iot *, const char *, const void *, uint16_t, void **, uint16_t *);
...
typedef int (*FUNC_auth_plugin_acl_check_v3)(void *, int, const struct mosq_iot *, struct mosquitto_acl_msg *);
typedef int (*FUNC_auth_plugin_unpwd_check_v3)(void *, const struct mosq_iot *, const char *, const char *);
typedef int (*FUNC_auth_plugin_psk_key_get_v3)(void *, const struct mosq_iot *, const char *, const char *, char *, int);
...
struct mosquitto_db
{
	...
	struct mosq_iot *contexts_for_free;
#ifdef WITH_BRIDGE
...
int iot_send__unsuback(struct mosq_iot *context, uint16_t mid, int reason_code_count, uint8_t *reason_codes, const mosquitto_property *properties);
int iot_send__auth(struct mosq_iot *context, uint8_t reason_code, const void *auth_data, uint16_t auth_data_len);

/* ============================================================
 * Network functions
 * ============================================================ */
void net__iot_init(void);
void net__iot_cleanup(void);
struct mosq_iot *net__socket_accept(struct mosquitto__listener_sock *listensock);
...
int iot_handle__connack(struct mosq_iot *context);
int iot_handle__connect(struct mosq_iot *context);
int iot_handle__disconnect(struct mosq_iot *context);
int iot_handle__publish(struct mosq_iot *context);
int iot_handle__subscribe(struct mosq_iot *context);
int iot_handle__unsubscribe(struct mosq_iot *context);
int iot_handle__auth(struct mosq_iot *context);
...
int db__message_delete_outgoing(struct mosq_iot *context, uint16_t mid, enum mosquitto_msg_state expect_state, int qos);
int db__message_insert(struct mosq_iot *context, uint16_t mid, enum mosquitto_msg_direction dir, uint8_t qos, bool retain, struct mosquitto_msg_store *stored, mosquitto_property *properties, bool update);
int db__message_remove_incoming(struct mosq_iot *context, uint16_t mid);
int db__message_release_incoming(struct mosq_iot *context, uint16_t mid);
int db__message_update_outgoing(struct mosq_iot *context, uint16_t mid, enum mosquitto_msg_state state, int qos);
void db__message_dequeue_first(struct mosq_iot *context, struct mosquitto_msg_data *msg_data);
...
int db__message_reconnect_reset(struct mosq_iot *context);
bool db__ready_for_flight(struct mosquitto_msg_data *msgs, int qos);
bool db__ready_for_queue(struct mosq_iot *context, int qos, struct mosquitto_msg_data *msg_data);
void sys_tree__init(void);
void sys_tree__update(int interval, time_t start_time);
int db__message_write_inflight_out_all(struct mosq_iot *context);
int db__message_write_inflight_out_latest(struct mosq_iot *context);
int db__message_write_queued_out(struct mosq_iot *context);
int db__message_write_queued_in(struct mosq_iot *context);

/* ============================================================
 * Subscription functions
 * ============================================================ */
int sub__add(struct mosq_iot *context, const char *sub, uint8_t qos, uint32_t identifier, int options, struct mosquitto__subhier **root);
...
int sub__remove(struct mosq_iot *context, const char *sub, struct mosquitto__subhier *root, uint8_t *reason);
 ...
 * Context functions
 * ============================================================ */
struct mosq_iot *context__init(mosq_sock_t sock);
void context__cleanup(struct mosq_iot *context, bool force_free);
void context__disconnect(struct mosq_iot *context);
void context__add_to_disused(struct mosq_iot *context);
void context__free_disused(void);
void context__send_will(struct mosq_iot *context);
void context__remove_from_by_id(struct mosq_iot *context);

int connect__on_authorised(struct mosq_iot *context, void *auth_data_out, uint16_t auth_data_out_len);

/* ============================================================
 * Control functions
 * ============================================================ */
#ifdef WITH_CONTROL
int control__process(struct mosq_iot *context, struct mosquitto_msg_store *stored);
...
int iot_log__printf(struct mosq_iot *mosq, unsigned int level, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
...
void bridge__cleanup(struct mosq_iot *context);
int bridge__connect(struct mosq_iot *context);
int bridge__connect_step1(struct mosq_iot *context);
int bridge__connect_step2(struct mosq_iot *context);
int bridge__connect_step3(struct mosq_iot *context);
int bridge__on_connect(struct mosq_iot *context);
void bridge__packet_cleanup(struct mosq_iot *context);
...
int bridge__remap_topic_in(struct mosq_iot *context, char **topic);
#endif
...
int iot_mux__loop_prepare(void);
...
void plugin__handle_disconnect(struct mosq_iot *context, int reason);
int plugin__handle_message(struct mosq_iot *context, struct mosquitto_msg_store *stored);
void LIB_ERROR(void);
 ...
 * Property related functions
 * ============================================================ */
int property__process_connect(struct mosq_iot *context, mosquitto_property **props);
int property__process_will(struct mosq_iot *context, struct mosquitto_message_all *msg, mosquitto_property **props);
int property__process_disconnect(struct mosq_iot *context, mosquitto_property **props);
 ...
 * ============================================================ */
int acl__find_acls(struct mosq_iot *context);
...
int mosquitto_acl_check(struct mosq_iot *context, const char *topic, uint32_t payloadlen, void *payload, uint8_t qos, bool retain, int access);
int mosquitto_unpwd_check(struct mosq_iot *context);
int mosquitto_psk_key_get(struct mosq_iot *context, const char *hint, const char *identity, char *key, int max_key_len);
...
int mosquitto_psk_key_get_default(struct mosq_iot *context, const char *hint, const char *identity, char *key, int max_key_len);

int mosquitto_security_auth_start(struct mosq_iot *context, bool reauth, const void *data_in, uint16_t data_in_len, void **data_out, uint16_t *data_out_len);
int mosquitto_security_auth_continue(struct mosq_iot *context, const void *data_in, uint16_t data_len, void **data_out, uint16_t *data_out_len);
 ...
 * Session expiry
 * ============================================================ */
int session_expiry__add(struct mosq_iot *context);
void session_expiry__remove(struct mosq_iot *context);
 ...
 * Will delay
 * ============================================================ */
int iot_will_delay__add(struct mosq_iot *context);
void iot_will_delay__check(void);
void iot_will_delay__send_all(void);
void iot_will_delay__remove(struct mosq_iot *mosq);
...
const char *iot_mosquitto_client_username(const struct mosq_iot *context);
