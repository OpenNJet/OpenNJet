...
int config__parse_args(struct mosquitto__config *config, int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
		if (!strcmp(argv[i], "-c") || !strcmp(argv[i], "--config-file"))
		{
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: -c argument given, but no config file specified.");
				return MOSQ_ERR_INVAL;
			}
			i++;
		}
		...
		else if (!strcmp(argv[i], "-p") || !strcmp(argv[i], "--port"))
		{
			if (i < argc - 1)
			{
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid port specified (%d).", port_tmp);
					return MOSQ_ERR_INVAL;
				}
				else
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Only %d ports can be specified on the command line.", CMD_PORT_LIMIT);
						return MOSQ_ERR_INVAL;
					}
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: -p argument given, but no port specified.");
				return MOSQ_ERR_INVAL;
			}
			i++;
		}
...
int config__read(struct mosquitto__config *config, bool reload)
{
	...
	if (rc)
	{
		if (lineno > 0)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error found at %s:%d.", db.config_file, lineno);
		}
		return rc;
	}
	...
	for (i = 0; i < config->bridge_count; i++)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge.configuration: bridge name not defined.");
			return MOSQ_ERR_INVAL;
		}
		if (config->bridges[i].addresses == 0)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge.configuration: no remote addresses defined.");
			return MOSQ_ERR_INVAL;
		}
		if (config->bridges[i].topic_count == 0)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge.configuration: no topics defined.");
			return MOSQ_ERR_INVAL;
		}
#ifdef FINAL_WITH_TLS_PSK
		if (config->bridges[i].tls_psk && !config->bridges[i].tls_psk_identity)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge.configuration: missing bridge_identity.");
			return MOSQ_ERR_INVAL;
		}
		if (config->bridges[i].tls_psk_identity && !config->bridges[i].tls_psk)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge.configuration: missing bridge_psk.");
			return MOSQ_ERR_INVAL;
		}
#endif
	}
		...
		if ((*buf)[0] != '#' && (*buf)[0] != 10 && (*buf)[0] != 13)
		{
			...
			if (token)
			{
				...
				else if (!strcmp(token, "address") || !strcmp(token, "addresses"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge.configuration.");
						return MOSQ_ERR_INVAL;
					}
					...
					for (i = 0; i < cur_bridge->address_count; i++)
					{
						...
						if (tmp_char)
						{
							...
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid port value (%d).", tmp_int);
								return MOSQ_ERR_INVAL;
							}
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty address value in configuration.");
						return MOSQ_ERR_INVAL;
					}
#else
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "The 'allow_duplicate_messages' option is now deprecated and will be removed in a future version. The behaviour will default to true.");
				...
				else if (!strncmp(token, "auth_opt_", strlen("auth_opt_")) || !strncmp(token, "plugin_opt_", strlen("plugin_opt_")))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: An auth_opt_ option exists in the config file without an auth_plugin.");
						return MOSQ_ERR_INVAL;
					}
					...
					if (strlen(token) < prefix_len + 3)
					{
						...
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid auth_opt_ config option.");
						return MOSQ_ERR_INVAL;
					}
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty %s value in configuration.", key);
						mosquitto__free(key);
						return MOSQ_ERR_INVAL;
					}
				}
				...
				else if (!strcmp(token, "auth_plugin_deny_special_chars"))
				{
					...
					if (!cur_auth_plugin_config)
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: An auth_plugin_deny_special_chars option exists in the config file without an auth_plugin.");
						return MOSQ_ERR_INVAL;
					}
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "The 'bind_address' option is now deprecated and will be removed in a future version. The behaviour will default to true.");
				...
				else if (!strcmp(token, "bind_interface"))
				{
					...
					if (conf__parse_string(&token, "bind_interface", &cur_listener->bind_interface, saveptr))
						return MOSQ_ERR_INVAL;
#else
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: bind_interface specified but socket option not available.");
					return MOSQ_ERR_INVAL;
#endif
				}
				...
				else if (!strcmp(token, "bridge.cafile"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Cannot use both certificate and psk encryption in a single bridge.");
						return MOSQ_ERR_INVAL;
					}
#endif
				...
				else if (!strcmp(token, "bridge_identity"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Cannot use both certificate and identity encryption in a single bridge.");
						return MOSQ_ERR_INVAL;
					}
					if (conf__parse_string(&token, "bridge_identity", &cur_bridge->tls_psk_identity, saveptr))
						return MOSQ_ERR_INVAL;
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Bridge and/or TLS-PSK support not available.");
#endif
				}
				...
				else if (!strcmp(token, "bridge_require_ocsp"))
				{
					...
					if (conf__parse_bool(&token, "bridge_require_ocsp", &cur_bridge->tls_ocsp_required, saveptr))
						return MOSQ_ERR_INVAL;
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: TLS support not available.");
#endif
				}
				...
				else if (!strcmp(token, "bridge_protocol_version"))
				{
					...
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge_protocol_version value (%s).", token);
							return MOSQ_ERR_INVAL;
						}
					}
					else
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty bridge_protocol_version value in configuration.");
						return MOSQ_ERR_INVAL;
					}
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Bridge support not available.");
				...
				else if (!strcmp(token, "cafile"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Cannot use both certificate and psk encryption in a single listener.");
						return MOSQ_ERR_INVAL;
					}
				...
				else if (!strcmp(token, "ciphers_tls1.3"))
				{
					...
					if (conf__parse_string(&token, "ciphers_tls1.3", &cur_listener->ciphers_tls13, saveptr))
						return MOSQ_ERR_INVAL;
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: ciphers_tls1.3 support not available.");
#endif
				}
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "The 'clientid_prefixes' option is now deprecated and will be removed in a future version.");
					if (reload)
					{
						mosquitto__free(config->clientid_prefixes);
						config->clientid_prefixes = NULL;
				...
				else if (!strcmp(token, "connection"))
				{
					...
					if (token)
					{
						...
						for (i = 0; i < config->bridge_count; i++)
						{
							...
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Duplicate bridge name \"%s\".", token);
								return MOSQ_ERR_INVAL;
							}
						}
				...
				else if (!strcmp(token, "http_dir"))
				{
					...
					if (conf__parse_string(&token, "http_dir", &cur_listener->http_dir, saveptr))
						return MOSQ_ERR_INVAL;
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Websockets support not available.");
#endif
				}
				else if (!strcmp(token, "idle_timeout"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_NOTICE, "idle_timeout interval too low, using 1 second.");
				...
				else if (!strcmp(token, "include_dir"))
				{
					if (level == 0)
					{
						...
						if (!token)
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty include_dir value in configuration.");
							return 1;
						}
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_INFO, "Loading config file %s", files[i]);
							...
							if (rc)
							{
								if (lineno_ext > 0)
								{
									iot_log__printf(NULL, MOSQ_LOG_ERR, "Error found at %s:%d.", files[i], lineno_ext);
								}
								/* Free happens below */
								break;
							}
				...
				else if (!strcmp(token, "keepalive_interval"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Bridge keepalive value too high.");
						return MOSQ_ERR_INVAL;
					}
					if (tmp_int < 5)
					{
						iot_log__printf(NULL, MOSQ_LOG_NOTICE, "keepalive interval too low, using 5 seconds.");
						tmp_int = 5;
					}
				...
				else if (!strcmp(token, "listener"))
				{
					...
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: A listener with port 0 must provide a Unix socket path.");
							return MOSQ_ERR_INVAL;
						}
						...
						if (reload)
						{
							...
							if (!cur_listener)
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: It is not currently possible to add/remove listeners when reloading the config file.");
								return MOSQ_ERR_INVAL;
							}
						}
						else
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty listener value in configuration.");
						return MOSQ_ERR_INVAL;
					}
				}
				...
				else if (!strcmp(token, "log_dest"))
				{
					...
					if (token)
					{
						...
						else if (!strcmp(token, "file"))
						{
							...
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Duplicate \"log_dest file\" value.");
								return MOSQ_ERR_INVAL;
							}
							...
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty \"log_dest file\" value in configuration.");
								return MOSQ_ERR_INVAL;
							}
						}
						else
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid log_dest value (%s).", token);
							return MOSQ_ERR_INVAL;
						}
#if defined(WIN32) || defined(__CYGWIN__)
						if (service_handle)
						{
							...
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Cannot log to stdout/stderr when running as a Windows service.");
								return MOSQ_ERR_INVAL;
							}
						}
#endif
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty log_dest value in configuration.");
						return MOSQ_ERR_INVAL;
					}
				}
				...
				{
#if defined(WIN32) || defined(__CYGWIN__)
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: log_facility not supported on Windows.");
#else
					...
					case 7:
						...
						break;
					default:
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid log_facility value (%d).", tmp_int);
						return MOSQ_ERR_INVAL;
					}
#endif
				}
				...
				else if (!strcmp(token, "log_type"))
				{
					...
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid log_type value (%s).", token);
							return MOSQ_ERR_INVAL;
						}
					}
					else
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty log_type value in configuration.");
					}
				}
				else if (!strcmp(token, "max_connections"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty max_connections value in configuration.");
					}
				}
				else if (!strcmp(token, "maximum_qos") || !strcmp(token, "max_qos"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: max_qos must be between 0 and 2 inclusive.");
						return MOSQ_ERR_INVAL;
					}
				...
				else if (!strcmp(token, "max_inflight_messages"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: max_inflight_messages must be <= 65535.");
						return MOSQ_ERR_INVAL;
					}
				...
				else if (!strcmp(token, "max_keepalive"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid max_keepalive value (%d).", tmp_int);
						return MOSQ_ERR_INVAL;
					}
				...
				else if (!strcmp(token, "max_packet_size"))
				{
					...
					if (tmp_int < 20)
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid max_packet_size value (%d).", tmp_int);
						return MOSQ_ERR_INVAL;
					}
				...
				else if (!strcmp(token, "memory_limit"))
				{
					...
					if (lim < 0)
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid memory_limit value (%ld).", lim);
						return MOSQ_ERR_INVAL;
					}
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Note: It is recommended to replace `message_size_limit` with `max_packet_size`.");
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid message_size_limit value (%u).", config->message_size_limit);
						return MOSQ_ERR_INVAL;
					}
				}
				else if (!strcmp(token, "mount_point"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: You must use create a listener before using the mount_point option in the configuration file.");
						return MOSQ_ERR_INVAL;
					}
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR,
				...
				else if (!strcmp(token, "per_listener_settings"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: per_listener_settings must be set before any other security settings.");
						return MOSQ_ERR_INVAL;
					}
				}
				...
				else if (!strcmp(token, "persistent_client_expiration"))
				{
					...
					if (token)
					{
						...
						case 'y':
							...
							break;
						default:
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid persistent_client_expiration duration in configuration.");
							return MOSQ_ERR_INVAL;
						}
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty persistent_client_expiration value in configuration.");
					}
				}
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "The 'port' option is now deprecated and will be removed in a future version. Please use 'listener' instead.");
					config->local_only = false;
					if (reload)
						continue; /* Listeners not valid for reloading. */
					if (config->default_listener.port)
					{
						iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Default listener port specified multiple times. Only the latest will be used.");
					}
...
#else
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Websockets support not available.");
							return MOSQ_ERR_INVAL;
#endif
						}
						else
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid protocol value (%s).", token);
							return MOSQ_ERR_INVAL;
						}
					}
					else
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty protocol value in configuration.");
					}
				}
				else if (!strcmp(token, "psk_file"))
				{
					...
					if (conf__parse_string(&token, "psk_file", &cur_security_options->psk_file, saveptr))
						return MOSQ_ERR_INVAL;
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: TLS/TLS-PSK support not available.");
#endif
				}
				...
				else if (!strcmp(token, "restart_timeout"))
				{
					...
					if (!token)
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty restart_timeout value in configuration.");
						return MOSQ_ERR_INVAL;
					}
					cur_bridge->restart_timeout = atoi(token);
					if (cur_bridge->restart_timeout < 1)
					{
						iot_log__printf(NULL, MOSQ_LOG_NOTICE, "restart_timeout interval too low, using 1 second.");
					...
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: backoff cap is lower than the base in restart_timeout.");
							return MOSQ_ERR_INVAL;
						}
					}
#else
				...
				else if (!strcmp(token, "start_type"))
				{
					...
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Manual start_type not supported.");
							return MOSQ_ERR_INVAL;
						}
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid start_type value in configuration (%s).", token);
							return MOSQ_ERR_INVAL;
						}
					}
					else
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty start_type value in configuration.");
						return MOSQ_ERR_INVAL;
					}
#else
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Bridge support not available.");
				...
				else if (!strcmp(token, "socket_domain"))
				{
					...
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid socket_domain value \"%s\" in configuration.", token);
							return MOSQ_ERR_INVAL;
						}
					}
					else
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty socket_domain value in configuration.");
						return MOSQ_ERR_INVAL;
					}
				}
				else if (!strcmp(token, "sys_interval"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid sys_interval value (%d).", config->sys_interval);
						return MOSQ_ERR_INVAL;
					}
				}
				else if (!strcmp(token, "threshold"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_NOTICE, "threshold too low, using 1 message.");
				...
				else if (!strcmp(token, "topic"))
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty topic value in configuration.");
						return MOSQ_ERR_INVAL;
					}
					token = strtok_r(NULL, " ", &saveptr);
					if (token)
					{
						...
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge topic direction '%s'.", token);
							return MOSQ_ERR_INVAL;
						}
						token = strtok_r(NULL, " ", &saveptr);
						if (token)
						{
							...
							if (qos > 2)
							{
								iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge QoS level '%s'.", token);
								return MOSQ_ERR_INVAL;
							}
				...
				else if (!strcmp(token, "max_topic_alias"))
				{
					...
					if (token)
					{
						...
						if (tmp_int < 0 || tmp_int > UINT16_MAX)
						{
							iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid max_topic_alias value in configuration.");
							return MOSQ_ERR_INVAL;
						}
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty max_topic_alias value in configuration.");
						return MOSQ_ERR_INVAL;
					}
				}
				...
				else if (!strcmp(token, "websockets_headers_size"))
				{
					...
					if (tmp_int < 0 || tmp_int > UINT16_MAX)
					{
						iot_log__printf(NULL, MOSQ_LOG_WARNING, "Error: Websockets headers size must be between 0 and 65535 inclusive.");
						return MOSQ_ERR_INVAL;
					}
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unknown configuration variable \"%s\".", token);
					return MOSQ_ERR_INVAL;
				}
			}
		}
...
int config__read_file(struct mosquitto__config *config, bool reload, const char *file, struct config_recurse *cr, int level, int *lineno)
{
	...
	{
		closedir(dir);
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Config file %s is a directory.", file);
		return 1;
	}
#endif
	...
	if (!fptr)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to open config file %s.", file);
		return 1;
	}
...
static int config__check(struct mosquitto__config *config)
{
	...
	for (i = 0; i < config->bridge_count; i++)
	{
		...
		for (j = i + 1; j < config->bridge_count; j++)
		{
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Bridge local_clientid "
...
int conf__parse_bool(char **token, const char *name, bool *value, char *saveptr)
{
	*token = strtok_r(NULL, " ", &saveptr);
	if (*token)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid %s value (%s).", name, *token);
			return MOSQ_ERR_INVAL;
		}
	}
	else
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty %s value in configuration.", name);
		return MOSQ_ERR_INVAL;
	}
...
int conf__parse_string(char **token, const char *name, char **value, char *saveptr)
{
	...
	*token = strtok_r(NULL, "", &saveptr);
	if (*token)
	{
		if (*value)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Duplicate %s value in configuration.", name);
			return MOSQ_ERR_INVAL;
		}
