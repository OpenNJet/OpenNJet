...
static int retain__process(struct mosquitto__retainhier *branch, struct mosq_iot *context, uint8_t sub_qos, uint32_t subscription_identifier)
{
	int rc = 0;
	uint8_t qos;
	uint16_t mid;
...
static int retain__search(struct mosquitto__retainhier *retainhier, char **split_topics, struct mosq_iot *context, const char *sub, uint8_t sub_qos, uint32_t subscription_identifier, int level)
{
	...
	else
	{
		if (!strcmp(split_topics[0], "+"))
		{
			HASH_ITER(hh, retainhier->children, branch, branch_tmp)
			{
				...
				{
					if (retain__search(branch, &(split_topics[1]), context, sub, sub_qos, subscription_identifier, level + 1) == -1 || (split_topics[1] != NULL && !strcmp(split_topics[1], "#") && level > 0))
					{
