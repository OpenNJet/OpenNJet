...
// #include "will_mosq.h"
#include "njet_iot_will_mosq.h"
#include "njet_iot_send_mosq.h"
#include "njet_iot_net_mosq.h"

#ifdef WITH_BRIDGE

static void bridge__backoff_step(struct mosq_iot *context);
static void bridge__backoff_reset(struct mosq_iot *context);
...
{
	struct mosq_iot *new_context = NULL;
	...
	bridges = mosquitto__realloc(db.bridges, (size_t)(db.bridge_count + 1) * sizeof(struct mosq_iot *));
	if (bridges)
	{
		db.bridges = bridges;
		db.bridge_count++;
...
int bridge__connect_step1(struct mosq_iot *context)
{
	...
	iot_mqtt__set_state(context, mosq_cs_new);
	...
	for (i = 0; i < context->bridge->topic_count; i++)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Bridge %s doing local SUBSCRIBE on topic %s", context->id, context->bridge->topics[i].local_topic);
	...
	if (context->bridge->notifications)
	{
		...
		if (context->bridge->notification_topic)
		{
			...
			notification_payload = '0';
			rc = iot_will__set(context, context->bridge->notification_topic, 1, &notification_payload, qos, true, NULL);
		...
		else
		{
			...
			notification_payload = '0';
			rc = iot_will__set(context, notification_topic, 1, &notification_payload, qos, true, NULL);
	...
	iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Connecting bridge (step 1) %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);
	rc = iot_net__try_connect_step1(context, context->bridge->addresses[context->bridge->cur_address].address);
	if (rc > 0)
	{
		if (rc == MOSQ_ERR_TLS)
		{
			iot_net__socket_close(context);
			return rc; /* Error already printed */
		}
		else if (rc == MOSQ_ERR_ERRNO)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error creating bridge: %s.", strerror(errno));
		}
		else if (rc == MOSQ_ERR_EAI)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error creating bridge: %s.", gai_strerror(errno));
		}
...
int bridge__connect_step2(struct mosq_iot *context)
{
	...
	iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Connecting bridge (step 2) %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);
	rc = iot_net__try_connect_step2(context, context->bridge->addresses[context->bridge->cur_address].port, &context->sock);
	if (rc > 0)
	{
		if (rc == MOSQ_ERR_TLS)
		{
	...
	{
		iot_mqtt__set_state(context, mosq_cs_connect_pending);
	}
	return rc;
}
...
int bridge__connect_step3(struct mosq_iot *context)
{
	...
	rc = iot_net__socket_connect_step3(context, context->bridge->addresses[context->bridge->cur_address].address);
	if (rc > 0)
	{
		if (rc == MOSQ_ERR_TLS)
		{
	...
	rc = iot_send__connect(context, context->keepalive, context->clean_start, NULL);
	if (rc == MOSQ_ERR_SUCCESS)
	{
		return MOSQ_ERR_SUCCESS;
	}
...
int bridge__connect(struct mosq_iot *context)
{
	...
	iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Connecting bridge %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);
	rc = iot_net__socket_connect(context,
	...
	rc2 = iot_send__connect(context, context->keepalive, context->clean_start, NULL);
...
int bridge__on_connect(struct mosq_iot *context)
{
	...
	if (context->bridge->notifications)
	{
		...
		if (context->bridge->notification_topic)
		{
			...
			{
				if (iot_send__real_publish(context, iot_mqtt__mid_generate(context),
	...
	for (i = 0; i < context->bridge->topic_count; i++)
	{
		if (context->bridge->topics[i].direction == bd_in || context->bridge->topics[i].direction == bd_both)
		{
			...
			{
				sub_opts = sub_opts | MQTT_SUB_OPT_NO_LOCAL | MQTT_SUB_OPT_RETAIN_AS_PUBLISHED | MQTT_SUB_OPT_SEND_RETAIN_ALWAYS;
			}
			if (iot_send__subscribe(context, NULL, 1, &context->bridge->topics[i].remote_topic, sub_opts, NULL))
			{
				return 1;
			}
		}
		else
		{
			...
			{
				if (iot_send__unsubscribe(context, NULL, 1, &context->bridge->topics[i].remote_topic, NULL))
				{
...
{
	struct mosq_iot *context, *ctxt_tmp = NULL;

	HASH_ITER(hh_sock, db.contexts_by_sock, context, ctxt_tmp)
	{
		if (context->bridge)
		{
			if (iot_mux__add_in(context))
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error in initial bridge registration: %s", strerror(errno));
				return MOSQ_ERR_UNKNOWN;
			}
			iot_mux__add_out(context);
		}
	}
	return MOSQ_ERR_SUCCESS;
}
...
static void bridge_check_pending(struct mosq_iot *context)
{
	int err;
	socklen_t len;
...
void bridge_check(void)
{
	...
	for (i = 0; i < db.bridge_count; i++)
	{
		...
		{
			iot_mqtt__check_keepalive(context);
			...
			if (context->bridge->round_robin == false && context->bridge->cur_address != 0 && context->bridge->primary_retry && db.now_s > context->bridge->primary_retry)
			{
		...
		if (context->sock == INVALID_SOCKET)
		{
			...
			{
				if ((context->bridge->start_type == bst_lazy && context->bridge->lazy_reconnect) || (context->bridge->start_type == bst_automatic && db.now_s > context->bridge->restart_t))
				{
