...
static void iot_loop_handle_reads_writes(struct mosq_iot *context, uint32_t events);
...
int iot_mux_epoll__init(struct mosquitto__listener_sock *listensock, int listensock_count)
{
	...
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error in epoll creating: %s", strerror(errno));
	...
	for (i = 0; i < listensock_count; i++)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error in epoll initial registering: %s", strerror(errno));
...
int iot_mux_epoll__loop_setup(void)
{
	return MOSQ_ERR_SUCCESS;
}
...
int iot_mux_epoll__add_out(struct mosq_iot *context)
{
	...
	if (!(context->events & EPOLLOUT))
	{
		...
		if (epoll_ctl(db.epollfd, EPOLL_CTL_ADD, context->sock, &ev) == -1)
		{
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Error in epoll re-registering to EPOLLOUT: %s", strerror(errno));
			}
		}
...
int iot_mux_epoll__remove_out(struct mosq_iot *context)
{
	...
	if (context->events & EPOLLOUT)
	{
		...
		if (epoll_ctl(db.epollfd, EPOLL_CTL_ADD, context->sock, &ev) == -1)
		{
			if ((errno != EEXIST) || (epoll_ctl(db.epollfd, EPOLL_CTL_MOD, context->sock, &ev) == -1))
			{
				iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Error in epoll re-registering to EPOLLIN: %s", strerror(errno));
			}
		}
...
int iot_mux_epoll__add_in(struct mosq_iot *context)
{
	...
	if (epoll_ctl(db.epollfd, EPOLL_CTL_ADD, context->sock, &ev) == -1)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error in epoll accepting: %s", strerror(errno));
	}
	context->events = EPOLLIN;
	return MOSQ_ERR_SUCCESS;
}
...
int iot_mux_epoll__handle(void)
{
	...
	case -1:
		if (errno != EINTR)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error in epoll waiting: %s.", strerror(errno));
		}
		break;
	case 0:
		break;
	default:
		for (i = 0; i < event_count; i++)
		{
			...
			{
				iot_loop_handle_reads_writes(context, ep_events[i].events);
			}
...
static void iot_loop_handle_reads_writes(struct mosq_iot *context, uint32_t events)
{
	...
	if (events & EPOLLOUT
#ifdef WITH_TLS
		|| context->want_write || (context->ssl && context->state == mosq_cs_new)
#endif
	)
	{
