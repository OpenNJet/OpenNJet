...
void LIB_ERROR(void)
{
	...
	iot_log__printf(NULL, MOSQ_LOG_ERR, "Load error: %s", buf);
	LocalFree(buf);
#else
	iot_log__printf(NULL, MOSQ_LOG_ERR, "Load error: %s", dlerror());
#endif
}
...
static int security__module_init_single(struct mosquitto__listener *listener, struct mosquitto__security_options *opts)
{
	...
	for (i = 0; i < opts->auth_plugin_config_count; i++)
	{
		if (opts->auth_plugin_configs[i].path)
		{
			...
			iot_log__printf(NULL, MOSQ_LOG_INFO, "Loading plugin: %s", opts->auth_plugin_configs[i].path);
...
/* int mosquitto_acl_check(struct mosq_iot *context, const char *topic, int access) */
static int iot_acl__check_single(struct mosquitto__auth_plugin_config *auth_plugin, struct mosq_iot *context, struct mosquitto_acl_msg *msg, int access)
{
	const char *username;
	const char *topic = msg->topic;

	username = iot_mosquitto_client_username(context);
	if (auth_plugin->deny_special_chars == true)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_NOTICE, "ACL denying access to client with dangerous username \"%s\"", username);
			return MOSQ_ERR_ACL_DENIED;
		}
		if (context->id && strpbrk(context->id, "+#"))
		{
			iot_log__printf(NULL, MOSQ_LOG_NOTICE, "ACL denying access to client with dangerous client id \"%s\"", context->id);
			return MOSQ_ERR_ACL_DENIED;
		}
	}
...
int mosquitto_acl_check(struct mosq_iot *context, const char *topic, uint32_t payloadlen, void *payload, uint8_t qos, bool retain, int access)
{
	...
	struct mosquitto__callback *cb_base;
	struct iot_mosquitto_evt_acl_check event_data;
	...
	for (i = 0; i < opts->auth_plugin_config_count; i++)
	{
		if (opts->auth_plugin_configs[i].plugin.version < 5)
		{
			rc = iot_acl__check_single(&opts->auth_plugin_configs[i], context, &msg, access);
			if (rc != MOSQ_ERR_PLUGIN_DEFER)
			{
				return rc;
			}
...
int mosquitto_unpwd_check(struct mosq_iot *context)
{
	...
	int i;
	struct mosquitto__security_options *opts;
	struct iot_mosquitto_evt_basic_auth event_data;
	struct mosquitto__callback *cb_base;
	...
	for (i = 0; i < opts->auth_plugin_config_count; i++)
	{
		if (opts->auth_plugin_configs[i].plugin.version == 4 && opts->auth_plugin_configs[i].plugin.unpwd_check_v4)
		{
	...
	{
		if ((db.config->per_listener_settings && context->listener->security_options.allow_anonymous != false) || (!db.config->per_listener_settings && db.config->security_options.allow_anonymous != false))
		{
	...
	else
	{
		if (rc == MOSQ_ERR_PLUGIN_DEFER)
		{
			if (context->username == NULL &&
				((db.config->per_listener_settings && context->listener->security_options.allow_anonymous != false) || (!db.config->per_listener_settings && db.config->security_options.allow_anonymous != false)))
			{
...
int mosquitto_psk_key_get(struct mosq_iot *context, const char *hint, const char *identity, char *key, int max_key_len)
{
	...
	int i;
	struct mosquitto__security_options *opts;
	struct iot_mosquitto_evt_psk_key event_data;
	struct mosquitto__callback *cb_base;
	...
	for (i = 0; i < opts->auth_plugin_config_count; i++)
	{
		if (opts->auth_plugin_configs[i].plugin.version == 4 && opts->auth_plugin_configs[i].plugin.psk_key_get_v4)
		{
...
int mosquitto_security_auth_start(struct mosq_iot *context, bool reauth, const void *data_in, uint16_t data_in_len, void **data_out, uint16_t *data_out_len)
{
	...
	int i;
	struct mosquitto__security_options *opts;
	struct iot_mosquitto_evt_extended_auth event_data;
	struct mosquitto__callback *cb_base;
...
int mosquitto_security_auth_continue(struct mosq_iot *context, const void *data_in, uint16_t data_in_len, void **data_out, uint16_t *data_out_len)
{
	int rc = MOSQ_ERR_PLUGIN_DEFER;
	int i;
	struct mosquitto__security_options *opts;
