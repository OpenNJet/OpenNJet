...
void connection_check_acl(struct mosq_iot *context, struct mosquitto_client_msg **head)
{
...
int connect__on_authorised(struct mosq_iot *context, void *auth_data_out, uint16_t auth_data_out_len)
{
	...
	if (found_context)
	{
		...
		else
		{
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Client %s already connected, closing old connection.", context->id);
			}
		}
		...
		iot_will_delay__remove(found_context);
		iot_will__clear(found_context);

		found_context->clean_start = true;
		found_context->session_expiry_interval = 0;
		iot_mqtt__set_state(found_context, mosq_cs_duplicate);
	...
	if (db.config->connection_messages == true)
	{
		if (context->is_bridge)
		{
			if (context->username)
			{
				iot_log__printf(NULL, MOSQ_LOG_NOTICE, "New bridge.connected from %s:%d as %s (p%d, c%d, k%d, u'%s').",
								context->address, context->remote_port, context->id, context->protocol, context->clean_start, context->keepalive, context->username);
			}
			else
			{
				iot_log__printf(NULL, MOSQ_LOG_NOTICE, "New bridge.connected from %s:%d as %s (p%d, c%d, k%d).",
		...
		else
		{
			if (context->username)
			{
				iot_log__printf(NULL, MOSQ_LOG_NOTICE, "New client connected from %s:%d as %s (p%d, c%d, k%d, u'%s').",
								context->address, context->remote_port, context->id, context->protocol, context->clean_start, context->keepalive, context->username);
			}
			else
			{
				iot_log__printf(NULL, MOSQ_LOG_NOTICE, "New client connected from %s:%d as %s (p%d, c%d, k%d).",
								context->address, context->remote_port, context->id, context->protocol, context->clean_start, context->keepalive);
			}
		}
		...
		if (context->will)
		{
			iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Will message specified (%ld bytes) (r%d, q%d).",
							(long)context->will->msg.payloadlen,
							context->will->msg.retain,
							context->will->msg.qos);

			iot_log__printf(NULL, MOSQ_LOG_DEBUG, "\t%s", context->will->msg.topic);
		}
		else
		{
			iot_log__printf(NULL, MOSQ_LOG_DEBUG, "No will message specified.");
		}
	}
	...
	iot_mqtt__set_state(context, mosq_cs_active);
	rc = iot_send__connack(context, connect_ack, CONNACK_ACCEPTED, connack_props);
...
static int iot_will__read(struct mosq_iot *context, const char *client_id, struct mosquitto_message_all **will, uint8_t will_qos, int will_retain)
{
	...
	if (will_struct->msg.payloadlen > 0)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Client %s connected with too large Will payload", client_id);
			if (context->protocol == mosq_p_mqtt5)
			{
				iot_send__connack(context, 0, MQTT_RC_PACKET_TOO_LARGE, NULL);
			}
			else
			{
				iot_send__connack(context, 0, CONNACK_REFUSED_NOT_AUTHORIZED, NULL);
			}
...
int iot_handle__connect(struct mosq_iot *context)
{
	...
	{
		iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Bad client %s sending multiple CONNECT messages.", context->id);
		rc = MOSQ_ERR_PROTOCOL;
	...
	if (!strcmp(protocol_name, PROTOCOL_NAME_v31))
	{
		if ((protocol_version & 0x7F) != PROTOCOL_VERSION_v31)
		{
			if (db.config->connection_messages == true)
			{
				iot_log__printf(NULL, MOSQ_LOG_INFO, "Invalid protocol version %d in CONNECT from %s.",
								protocol_version, context->address);
			}
			iot_send__connack(context, 0, CONNACK_REFUSED_PROTOCOL_VERSION, NULL);
			rc = MOSQ_ERR_PROTOCOL;
			goto handle_connect_error;
		}
	...
	else
	{
		if (db.config->connection_messages == true)
		{
			iot_log__printf(NULL, MOSQ_LOG_INFO, "Invalid protocol \"%s\" in CONNECT from %s.",
	...
	if (will_qos == 3)
	{
		iot_log__printf(NULL, MOSQ_LOG_INFO, "Invalid Will QoS in CONNECT from %s.",
						context->address);
		rc = MOSQ_ERR_PROTOCOL;
		goto handle_connect_error;
	}
	...
	if (will && will_retain && db.config->retain_available == false)
	{
		...
		{
			iot_send__connack(context, 0, MQTT_RC_RETAIN_NOT_SUPPORTED, NULL);
		}
	...
	if (will && will_qos > context->listener->max_qos)
	{
		if (protocol_version == mosq_p_mqtt5)
		{
			iot_send__connack(context, 0, MQTT_RC_QOS_NOT_SUPPORTED, NULL);
		}
		rc = MOSQ_ERR_NOT_SUPPORTED;
		goto handle_connect_error;
	}
	...
	if (slen == 0)
	{
		...
		{
			iot_send__connack(context, 0, CONNACK_REFUSED_IDENTIFIER_REJECTED, NULL);
			rc = MOSQ_ERR_PROTOCOL;
			goto handle_connect_error;
		}
		else
		{ /* mqtt311/mqtt5 */
			...
			if ((context->protocol == mosq_p_mqtt311 && clean_start == 0) || allow_zero_length_clientid == false)
			{
				...
				{
					iot_send__connack(context, 0, MQTT_RC_UNSPECIFIED, NULL);
				}
				rc = MOSQ_ERR_PROTOCOL;
				goto handle_connect_error;
			}
	...
	if (db.config->clientid_prefixes)
	{
		if (strncmp(db.config->clientid_prefixes, client_id, strlen(db.config->clientid_prefixes)))
		{
			if (context->protocol == mosq_p_mqtt5)
			{
				iot_send__connack(context, 0, MQTT_RC_NOT_AUTHORIZED, NULL);
			}
			else
			{
				iot_send__connack(context, 0, CONNACK_REFUSED_NOT_AUTHORIZED, NULL);
	...
	if (will)
	{
		rc = iot_will__read(context, client_id, &will_struct, will_qos, will_retain);
		if (rc)
			goto handle_connect_error;
	}
	else
	...
	else
	{
		if (context->protocol == mosq_p_mqtt311 || context->protocol == mosq_p_mqtt31)
		{
			if (password_flag)
			{
				...
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Protocol error from %s: password without username, closing connection.", client_id);
				rc = MOSQ_ERR_PROTOCOL;
				goto handle_connect_error;
			}
		}
	...
	if (context->listener->ssl_ctx && (context->listener->use_identity_as_username || context->listener->use_subject_as_username))
	{
		...
		if (!context->ssl)
		{
			if (context->protocol == mosq_p_mqtt5)
			{
				iot_send__connack(context, 0, MQTT_RC_BAD_USERNAME_OR_PASSWORD, NULL);
			}
			else
			{
				iot_send__connack(context, 0, CONNACK_REFUSED_BAD_USERNAME_PASSWORD, NULL);
			}
			rc = MOSQ_ERR_AUTH;
			goto handle_connect_error;
		}
			...
			{ /* use_identity_as_username */
				...
				context->username = mosquitto__strdup((char *)ASN1_STRING_get0_data(name_asn1));
					...
					if (!context->username)
					{
						if (context->protocol == mosq_p_mqtt5)
						{
							iot_send__connack(context, 0, MQTT_RC_SERVER_UNAVAILABLE, NULL);
						}
						else
						{
							iot_send__connack(context, 0, CONNACK_REFUSED_SERVER_UNAVAILABLE, NULL);
						}
						rc = MOSQ_ERR_NOMEM;
						goto handle_connect_error;
					}
	...
	if (context->auth_method)
	{
		...
		{
			iot_mqtt__set_state(context, mosq_cs_authenticating);
			rc = iot_send__auth(context, MQTT_RC_CONTINUE_AUTHENTICATION, auth_data_out, auth_data_out_len);
			free(auth_data_out);
			return rc;
		}
		else
		{
			...
			else if (rc == MOSQ_ERR_NOT_SUPPORTED)
			{
				...
				iot_send__connack(context, 0, MQTT_RC_BAD_AUTHENTICATION_METHOD, NULL);
				mosquitto__free(context->id);
				context->id = NULL;
				goto handle_connect_error;
			}
