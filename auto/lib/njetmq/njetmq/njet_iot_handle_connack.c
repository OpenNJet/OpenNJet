	...
	else
	{
		if (context->protocol == mosq_p_mqtt5)
		{
			...
			case MQTT_RC_RETAIN_NOT_SUPPORTED:
				...
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: retain not available (will retry)");
			...
			case MQTT_RC_QOS_NOT_SUPPORTED:
				...
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: QoS not supported (will retry)");
				return MOSQ_ERR_CONN_LOST;
			default:
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: %s", mosquitto_reason_string(reason_code));
				return MOSQ_ERR_CONN_LOST;
			}
		}
		else
		{
			...
			case CONNACK_REFUSED_PROTOCOL_VERSION:
				...
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: unacceptable protocol version");
				return MOSQ_ERR_CONN_LOST;
			case CONNACK_REFUSED_IDENTIFIER_REJECTED:
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: identifier rejected");
				return MOSQ_ERR_CONN_LOST;
			case CONNACK_REFUSED_SERVER_UNAVAILABLE:
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: broker unavailable");
				return MOSQ_ERR_CONN_LOST;
			case CONNACK_REFUSED_BAD_USERNAME_PASSWORD:
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: broker unavailable");
				return MOSQ_ERR_CONN_LOST;
			case CONNACK_REFUSED_NOT_AUTHORIZED:
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: not authorised");
				return MOSQ_ERR_CONN_LOST;
			default:
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Connection Refused: unknown reason");
				return MOSQ_ERR_CONN_LOST;
			}
		}
	}
