...
struct mosq_iot *context__init(mosq_sock_t sock)
{
	...
	context = mosquitto__calloc(1, sizeof(struct mosq_iot));
	if (!context)
		return NULL;
...
void context__send_will(struct mosq_iot *ctxt)
{
	if (ctxt->state != mosq_cs_disconnecting && ctxt->will)
	{
		if (ctxt->will_delay_interval > 0)
		{
			iot_will_delay__add(ctxt);
			return;
		}
	...
	iot_will__clear(ctxt);
}

void context__disconnect(struct mosq_iot *context)
{
	if (iot_mqtt__get_state(context) == mosq_cs_disconnected)
	{
		return;
	}
	...
	iot_mqtt__set_state(context, mosq_cs_disconnected);
}
...
void context__add_to_disused(struct mosq_iot *context)
{
	...
	iot_mqtt__set_state(context, mosq_cs_disused);
...
{
	struct mosq_iot *context, *next;
#ifdef WITH_WEBSOCKETS
	struct mosq_iot *last = NULL;
#endif
