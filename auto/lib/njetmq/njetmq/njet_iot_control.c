...
int control__process(struct mosq_iot *context, struct mosquitto_msg_store *stored)
{
	...
	struct iot_mosquitto_evt_control event_data;
	...
	if (stored->qos == 1)
	{
		if (iot_send__puback(context, stored->source_mid, MQTT_RC_SUCCESS, properties))
			rc = 1;
	}
	else if (stored->qos == 2)
	{
		if (iot_send__pubrec(context, stored->source_mid, MQTT_RC_SUCCESS, properties))
			rc = 1;
	}
