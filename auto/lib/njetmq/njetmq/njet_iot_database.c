...
int db__message_insert(struct mosq_iot *context, uint16_t mid, enum mosquitto_msg_direction dir, uint8_t qos, bool retain, struct mosquitto_msg_store *stored, mosquitto_property *properties, bool update)
{
	 ...
	 */
	if (context->protocol != mosq_p_mqtt5 && db.config->allow_duplicate_messages == false && dir == mosq_md_out && retain == false && stored->dest_ids)
	{
...
#ifdef WITH_BRIDGE
	if (context->bridge && context->bridge->start_type == bst_lazy && context->sock == INVALID_SOCKET && context->msgs_out.msg_count >= context->bridge->threshold)
	{
...
int db__messages_delete(struct mosq_iot *context, bool force_free)
{
	...
	if (force_free || (context->bridge && context->bridge->clean_start_local) || (context->bridge == NULL && context->clean_start))
	{
...
int db__messages_easy_queue(struct mosq_iot *context, const char *topic, uint8_t qos, uint32_t payloadlen, const void *payload, int retain, uint32_t message_expiry_interval, mosquitto_property **properties)
{
	...
	if (db__message_store(context, stored, message_expiry_interval, 0, origin))
		return 1;
...
// int db__message_store(const struct mosq_iot *source, uint16_t source_mid, char *topic, int qos, uint32_t payloadlen, mosquitto__payload_uhpa *payload, int retain, struct mosquitto_msg_store **stored, uint32_t message_expiry_interval, mosquitto_property *properties, dbid_t store_id, enum mosquitto_msg_origin origin)
{
	assert(stored);
...
int db__message_reconnect_reset_outgoing(struct mosq_iot *context)
{
...
int db__message_reconnect_reset_incoming(struct mosq_iot *context)
{
	struct mosquitto_client_msg *msg, *tmp;
...
int db__message_release_incoming(struct mosq_iot *context, uint16_t mid)
{
	...
	DL_FOREACH_SAFE(context->msgs_in.queued, tail, tmp)
	{
		...
		if (tail->qos == 2)
		{
			iot_send__pubrec(context, tail->mid, 0, NULL);
...
int db__message_write_inflight_in(struct mosq_iot *context)
{
	struct mosquitto_client_msg *tail, *tmp;
	int rc;
	...
	DL_FOREACH_SAFE(context->msgs_in.inflight, tail, tmp)
	{
		if (tail->store->message_expiry_time)
		{
			if (db.now_real_s > tail->store->message_expiry_time)
			{
				...
				if (tail->qos > 0)
				{
					iot_util__increment_receive_quota(context);
				}
				continue;
			}
		}
		...
		case mosq_ms_send_pubrec:
			rc = iot_send__pubrec(context, tail->mid, 0, NULL);
			if (!rc)
			{
				tail->state = mosq_ms_wait_for_pubrel;
			}
		...
		case mosq_ms_resend_pubcomp:
			rc = iot_send__pubcomp(context, tail->mid, NULL);
			if (!rc)
			{
				tail->state = mosq_ms_wait_for_pubrel;
			}
...
static int db__message_write_inflight_out_single(struct mosq_iot *context, struct mosquitto_client_msg *msg)
{
	...
	if (msg->store->message_expiry_time)
	{
		if (db.now_real_s > msg->store->message_expiry_time)
		{
			...
			{
				iot_util__increment_send_quota(context);
			}
	...
	{
	case mosq_ms_publish_qos0:
		rc = iot_send__publish(context, mid, topic, payloadlen, payload, qos, retain, retries, cmsg_props, store_props, expiry_interval);
	...
	case mosq_ms_resend_pubrel:
		rc = iot_send__pubrel(context, mid, NULL);
		if (!rc)
		{
...
int db__message_write_inflight_out_latest(struct mosq_iot *context)
{
	...
	if (context->state != mosq_cs_active || context->sock == INVALID_SOCKET || context->msgs_out.inflight == NULL)
	{
	...
	while (tail != context->msgs_out.inflight &&
		   (tail->state == mosq_ms_publish_qos0 || tail->state == mosq_ms_publish_qos1 || tail->state == mosq_ms_publish_qos2))
	{
