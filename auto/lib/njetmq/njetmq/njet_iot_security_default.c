...
int mosquitto_security_init_default(bool reload)
{
	...
	if (db.config->per_listener_settings)
	{
		for (i = 0; i < db.config->listener_count; i++)
		{
			...
			if (pwf)
			{
				...
				if (rc)
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error opening password file \"%s\".", pwf);
					return rc;
				}
	...
	if (db.config->per_listener_settings)
	{
		for (i = 0; i < db.config->listener_count; i++)
		{
			if (db.config->listeners[i].security_options.acl_file)
			{
				...
				if (rc)
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error opening acl file \"%s\".", db.config->listeners[i].security_options.acl_file);
					return rc;
				}
				mosquitto_callback_register(db.config->listeners[i].security_options.pid,
	...
	else
	{
		if (db.config->security_options.acl_file)
		{
			...
			if (rc)
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error opening acl file \"%s\".", db.config->security_options.acl_file);
				return rc;
			}
			mosquitto_callback_register(db.config->security_options.pid,
										MOSQ_EVT_ACL_CHECK, mosquitto_acl_check_default, NULL, NULL);
	...
	if (db.config->per_listener_settings)
	{
		for (i = 0; i < db.config->listener_count; i++)
		{
			...
			if (pskf)
			{
				...
				if (rc)
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error opening psk file \"%s\".", pskf);
					return rc;
				}
			}
		}
...
static int mosquitto_acl_check_default(int event, void *event_data, void *userdata)
{
	...
	if (!ed->client->acl_list && !security_opts->acl_patterns)
		return MOSQ_ERR_ACL_DENIED;
	...
	if (acl_root)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_NOTICE, "ACL denying access to client with dangerous username \"%s\"", ed->client->username);
			return MOSQ_ERR_ACL_DENIED;
		}

		if (ed->client->id && strpbrk(ed->client->id, "+#"))
		{
			iot_log__printf(NULL, MOSQ_LOG_NOTICE, "ACL denying access to client with dangerous client id \"%s\"", ed->client->id);
			return MOSQ_ERR_ACL_DENIED;
		}
	}
...
static int aclfile__parse(struct mosquitto__security_options *security_opts)
{
	...
	{
		mosquitto__free(buf);
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to open acl_file \"%s\".", security_opts->acl_file);
	...
	while (fgets_extending(&buf, &buflen, aclfptr))
	{
		...
		if (token)
		{
			if (!strcmp(token, "topic") || !strcmp(token, "pattern"))
			{
				...
				if (!access_s)
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty topic in acl_file \"%s\".", security_opts->acl_file);
					rc = MOSQ_ERR_INVAL;
					break;
				}
				...
				if (access_s)
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid topic access type \"%s\" in acl_file \"%s\".", access_s, security_opts->acl_file);
						rc = MOSQ_ERR_INVAL;
						break;
					}
				}
				...
				if (rc != MOSQ_ERR_SUCCESS)
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid ACL topic \"%s\" in acl_file \"%s\".", topic, security_opts->acl_file);
					rc = MOSQ_ERR_INVAL;
					break;
				}
			...
			else if (!strcmp(token, "user"))
			{
				...
				if (token)
				{
					...
					if (slen == 0)
					{
						iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Missing username in acl_file \"%s\".", security_opts->acl_file);
						rc = MOSQ_ERR_INVAL;
						break;
					}
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid line in acl_file \"%s\": %s.", security_opts->acl_file, buf);
				rc = MOSQ_ERR_INVAL;
				break;
			}
		}
...
static int pwfile__parse(const char *file, struct mosquitto__unpwd **root)
{
	...
	if (!pwfile)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to open pwfile \"%s\".", file);
		mosquitto__free(buf);
		return MOSQ_ERR_UNKNOWN;
	}
	...
	while (!feof(pwfile))
	{
		if (fgets_extending(&buf, &buflen, pwfile))
		{
			...
			if (username)
			{
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Warning: Invalid line in password file '%s', username too long.", file);
				...
				if (password)
				{
					...
					{
						iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Warning: Invalid line in password file '%s', password too long.", file);
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Warning: Invalid line in password file '%s': %s", file, buf);
					mosquitto__free(unpwd->username);
					mosquitto__free(unpwd);
				}
			}
...
static int unpwd__decode_passwords(struct mosquitto__unpwd **unpwd)
{
	...
	HASH_ITER(hh, *unpwd, u, tmp)
	{
		...
		if (u->password == NULL)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Missing password hash for user %s, removing entry.", u->username);
		...
		if (token == NULL)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid password hash for user %s, removing entry.", u->username);
			unpwd__free_item(unpwd, u);
			continue;
		}
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid password hash type for user %s, removing entry.", u->username);
			unpwd__free_item(unpwd, u);
			continue;
		}
		...
		if (hashtype == pw_sha512_pbkdf2)
		{
			...
			if (u->iterations < 1)
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid hash iterations for user %s, removing entry.", u->username);
				unpwd__free_item(unpwd, u);
				continue;
			}
		}
		...
		if (rc == MOSQ_ERR_SUCCESS && salt_len == 12)
		{
			...
			if (token)
			{
				...
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to decode password for user %s, removing entry.", u->username);
					unpwd__free_item(unpwd, u);
				}
			}
			else
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to decode password salt for user %s, removing entry.", u->username);
			unpwd__free_item(unpwd, u);
		}
	}
...
static int psk__file_parse(struct mosquitto__unpwd **psk_id, const char *psk_file)
{
	...
	HASH_ITER(hh, (*psk_id), u, tmp)
	{
		...
		if (!u->password)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty psk for identity \"%s\".", u->username);
			return MOSQ_ERR_INVAL;
		}
		if (strspn(u->password, "0123456789abcdefABCDEF") < strlen(u->password))
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: psk for identity \"%s\" contains non-hexadecimal characters.", u->username);
			return MOSQ_ERR_INVAL;
		}
	}
	return MOSQ_ERR_SUCCESS;
...
#ifdef WITH_TLS
static void security__disconnect_auth(struct mosq_iot *context)
{
