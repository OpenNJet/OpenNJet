...
int iot_handle__auth(struct mosq_iot *context)
{
	...
	if (context->in_packet.remaining_length > 0)
	{
		...
		if ((reason_code == MQTT_RC_REAUTHENTICATE && context->state != mosq_cs_active) || (reason_code == MQTT_RC_CONTINUE_AUTHENTICATION && context->state != mosq_cs_authenticating && context->state != mosq_cs_reauthenticating))
		{
	...
	iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Received AUTH from %s (rc%d, %s)", context->id, reason_code, context->auth_method);

	if (reason_code == MQTT_RC_REAUTHENTICATE)
	{
		/* This is a re-authentication attempt */
		iot_mqtt__set_state(context, mosq_cs_reauthenticating);
	...
	if (rc == MOSQ_ERR_SUCCESS)
	{
		...
		else
		{
			...
			rc = iot_send__auth(context, MQTT_RC_SUCCESS, auth_data_out, auth_data_out_len);
			free(auth_data_out);
			return rc;
		}
	}
