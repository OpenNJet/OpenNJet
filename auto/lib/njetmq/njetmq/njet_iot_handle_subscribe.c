...
int iot_handle__subscribe(struct mosq_iot *context)
{
	...
	iot_log__printf(NULL, MOSQ_LOG_DEBUG, "Received SUBSCRIBE from %s", context->id);
	...
	while (context->in_packet.pos < context->in_packet.remaining_length)
	{
		...
		if (sub)
		{
			...
			iot_log__printf(NULL, MOSQ_LOG_DEBUG, "\t%s (QoS %d)", sub, qos);
			...
			if (allowed)
			{
				...
				if (context->protocol == mosq_p_mqtt311 || context->protocol == mosq_p_mqtt31)
				{
					...
					{
						if (retain__queue(context, sub, qos, 0))
							rc = 1;
					}
				}
				else
				{
					if ((retain_handling == MQTT_SUB_OPT_SEND_RETAIN_ALWAYS) || (rc2 == MOSQ_ERR_SUCCESS && retain_handling == MQTT_SUB_OPT_SEND_RETAIN_NEW))
					{

						if (retain__queue(context, sub, qos, subscription_identifier))
							rc = 1;
					}
				}

				iot_log__printf(NULL, MOSQ_LOG_SUBSCRIBE, "%s %d %s", context->id, qos, sub);
			}
			mosquitto__free(sub);
	...
	if (iot_send__suback(context, mid, payloadlen, payload))
		rc = 1;
	mosquitto__free(payload);
