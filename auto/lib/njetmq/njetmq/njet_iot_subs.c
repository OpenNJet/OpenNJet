...
static int subs__send(struct mosquitto__subleaf *leaf, const char *topic, uint8_t qos, int retain, struct mosquitto_msg_store *stored)
{
	...
	else if (rc2 == MOSQ_ERR_SUCCESS)
	{
		...
		if (msg_qos)
		{
			mid = iot_mqtt__mid_generate(leaf->context);
		}
		else
		{
			mid = 0;
...
static int sub__add_leaf(struct mosq_iot *context, uint8_t qos, uint32_t identifier, int options, struct mosquitto__subleaf **head, struct mosquitto__subleaf **newleaf)
{
	struct mosquitto__subleaf *leaf;
...
static int sub__remove_normal(struct mosq_iot *context, struct mosquitto__subhier *subhier, uint8_t *reason)
{
	struct mosquitto__subleaf *leaf;
	int i;
...
static int sub__remove_shared(struct mosq_iot *context, struct mosquitto__subhier *subhier, uint8_t *reason, const char *sharename)
{
...
static int sub__remove_recurse(struct mosq_iot *context, struct mosquitto__subhier *subhier, char **topics, uint8_t *reason, const char *sharename)
{
	struct mosquitto__subhier *branch;
