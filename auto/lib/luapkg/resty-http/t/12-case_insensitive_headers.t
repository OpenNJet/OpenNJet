...
location = /a {
    content_by_lua_block {
        ...
        assert(httpc:connect("127.0.0.1", njt.var.server_port),
        ...
        njt.say(res.headers["X-Foo-Header"])
        njt.say(res.headers["x-fOo-heaDeR"])
...
location = /b {
    content_by_lua_block {
        njt.say(njt.req.get_headers()["User-Agent"])
        njt.say(njt.req.get_headers(nil, true)["X_Foo"])
    }
}
--- request
GET /a
