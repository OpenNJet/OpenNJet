...
local njt_req = njt.req
local njt_req_socket = njt_req.socket
local njt_req_get_headers = njt_req.get_headers
local njt_req_get_method = njt_req.get_method
...
local njt_encode_args = njt.encode_args
local njt_re_match = njt.re.match
local njt_re_gmatch = njt.re.gmatch
local njt_re_sub = njt.re.sub
local njt_re_gsub = njt.re.gsub
local njt_re_find = njt.re.find
local njt_log = njt.log
local njt_DEBUG = njt.DEBUG
local njt_ERR = njt.ERR
local njt_var = njt.var
local njt_print = njt.print
local njt_header = njt.header
...
_M._USER_AGENT = "lua-resty-http/" .. _M._VERSION .. " (Lua) njt_lua/" .. njt.config.njt_lua_version
...
function _M.tcp_only_connect(self, ...)
    njt_log(njt_DEBUG, "Use of deprecated `connect` method signature")
...
local function _receive_headers(sock)
    ...
    repeat
        ...
        local m, err = njt_re_match(line, "([^:\\s]+):\\s*(.*)", "jo")
        if err then njt_log(njt_ERR, err) end
        ...
        if not content_length and max_chunk_size then
            ...
            repeat
                ...
                if not max_chunk_size then
                    njt_log(njt_ERR, "Buffer size not specified, bailing")
                    break
                end
            until not str
...
function _M.send_request(self, params)
    ...
    if DEBUG then njt_log(njt_DEBUG, "\n", req) end
...
function _M.get_client_body_reader(_, chunksize, sock)
    ...
    if not sock then
        local ok, err
        ok, sock, err = pcall(njt_req_socket)
    ...
    local headers = njt_req_get_headers()
    local length = headers.content_length
    if length then
        return _body_reader(sock, tonumber(length), chunksize)
    elseif transfer_encoding_is_chunked(headers) then
        -- Not yet supported by njt_lua but should just work...
...
function _M.get_proxy_uri(self, scheme, host)
    ...
    if self.proxy_opts.no_proxy then
        ...
        for host_suffix in njt_re_gmatch(self.proxy_opts.no_proxy, "\\.?([^,]+)", "jo") do
        ...
        repeat
            ...
            -- is on the list
            host = njt_re_sub(host, "^[^.]+\\.", "", "jo")
        until not njt_re_find(host, "\\.", "jo")
    end
...
function _M.ssl_handshake(self, ...)
    njt_log(njt_DEBUG, "Use of deprecated function `ssl_handshake`")
...
function _M.connect_proxy(self, proxy_uri, scheme, host, port, proxy_authorization)
    njt_log(njt_DEBUG, "Use of deprecated function `connect_proxy`")
...
function _M.proxy_request(self, chunksize)
    njt_log(njt_DEBUG, "Use of deprecated function `proxy_request`")

    return self:request({
        method = njt_req_get_method(),
        path = njt_re_gsub(njt_var.uri, "\\s", "%20", "jo") .. njt_var.is_args .. (njt_var.query_string or ""),
        body = self:get_client_body_reader(chunksize),
        headers = njt_req_get_headers(),
    })
end


function _M.proxy_response(_, response, chunksize)
    njt_log(njt_DEBUG, "Use of deprecated function `proxy_response`")

    if not response then
        njt_log(njt_ERR, "no response provided")
        return
    end

    njt.status = response.status
    ...
    repeat
        ...
        if read_err then
            njt_log(njt_ERR, read_err)
        end

        if chunk then
            ok, print_err = njt_print(chunk)
            if not ok then
                njt_log(njt_ERR, print_err)
            end
        end
