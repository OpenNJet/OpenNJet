...
int tls13_enc(SSL *s, SSL3_RECORD *recs, size_t n_recs, int sending)
{
    ...
    uint32_t alg_enc;
    WPACKET wpkt;
    unsigned char dtls13RecoedNumber[SEQ_NUM_SIZE], *p = dtls13RecoedNumber;
    ...
    if (sending) {
        ...
        if (SSL_IS_DTLS(s)) {
            ...
            memcpy(p, RECORD_LAYER_get_write_sequence(&s->rlayer)+2, 6);
            seq = dtls13RecoedNumber;
        }
        else
        ...
        staticiv = s->read_iv;
        if (SSL_IS_DTLS(s)) {
            s2n(DTLS_RECORD_LAYER_get_r_epoch(&s->rlayer), p);
            memcpy(p, RECORD_LAYER_get_read_sequence(&s->rlayer)+2, 6);
            seq = dtls13RecoedNumber;
        }
        else
     ...
     */
    if (ctx == NULL || (!SSL_IS_DTLS(s) && rec->type == SSL3_RT_ALERT)) {
    ...
    if (SSL_IS_DTLS(s)) {
        if (sending) {
            if (!dtls13_set_unified_hdr_L(s, rec->length + taglen,
                            SSL3_RECORD_get_dtls13_unified_hdr(recs),
                            SSL3_RECORD_get_dtls13_unified_hdr_len(recs)))
              return -1;
        }
        if (!WPACKET_init_static_len(&wpkt, recheader, sizeof(recheader), 0)
                    ...
                    || hdrlen != SSL3_RECORD_get_dtls13_unified_hdr_len(recs)
                    || !WPACKET_finish(&wpkt)) {
