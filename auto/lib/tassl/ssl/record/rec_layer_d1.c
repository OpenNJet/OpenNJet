 ...
 start:
    ...
    if (type == SSL3_RECORD_get_type(rr)
        || ((SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC
            ||SSL3_RECORD_get_type(rr) == DTLS13_RT_ACK) 
                   ...
                   size_t len, int create_empty_fragment, size_t *written)
{
    ...
    SSL3_BUFFER *wb;
    SSL_SESSION *sess;
    int dtls13_unified_hdr_write = SSL_IS_DTLS13(s) && s->enc_write_ctx != NULL;
    int unified_hdr_len;
    ...
    /* write the header */
    if (dtls13_unified_hdr_write) {
        if (!dtls13_set_unified_hdr(s, 
                        DTLS13_UNIFIED_HDR_FIXED_BITS | 
                        DTLS13_UNIFIED_HDR_S_MASK | 
                        DTLS13_UNIFIED_HDR_L_MASK,
                        p, &unified_hdr_len)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DO_DTLS1_WRITE,
                        SSL_R_SET_UNIFIED_HDR);
            return -1;
        }
        SSL3_RECORD_set_dtls13_unified_hdr(&wr, p);
        SSL3_RECORD_set_dtls13_unified_hdr_len(&wr, unified_hdr_len);
        p += unified_hdr_len;
    ...
    if (dtls13_unified_hdr_write) {
        /* add type and padding */
        *(SSL3_RECORD_get_input(&wr) + SSL3_RECORD_get_length(&wr)) = type;
        SSL3_RECORD_add_length(&wr, 1);
    }
    /*
    ...
    if (s->msg_callback)
        s->msg_callback(1, 0, SSL3_RT_HEADER, pseq - (dtls13_unified_hdr_write ? 
                        SSL3_RECORD_get_dtls13_unified_hdr_len(&wr) : DTLS1_RT_HEADER_LENGTH),
                        (dtls13_unified_hdr_write ? 
                         SSL3_RECORD_get_dtls13_unified_hdr_len(&wr) : DTLS1_RT_HEADER_LENGTH), 
    ...
    SSL3_RECORD_add_length(&wr, dtls13_unified_hdr_write ? 
                SSL3_RECORD_get_dtls13_unified_hdr_len(&wr) : DTLS1_RT_HEADER_LENGTH);
...
void dtls1_reset_seq_numbers(SSL *s, int rw, int epoch)
{
    unsigned char *seq;
...
int dtls13_set_unified_hdr(SSL *s, unsigned char bits, unsigned char *p, int *unified_hdr_len)
{
    /* bits */
    *(p++) = bits | (s->rlayer.d->w_epoch & DTLS13_UNIFIED_HDR_E_MASK);
    *unified_hdr_len = DTLS13_UNIFIED_HDR_BITS_LEN;
   
    /* TODO: when C present */
    if (bits & DTLS13_UNIFIED_HDR_C_MASK) {
        return 0;
    }

    /* Sequence Number field */
    if (bits & DTLS13_UNIFIED_HDR_S_MASK) {
        memcpy(p, RECORD_LAYER_get_write_sequence(&s->rlayer) + SEQ_NUM_SIZE - 2, 2);
        p += 2;
        *unified_hdr_len += 2;
    } else {
        memcpy(p, RECORD_LAYER_get_write_sequence(&s->rlayer) + SEQ_NUM_SIZE - 1, 1);
        p += 1;
        *unified_hdr_len += 1;
    }
    
    /* Length field, filled later in dtls13_set_unified_hdr_L */
    if (bits & DTLS13_UNIFIED_HDR_L_MASK) {
        p += 2;
        *unified_hdr_len += 2;
    }
...
int dtls13_get_unified_hdr(SSL *s, unsigned char bits, size_t *unified_hdr_len, unsigned long *epoch)
{
    if (DTLS13_UNIFIED_HDR_FIXED_BITS != (bits & DTLS13_UNIFIED_HDR_FIXED_BITS_MASK)) {
        return 0;
    }
    ...
    if (epoch) {
        *epoch = DTLS13_UNIFIED_HDR_E_MASK & bits;
    }

    return 1;
}

int dtls13_set_unified_hdr_L(SSL *s, size_t length, unsigned char *p, int unified_hdr_len)
{
    unsigned char *L_offset = p + unified_hdr_len - 2;
    if (*p & DTLS13_UNIFIED_HDR_L_MASK) {
        s2n(length, L_offset);
    }
    return 1;
}

int dtls13_get_unified_hdr_L(SSL *s, size_t *length, unsigned char *p, int unified_hdr_len)
{
    unsigned char *L_offset = NULL;
    if (*p & DTLS13_UNIFIED_HDR_L_MASK) {
        L_offset = p + unified_hdr_len - 2;
        n2s(L_offset, *length);
    } else {
        /* TODO: how to get length when L not present */
        return 0;
    }
    return 1;
}

int dtls13_get_unified_hdr_S(SSL *s, unsigned char **sn, int *s_len, unsigned char *p, int unified_hdr_len)
{
    if (*p & DTLS13_UNIFIED_HDR_S_MASK)
      *s_len = 2;
    else
      *s_len = 1;

    if (*p & DTLS13_UNIFIED_HDR_L_MASK)
      *sn = p + unified_hdr_len - 2 - *s_len;
    else
      *sn = p + unified_hdr_len - *s_len;
