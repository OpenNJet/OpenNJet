...
static void net__print_error(unsigned int log, const char *format_str)
{
	...
	iot_log__printf(NULL, log, format_str, buf);
	LocalFree(buf);
#else
	buf = strerror(errno);
	iot_log__printf(NULL, log, format_str, buf);
...
struct mosq_iot *net__socket_accept(struct mosquitto__listener_sock *listensock)
{
	...
	if (!hosts_access(&wrap_req))
	{
		...
		if (db.config->connection_messages == true)
		{
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Client connection from %s denied access by tcpd.", address);
			}
		}
		COMPAT_CLOSE(new_sock);
		return NULL;
	...
	if (db.config->set_tcp_nodelay)
	{
		...
		{
#endif
			iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Unable to set TCP_NODELAY.");
		}
	}
	...
	if (new_context->listener->max_connections > 0 && new_context->listener->client_count > new_context->listener->max_connections)
	{
		if (db.config->connection_messages == true)
		{
			iot_log__printf(NULL, MOSQ_LOG_NOTICE, "Client connection from %s denied: max_connections exceeded.", new_context->address);
		}
		context__cleanup(new_context, true);
		return NULL;
	}
	...
	if (db.config->connection_messages == true)
	{
		iot_log__printf(NULL, MOSQ_LOG_NOTICE, "New connection from %s:%d on port %d.",
...
int net__tls_server_ctx(struct mosquitto__listener *listener)
{
	...
	if (!listener->ssl_ctx)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to create TLS context.");
		return MOSQ_ERR_TLS;
	}
	...
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unsupported tls_version \"%s\".", listener->tls_version);
		return MOSQ_ERR_TLS;
	}
	...
	if (listener->ciphers)
	{
		...
		if (rc == 0)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to set TLS ciphers. Check cipher list \"%s\".", listener->ciphers);
			return MOSQ_ERR_TLS;
		}
	}
	else
	...
	if (listener->ciphers_tls13)
	{
		...
		if (rc == 0)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to set TLS 1.3 ciphersuites. Check cipher_tls13 list \"%s\".", listener->ciphers_tls13);
			return MOSQ_ERR_TLS;
		}
	}
#endif
	...
	if (listener->dhparamfile)
	{
		...
		if (!dhparamfile)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error loading dhparamfile \"%s\".", listener->dhparamfile);
			return MOSQ_ERR_TLS;
		}
...
int net__load_crl_file(struct mosquitto__listener *listener)
{
	...
	if (!store)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to obtain TLS store.");
	...
	if (rc < 1)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load certificate revocation file \"%s\". Check crlfile.", listener->crlfile);
		net__print_error(MOSQ_LOG_ERR, "Error: %s");
		net__print_ssl_error(NULL);
		return MOSQ_ERR_TLS;
	}
...
int net__load_certificates(struct mosquitto__listener *listener)
{
	...
	if (rc != 1)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load server certificate \"%s\". Check certfile.", listener->certfile);
		net__print_ssl_error(NULL);
		return MOSQ_ERR_TLS;
	}
	if (listener->tls_engine == NULL)
	{
		...
		if (rc != 1)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load server key file \"%s\". Check keyfile.", listener->keyfile);
			net__print_ssl_error(NULL);
			return MOSQ_ERR_TLS;
		}
	}
	...
	if (rc != 1)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Server certificate/key are inconsistent.");
		net__print_ssl_error(NULL);
		return MOSQ_ERR_TLS;
	}
...
static int net__load_engine(struct mosquitto__listener *listener)
{
	...
	if (!engine)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error loading %s engine\n", listener->tls_engine);
		net__print_ssl_error(NULL);
		return MOSQ_ERR_TLS;
	}
	if (!ENGINE_init(engine))
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Failed engine initialisation\n");
		net__print_ssl_error(NULL);
		return MOSQ_ERR_TLS;
	}
	...
	if (listener->tls_keyform == mosq_k_engine)
	{
		...
		if (listener->tls_engine_kpass_sha1)
		{
			...
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to set engine secret mode sha");
				net__print_ssl_error(NULL);
				return MOSQ_ERR_TLS;
			}
			if (!ENGINE_ctrl_cmd(engine, ENGINE_PIN, 0, listener->tls_engine_kpass_sha1, NULL, 0))
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to set engine pin");
				net__print_ssl_error(NULL);
				return MOSQ_ERR_TLS;
			}
			ui_method = NULL;
		...
		if (!pkey)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load engine private key file \"%s\".", listener->keyfile);
			net__print_ssl_error(NULL);
			return MOSQ_ERR_TLS;
		}
		if (SSL_CTX_use_PrivateKey(listener->ssl_ctx, pkey) <= 0)
		{
			iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to use engine private key file \"%s\".", listener->keyfile);
			net__print_ssl_error(NULL);
			return MOSQ_ERR_TLS;
		}
	}
...
int net__tls_load_verify(struct mosquitto__listener *listener)
{
	...
	if (listener->cafile || listener->capath)
	{
		...
		if (rc == 0)
		{
			if (listener->cafile && listener->capath)
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load CA certificates. Check cafile \"%s\" and capath \"%s\".", listener->cafile, listener->capath);
			}
			else if (listener->cafile)
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load CA certificates. Check cafile \"%s\".", listener->cafile);
			}
			else
			{
				iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load CA certificates. Check capath \"%s\".", listener->capath);
			}
		}
	}
#else
...
static int net__bind_interface(struct mosquitto__listener *listener, struct addrinfo *rp)
{
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
	{
		...
		if (!strcasecmp(listener->bind_interface, ifa->ifa_name) && ifa->ifa_addr->sa_family == rp->ai_addr->sa_family)
		{
			...
			if (rp->ai_addr->sa_family == AF_INET)
			{
						   ...
						   sizeof(struct in_addr)))
				{
					...
					iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Interface address for %s does not match specified listener address (%s).",
	...
	freeifaddrs(ifaddr);
	iot_log__printf(NULL, MOSQ_LOG_WARNING, "Warning: Interface %s does not support %s configuration.",
...
static int net__socket_listen_tcp(struct mosquitto__listener *listener)
{
	...
	if (rc)
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error creating listener: %s.", gai_strerror(rc));
	...
	for (rp = ainfo; rp; rp = rp->ai_next)
	{
		...
		{
			iot_log__printf(NULL, MOSQ_LOG_INFO, "Opening ipv4 listen socket on port %d.", ntohs(((struct sockaddr_in *)rp->ai_addr)->sin_port));
		}
		else if (rp->ai_family == AF_INET6)
		{
			iot_log__printf(NULL, MOSQ_LOG_INFO, "Opening ipv6 listen socket on port %d.", ntohs(((struct sockaddr_in6 *)rp->ai_addr)->sin6_port));
		}
		else
		{
			continue;
...
static int net__socket_listen_unix(struct mosquitto__listener *listener)
{
	...
	{
		iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Path to unix socket is too long \"%s\".", listener->unix_socket_path);
		return MOSQ_ERR_INVAL;
	}

	unlink(listener->unix_socket_path);
	iot_log__printf(NULL, MOSQ_LOG_INFO, "Opening unix listen socket on path %s.", listener->unix_socket_path);
	...
	if (listener->sock_count > 0)
	{
		...
		else if (listener->psk_hint)
		{
			...
			if (listener->psk_hint)
			{
				...
				if (rc == 0)
				{
					iot_log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to set TLS PSK hint.");
					net__print_ssl_error(NULL);
					return 1;
				}
			}
