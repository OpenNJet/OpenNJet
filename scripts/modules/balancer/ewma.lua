...
local function lock(upstream)
  ...
  if err then
    if err ~= "timeout" then
      njt.log(njt.ERR, string.format("EWMA Balancer failed to lock: %s", tostring(err)))
    end
  end
...
local function unlock()
  ...
  if not ok then
    njt.log(njt.ERR, string.format("EWMA Balancer failed to unlock: %s", tostring(err)))
  end
...
local function store_stats(upstream, ewma, now)
  local success, err, forcible = njt.shared.balancer_ewma_last_touched_at:set(upstream, now)
  if not success then
    njt.log(njt.WARN, "balancer_ewma_last_touched_at:set failed " .. err)
  end
  if forcible then
    njt.log(njt.WARN, "balancer_ewma_last_touched_at:set valid items forcibly overwritten")
  end

  success, err, forcible = njt.shared.balancer_ewma:set(upstream, ewma)
  if not success then
    njt.log(njt.WARN, "balancer_ewma:set failed " .. err)
  end
  if forcible then
    njt.log(njt.WARN, "balancer_ewma:set valid items forcibly overwritten")
  end
end
...
local function get_or_update_ewma(upstream, rtt, update)
  ...
  local ewma = njt.shared.balancer_ewma:get(upstream) or 0
  if lock_err ~= nil then
    return ewma, lock_err
  end

  local now = njt.now()
  local last_touched_at = njt.shared.balancer_ewma_last_touched_at:get(upstream) or 0
...
local function calculate_slow_start_ewma(self)
  ...
  for _, endpoint in pairs(self.peers) do
    ...
    local ewma = njt.shared.balancer_ewma:get(endpoint_string)
  ...
  if endpoints_count == 0 then
    njt.log(njt.INFO, "no ewma value exists for the endpoints")
    return nil
  end
...
function _M.balance(self)
  ...
  if #peers > 1 then
    ...
    if not njt.ctx.balancer_ewma_tried_endpoints then
      tried_endpoints = {}
      njt.ctx.balancer_ewma_tried_endpoints = tried_endpoints
    else
      tried_endpoints = njt.ctx.balancer_ewma_tried_endpoints
    end
    ...
    if not filtered_peers then
      njt.log(njt.WARN, "all endpoints have been retried")
  ...
  njt.var.balancer_ewma_score = ewma_score
...
function _M.after_balance(_)
  local response_time = tonumber(split.get_last_value(njt.var.upstream_response_time)) or 0
  local connect_time = tonumber(split.get_last_value(njt.var.upstream_connect_time)) or 0
  local rtt = connect_time + response_time
  local upstream = split.get_last_value(njt.var.upstream_addr)
...
function _M.sync(self, backend)
  ...
  if #normalized_endpoints_added == 0 and #normalized_endpoints_removed == 0 then
    njt.log(njt.INFO, "endpoints did not change for backend " .. tostring(backend.name))
    return
  end

  njt_log(INFO, string_format("[%s] peers have changed for backend %s", self.name, backend.name))

  self.peers = backend.endpoints

  for _, endpoint_string in ipairs(normalized_endpoints_removed) do
    njt.shared.balancer_ewma:delete(endpoint_string)
    njt.shared.balancer_ewma_last_touched_at:delete(endpoint_string)
  end
