...
function _M.set_cookie(self, value)
  ...
  if not cookie_path then
    cookie_path = njt.var.location_path
  end
  ...
  if cookie_samesite then
    ...
    if cookie_conditional_samesite_none
        ...
        and not same_site.same_site_none_compatible(njt.var.http_user_agent) then
      cookie_samesite = nil
    end
  end
  ...
  if cookie_secure == nil then
    cookie_secure = njt.var.https == "on"
  end
  ...
  if self.cookie_session_affinity.expires and self.cookie_session_affinity.expires ~= "" then
      cookie_data.expires = njt.cookie_time(njt.time() +
...
function _M.get_last_failure()
  return njt_balancer.get_last_failure()
end

local function get_failed_upstreams()
  local indexed_upstream_addrs = {}
  local upstream_addrs = split.split_upstream_var(njt.var.upstream_addr) or {}
...
local function should_set_cookie(self)
  local host = njt.var.host
  if njt.var.server_name == '_' then
    host = njt.var.server_name
  end
  ...
  if self.cookie_session_affinity.locations then
    ...
    if locs == nil then
      ...
      local wildcard_host, _, err = njt.re.sub(host, "^[^\\.]+\\.", "*.", "jo")
      if err then
    ...
    if locs ~= nil then
      for _, path in pairs(locs) do
        if njt.var.location_path == path then
          return true
        end
      end
    end
...
function _M.balance(self)
  ...
  if not new_upstream then
    njt.log(njt.WARN, string.format("failed to get new upstream; using upstream %s", new_upstream))
...
function _M.sync(self, backend)
  ...
  self.backend_key = njt.md5(njt.md5(backend.name) .. backend.name)
end
