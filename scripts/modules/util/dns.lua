...
local function cache_set(host, addresses, ttl)
  ...
  njt_log(njt_INFO, string_format("cache set for '%s' with value of [%s] and ttl of %s.",
...
function _M.lookup(host)
  ...
  if not r then
    njt_log(njt_ERR, string_format("failed to instantiate the resolver: %s", err))
    return { host }
  end
  ...
  if is_fully_qualified(host) then
    ...
    njt_log(njt_ERR, "failed to query the DNS server for ",
