...
local function get_implementation(backend)
  ...
  if not implementation then
    njt.log(njt.WARN, string.format("%s is not supported, falling back to %s",
...
local function sync_backend(backend)
  ...
  njt.log(njt.INFO, "sync tcp/udp backend: ", cjson.encode(backend))
  ...
  if getmetatable(balancer) ~= implementation then
    njt.log(njt.INFO, string.format("LB algorithm changed from %s to %s, "
...
local function sync_backends()
  ...
  njt.update_time()
  local current_timestamp = njt.now()
  ...
  backends_last_synced_at = current_timestamp
end
...
function _M.init_worker()
  ...
  local _, err = njt.timer.every(BACKENDS_SYNC_INTERVAL, sync_backends)
  if err then
    njt.log(njt.ERR, string.format("error when setting up timer.every "
...
function _M.balance()
  ...
  if PROHIBITED_LOCALHOST_PORT then
     print(PROHIBITED_LOCALHOST_PORT)
  end
  ...
  if not ok then
    njt.log(njt.ERR, string.format("error while setting current upstream peer %s: %s", peer, err))
  end
end
