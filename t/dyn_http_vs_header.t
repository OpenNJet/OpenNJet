...
$response3 =$t->get_with_port($json_url, 'localhost', 8080);

diag("Running del vs 6");
# del vs
my $r_del_vs;
$r_del_vs = http(<<EOF);
...
Host: localhost
Connection: close
Content-Length: @{[length($json_payload_del_vs)]}

$json_payload_del_vs 

EOF
like($r_del_vs, qr/"code":0,"msg":"success./, 'del vs ok');


diag("Running renew add vs 7");
$r = http(<<EOF);
POST /api/v1/dyn_srv HTTP/1.1
Host: localhost
Connection: close
...
EOF
like($r, qr/"code":0,"msg":"success./, 'renew add vs ok');
