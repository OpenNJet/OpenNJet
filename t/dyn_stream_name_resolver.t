...
stream {
     ...
     upstream back{
                zone back 10M;
                server  test.muti.com:80 resolve;
     }  
...
$t->run();
sleep 5;
my $json_url = '/api/v1/upstream_api/stream/upstreams/back/';  
#my $response = http_get($url);  
diag("Running /api/v1/upstream_api/stream/upstreams/ format");
my $response2 =$t->get_with_port($json_url, 'localhost', 8081);

like($response2, qr/"server":"127.0.0.2:80"/, '127.0.0.2:80 ok');

sleep 6;
my $response3 =$t->get_with_port($json_url, 'localhost', 8081);
like($response3, qr/"server":"127.0.0.2:80"/, '127.0.0.2:80 ok');
like($response3, qr/"server":"127.0.0.3:80"/, '127.0.0.3:80 ok');

sleep 10;
my $response4 =$t->get_with_port($json_url, 'localhost', 8081);
like($response4, qr/"server":"127.0.0.4:80"/, '127.0.0.4:80 ok');
