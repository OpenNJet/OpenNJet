...
static njt_http_core_srv_conf_t *njt_dyn_bwlist_get_srv_by_port(njt_cycle_t *cycle, njt_pool_t *pool, njt_str_t *addr_port, njt_str_t *server_name)
{
        ...
        for (i = 0; i < cycle->listening.nelts; i++)
        {
            ...
            else
            {
                ...
                if (p != NULL)
                {
                    ...
                    if (dport.len == sport.len && njt_strncmp(dport.data, sport.data, dport.len) == 0 && wide_addr.len == len && njt_strncmp(wide_addr.data, ls[i].addr_text.data, wide_addr.len) == 0)
                    {
                        target_ls = &ls[i];
                        break;
                    }
...
static njt_int_t njt_dyn_bwlist_update_access_conf(njt_pool_t *pool, njt_http_dyn_bwlist_main_t *api_data)
{
    ...
    {
        cscf = njt_dyn_bwlist_get_srv_by_port(cycle, pool, (njt_str_t *)daas[i].listens.elts, (njt_str_t *)daas[i].server_names.elts);
        if (cscf == NULL)
        {
