...
struct	fargs {
	...
	size_t	   sourcesz; /* number of sources, must .eq. 1 now */ 
...
int io_read_line(struct sess *sess, int fd, char *buf, size_t bufsiz);
...
int	rsync_server(const struct opts *, int);
int rsync_server_daemon(const struct opts *opts, const char *bing_address, int port);
int *open_socket_in(int type, int port, const char *bind_addr, int af_hint); // new add
