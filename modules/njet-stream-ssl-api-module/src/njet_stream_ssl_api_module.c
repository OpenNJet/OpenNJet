...
static void
njt_stream_dyn_ssl_read_data(njt_http_request_t *r);

static njt_int_t
njt_stream_dyn_ssl_handler(njt_http_request_t *r);

static njt_int_t
njt_stream_dyn_ssl_init_worker(njt_cycle_t *cycle);

static void *
njt_stream_dyn_ssl_create_main_conf(njt_conf_t *cf);

static njt_int_t
njt_stream_dyn_ssl_init(njt_conf_t *cf);
...
static njt_int_t njt_stream_dyn_ssl_rpc_send(njt_http_request_t *r,njt_str_t *module_name,njt_str_t *msg, int retain);

static int njt_stream_dyn_ssl_request_output(njt_http_request_t *r,njt_int_t code, njt_str_t *msg);

typedef struct njt_stream_dyn_ssl_ctx_s {
} njt_stream_dyn_ssl_ctx_t, njt_stream_stream_dyn_ssl_ctx_t;


typedef struct njt_stream_dyn_ssl_main_conf_s {  //njt_stream_dyn_ssl_main_cf_t
	njt_http_request_t **reqs;
    njt_int_t size;
} njt_stream_dyn_ssl_main_conf_t;


typedef struct {
    njt_http_request_t *req;
    njt_int_t index;
    njt_stream_dyn_ssl_main_conf_t *dlmcf;
}njt_stream_dyn_ssl_rpc_ctx_t;
...
static njt_http_module_t njt_stream_ssl_api_module_ctx = {
        NULL,                              /* preconfiguration */
        njt_stream_dyn_ssl_init,             /* postconfiguration */

        njt_stream_dyn_ssl_create_main_conf, /* create main configuration */
...
njt_module_t njt_stream_ssl_api_module = {
        NJT_MODULE_V1,
        &njt_stream_ssl_api_module_ctx, /* module context */
        NULL,                           /* module directives */
        NJT_HTTP_MODULE,                    /* module type */
        NULL,                               /* init master */
        NULL,                               /* init module */
        njt_stream_dyn_ssl_init_worker, /* init process */
        NULL,                               /* init thread */
        NULL,                               /* exit thread */
        NULL,                               /* exit process */
        NULL,                               /* exit master */
...
njt_stream_dyn_ssl_init(njt_conf_t *cf) {
    ...
    dlmcf = njt_http_conf_get_module_main_conf(cf,njt_stream_ssl_api_module);
    if(dlmcf == NULL){
        return NJT_ERROR;
    }
    ...
    if(dlmcf->reqs == NULL){
        njt_log_error(NJT_LOG_EMERG, njt_cycle->log, 0, "njt_stream_dyn_ssl_postconfiguration alloc mem error");
        return NJT_ERROR;
    }

    njt_str_t  module_key = njt_string("/v1/stream_ssl");
    njt_memzero(&h, sizeof(njt_http_api_reg_info_t));
    h.key = &module_key;
    h.handler = njt_stream_dyn_ssl_handler;
...
njt_stream_dyn_ssl_create_main_conf(njt_conf_t *cf) {
    ...
    njt_stream_dyn_ssl_main_conf_t *uclcf;

    //size = (ssize_t)(10 * njt_pagesize);
    uclcf = njt_pcalloc(cf->pool, sizeof(njt_stream_dyn_ssl_main_conf_t));
    if (uclcf == NULL) {
        njt_log_error(NJT_LOG_ERR, cf->log, 0, "malloc njt_stream_dyn_ssl_main_conf_t eror");
        return NULL;
    }
...
njt_stream_dyn_ssl_handler(njt_http_request_t *r) {
    ...
    if(r->method == NJT_HTTP_PUT){
        rc = njt_http_read_client_request_body(r, njt_stream_dyn_ssl_read_data);
    ...
    if(r->method == NJT_HTTP_GET){
        ...
        njt_str_set(&topic, "/worker_a/rpc/stream_ssl");

        rc = njt_stream_dyn_ssl_rpc_send(r, &topic, &smsg, 0);
        if(rc != NJT_OK){
            goto err;
        }
        ++r->main->count;
    ...
    //     msg = srv_err;
    // }
    // return njt_stream_dyn_ssl_request_output(r,rc,&msg);

    err:
    return njt_stream_dyn_ssl_request_output(r, NJT_HTTP_INTERNAL_SERVER_ERROR, &srv_err);
}
...
static njt_int_t njt_stream_dyn_ssl_get_free_index(njt_stream_dyn_ssl_main_conf_t *dlmcf){
    njt_int_t i;
...
static void njt_stream_dyn_ssl_cleanup_handler(void *data){
    njt_stream_dyn_ssl_rpc_ctx_t *ctx;
...
static int njt_stream_dyn_ssl_rpc_msg_handler(njt_dyn_rpc_res_t* res, njt_str_t *msg){
    njt_stream_dyn_ssl_rpc_ctx_t          *ctx;
    njt_http_request_t                  *req;
    njt_int_t                           rc;
    ...
    if( ctx->dlmcf->size > ctx->index && ctx->dlmcf->reqs[ctx->index] == ctx->req){
        req =  ctx->req;
        if(res->rc == RPC_RC_OK){
            rc = njt_stream_dyn_ssl_request_output(req,NJT_OK,msg);
        }
        if(res->rc == RPC_RC_TIMEOUT){
            rc = njt_stream_dyn_ssl_request_output(req,NJT_HTTP_INTERNAL_SERVER_ERROR,&err_msg);
        }
...
static njt_int_t njt_stream_dyn_ssl_rpc_send(njt_http_request_t *r,njt_str_t *module_name,njt_str_t *msg, int retain){
    ...
    dlmcf = njt_http_get_module_main_conf(r,njt_stream_ssl_api_module);
    if(dlmcf == NULL){
        goto err;
    }
    index = njt_stream_dyn_ssl_get_free_index(dlmcf);
    if(index == -1 ){
    ...
    ctx = njt_pcalloc(r->pool, sizeof(njt_stream_dyn_ssl_rpc_ctx_t));
    if(ctx == NULL){
    ...
    cleanup->handler = njt_stream_dyn_ssl_cleanup_handler;
    cleanup->data = ctx;
    njt_log_error(NJT_LOG_INFO, r->pool->log, 0, "send rpc time : %M",njt_current_msec);
    rc = njt_dyn_rpc(module_name,msg, retain, index, njt_stream_dyn_ssl_rpc_msg_handler, ctx);
    if(rc == NJT_OK){
        dlmcf->reqs[index] = r;
    }
    return rc;
...
njt_stream_dyn_ssl_read_data(njt_http_request_t *r){
    ...
    uint32_t                            crc32;
    uint32_t						    topic_len = NJT_INT64_LEN  + 2 + 256; ///ins/stream_ssl/l_
    ...
    api_data = json_parse_stream_dyn_ssl_api(pool, &json_str, &err_info);
    if (api_data == NULL)
    {
	...
	for (i = 0; i < api_data->serverNames->nelts; i++){
        serverName = get_stream_dyn_ssl_api_serverNames_item(api_data->serverNames, i);
        if(serverName->len > 0){
	...
	for(i = 0; i < api_data->listens->nelts; i++){
        listen_str = get_stream_dyn_ssl_api_listens_item(api_data->listens, i);
        if(listen_str->len > 0){
            njt_crc32_update(&crc32, listen_str->data, listen_str->len);
        }
	}

    if(api_data->cert_info->cert_type == STREAM_DYN_SSL_API_CERT_INFO_CERT_TYPE_NTLS){
        njt_crc32_update(&crc32, (u_char*)"ntls", 4);
    }else if(api_data->cert_info->cert_type == STREAM_DYN_SSL_API_CERT_INFO_CERT_TYPE_RSA){
	...
	// 	p = njt_snprintf(topic_name.data,topic_len,"/ins/stream_ssl/l_%ui",crc32);
    ...
    //     njt_stream_dyn_ssl_request_output(r, NJT_HTTP_OK, &httpmsg);

    //     goto out;
	// } else {
    //     p = njt_snprintf(topic_name.data,topic_len,"/worker_a/ins/stream_ssl/l_%ui",crc32);
	// }

    //just send to pa
    p = njt_snprintf(topic_name.data,topic_len,"/worker_a/ins/stream_ssl/l_%ui",crc32);
	topic_name.len = p - topic_name.data;
	rc = njt_stream_dyn_ssl_rpc_send(r, &topic_name, &json_str, 0);
	if(rc == NJT_OK) {
		++r->main->count;
	}
...
err:
    ...
    err_ctx->success = 0;
    err_ctx->code = rc;
    njt_http_set_ctx(r, err_ctx, njt_stream_ssl_api_module);
    ...
    njt_stream_dyn_ssl_request_output(r, NJT_HTTP_BAD_REQUEST, &bad_req);
