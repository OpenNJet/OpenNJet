...
static bool parse_auth_passwd_patch_api(njt_pool_t *pool, parse_state_t *parse_state, auth_passwd_patch_api_t *out, js2c_parse_error_t *err_ret) {
    njt_uint_t i;
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (current_string_is(parse_state, "password")) {
...
static void get_json_length_auth_passwd_patch_api_password(njt_pool_t *pool, auth_passwd_patch_api_password_t *out, size_t *length, njt_int_t flags) {
    njt_str_t *dst = handle_escape_on_write(pool, out);
    *length += dst->len + 2; //  "str" 
}

static void get_json_length_auth_passwd_patch_api(njt_pool_t *pool, auth_passwd_patch_api_t *out, size_t *length, njt_int_t flags) {
    if (out == NULL) {
        *length += 4; // null
        return;
    }
    *length += 1;
    ...
    if (omit == 0) {
        *length += (8 + 3); // "password": 
        get_json_length_auth_passwd_patch_api_password(pool, (&out->password), length, flags);
        *length += 1; // ","
        count++;
    }
    if (count != 0) {
...
auth_passwd_patch_api_t* create_auth_passwd_patch_api(njt_pool_t *pool) {
    auth_passwd_patch_api_t* out = njt_pcalloc(pool, sizeof(auth_passwd_patch_api_t));
    return out;
}

static void to_oneline_json_auth_passwd_patch_api_password(njt_pool_t *pool, auth_passwd_patch_api_password_t *out, njt_str_t *buf, njt_int_t flags) {
...
static void to_oneline_json_auth_passwd_patch_api(njt_pool_t *pool, auth_passwd_patch_api_t *out, njt_str_t* buf, njt_int_t flags) {
    njt_int_t omit;
    u_char* cur = buf->data + buf->len;
    if (out == NULL) {
    ...
    if (omit == 0) {
        ...
        buf->len = cur - buf->data;
        to_oneline_json_auth_passwd_patch_api_password(pool, (&out->password), buf, flags);
...
auth_passwd_patch_api_t* json_parse_auth_passwd_patch_api(njt_pool_t *pool, const njt_str_t *json_string, js2c_parse_error_t *err_ret) {
    ...
    out = njt_pcalloc(pool, sizeof(auth_passwd_patch_api_t));;
    if (parse_auth_passwd_patch_api(pool, parse_state, out, err_ret)) {
        return NULL;
    }
    return out;
}
...
njt_str_t* to_json_auth_passwd_patch_api(njt_pool_t *pool, auth_passwd_patch_api_t* out, njt_int_t flags) {
    ...
    size_t str_len = 0;
    get_json_length_auth_passwd_patch_api(pool, out, &str_len, flags);
    json_str->data = (u_char*)njt_pcalloc(pool, str_len + 1);
    json_str->len = 0;
    to_oneline_json_auth_passwd_patch_api(pool, out, json_str, flags);
    return json_str;
}
