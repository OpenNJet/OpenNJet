...
njt_helper_run(helper_param param)
{
    ...
    init_cycle.shared_slab = njet_master_cycle->shared_slab; // for dyn slab
    ...
    njt_log_error(NJT_LOG_NOTICE, cycle->log, 0, "helper privilege started");
    njt_reconfigure = 1;
    ...
    for ( ;; ) {
        if (njt_reconfigure) {
            ...
            njt_log_error(NJT_LOG_NOTICE, cycle->log, 0, "privilege reconfiguring");
            cycle = njt_init_cycle(cycle);
            if (cycle == NULL) {
                cycle = (njt_cycle_t *) njt_cycle;
                njt_log_error(NJT_LOG_NOTICE, cycle->log, 0, "privilege reconfiguring continue");
                return;
            }
#if (NJT_KEEP_MASTER_CYCLE)
            cycle->old_cycle = njet_master_cycle;
#endif
            njt_log_error(NJT_LOG_NOTICE, cycle->log, 0, "privilege reconfiguring done");
            njt_cycle = cycle;
            ...
            for (i = 0; cycle->modules[i]; i++) {
                if (cycle->modules[i]->init_process) {
                    if (cycle->modules[i]->init_process(cycle) == NJT_ERROR) {
                        /* fatal */
                        njt_log_error(NJT_LOG_EMERG, njt_cycle->log, 0, "privilege fatal error");
                        njt_log_error(NJT_LOG_EMERG, cycle->log, 0, "privilege fatal error");
                        exit(2);
                    }
                }
            }
        ...
        if (cmd == NJT_HELPER_CMD_STOP) {
            njt_log_error(NJT_LOG_NOTICE, cycle->log, 0, "helper privilege exit");
...
njt_module_t njt_helper_privilege_module = {0};
