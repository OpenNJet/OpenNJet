...
void
njt_http_modsecurity_request_read(njt_http_request_t *r)
{
...
njt_int_t
njt_http_modsecurity_pre_access_handler(njt_http_request_t *r)
{
#if 1
    njt_pool_t                   *old_pool;
    njt_http_modsecurity_ctx_t   *ctx;
    ...
    */
    ...
    if (ctx->body_requested == 0)
    {
        ...
        rc = njt_http_read_client_request_body(r,
            njt_http_modsecurity_request_read);
        if (rc == NJT_ERROR || rc >= NJT_HTTP_SPECIAL_RESPONSE) {
#if (njet_version < 1002006) ||                                             \
    (njet_version >= 1003000 && njet_version < 1003009)
            r->main->count--;
#endif
        ...
        if (rc == NJT_AGAIN)
        {
            dd("njet is asking us to wait for more data.");
    ...
    if (ctx->waiting_more_body == 0)
    {
        ...
        njt_chain_t *chain = r->request_body->bufs;
         ...
         * the chunks to ModSecurity while njet keep calling this
         * function.
         */

        if (r->request_body->temp_file != NULL) {
            njt_str_t file_path = r->request_body->temp_file->file.name;
            const char *file_name = njt_str_to_char(file_path, r->pool);
            if (file_name == (char*)-1) {
                return NJT_HTTP_INTERNAL_SERVER_ERROR;
            }
            /*
...
/* XXX: once more -- is body can be modified ?  content-length need to be adjusted ? */
    ...
    dd("Nothing to add on the body inspection, reclaiming a NJT_DECLINED");
#endif
    return NJT_DECLINED;
}
