...
njt_stream_upstream_dynamic_servers_init(njt_conf_t *cf)
{
	...
	for (i = 0; i < umcf->upstreams.nelts; i++)
	{
		...
		if(have_dyserver == 1 &&  udsmcf->shm_zone == NULL) {
    ...
    njt_cycle_t *cycle)
{
	...
	njt_stream_upstream_dynamic_servers_cache_server(cycle);
    udsmcf = njt_stream_cycle_get_module_main_conf(cycle,
             njt_stream_upstream_dynamic_servers_module);
	if(udsmcf == NULL)
		return NJT_OK;
...
/*
static void *
njt_stream_upstream_dynamic_server_set_status(njt_stream_upstream_dynamic_server_conf_t *dynamic_server)
{
	njt_stream_upstream_rr_peer_t        *tail_peer;   
    njt_slab_pool_t              *pool;
    njt_stream_upstream_rr_peer_t  *dst;
