...
#include <njt_stream_dyn_upstream_parser.h>
...
extern njt_int_t njt_stream_upstream_init_zone(njt_shm_zone_t *shm_zone,
											 void *data);
extern void
njt_stream_upstream_zone_inherit_peer_status (njt_stream_upstream_rr_peers_t *peers,
                njt_stream_upstream_rr_peers_t *src_peers);
njt_int_t
njt_stream_dyn_upstream_merge_zone(njt_shm_zone_t *shm_zone, void *shpool);

extern njt_int_t njt_stream_upstream_keepalive_get_keepalive_time(njt_stream_upstream_srv_conf_t *upstream);
extern njt_int_t njt_stream_upstream_keepalive_get_keepalive_timeout(njt_stream_upstream_srv_conf_t *upstream);
extern njt_int_t njt_stream_upstream_keepalive_get_keepalive_requests(njt_stream_upstream_srv_conf_t *upstream);
extern njt_int_t njt_stream_upstream_keepalive_get_keepalive(njt_stream_upstream_srv_conf_t *upstream);
...
static njt_int_t njt_stream_dyn_upstream_write_data(njt_stream_dyn_upstream_info_t *upstream_info);

static njt_int_t njt_stream_check_upstream_body(njt_str_t cmd,void *data);
static njt_int_t   njt_stream_dyn_upstream_postconfiguration(njt_conf_t *cf);
static njt_int_t
njt_stream_dyn_upstream_init_zone_other(njt_shm_zone_t *shm_zone, njt_slab_pool_t *shpool);

static njt_stream_module_t njt_stream_dyn_upstream_module_ctx = {
	NULL, /* preconfiguration */
	&njt_stream_dyn_upstream_postconfiguration, /* postconfiguration */
...
njt_module_t njt_stream_dyn_upstream_module = {
	NJT_MODULE_V1,
	&njt_stream_dyn_upstream_module_ctx, /* module context */
...
static njt_int_t
njt_stream_dyn_upstream_delete_handler(njt_stream_dyn_upstream_info_t *upstream_info)
{
	...
	{ // 只删标准upstream，ref_count 默认是 1.
		...
		upstream->ref_count--;
		upstream->disable = 1;
		njt_str_set(&key,STREAM_UPSTREAM_OBJ);
		njt_http_object_dispatch_notice(&key,DELETE_NOTICE,upstream);
		if(njet_master_cycle != NULL) {
			njt_stream_upstream_del(njet_master_cycle,upstream);
			
		} else {
			njt_stream_upstream_del((njt_cycle_t *)njt_cycle,upstream);	
		}
		rc = NJT_OK;
...
static njt_int_t njt_stream_add_upstream_handler(njt_stream_dyn_upstream_info_t *upstream_info, njt_uint_t from_api_add)
{
	...
	njt_stream_upstream_srv_conf_t **uscfp = NULL;
	njt_stream_upstream_main_conf_t *umcf = NULL;
	njt_cycle_t *njet_curr_cycle = (njt_cycle_t *)njt_cycle;
	njt_conf_check_cmd_handler_t  cmd_handle;
	...
	if(njet_master_cycle != NULL) {
		stream_ctx = (njt_stream_conf_ctx_t *)njt_get_conf(njet_master_cycle->conf_ctx, njt_stream_module);
	...
	if(from_api_add == 1) {
		...
		njt_memzero(&cmd_handle,sizeof(cmd_handle));
	...
	if (ups_num == old_ups_num + 1)
	{
		...
		init = uscfp[old_ups_num]->peer.init_upstream ? uscfp[old_ups_num]->peer.init_upstream : njt_stream_upstream_init_round_robin;
		...
		uscfp[old_ups_num]->shm_zone->merge = njt_stream_dyn_upstream_merge_zone; //重写
		uscfp[old_ups_num]->shm_zone->noreuse = 1;
		hmcf_ctx = (njt_stream_conf_ctx_t *)njet_curr_cycle->conf_ctx[njt_stream_module.index];
		core_loc_conf  = hmcf_ctx->srv_conf[njt_stream_core_module.ctx_index];
		...
		if(uscfp[old_ups_num]->resolver != NULL) {
			njt_stream_upstream_add_name_resolve(uscfp[old_ups_num]);
		}
	}
out:
...
static int njt_agent_upstream_change_handler_internal(njt_str_t *key, njt_str_t *value, void *data, njt_str_t *out_msg)
{
	...
	njt_str_t obj_key = njt_string(UPS_DEL_STREAM_EVENT);
	njt_int_t rc = NJT_OK;
	njt_stream_dyn_upstream_info_t *upstream_info;
	njt_log_debug(NJT_LOG_DEBUG_STREAM, njt_cycle->log, 0, "topic msg get key=%V,value=%V", key, value);
	upstream_info = njt_stream_parser_upstream_data(*value, 0);
	...
	{
		rc = njt_stream_dyn_upstream_write_data(upstream_info);
		if (rc == NJT_OK)
		{
			...
			rc = njt_stream_add_upstream_handler(upstream_info, from_api_add); // njt_stream_dyn_upstream_delete_handler
			if (rc != NJT_OK)
			{
				if (from_api_add == 0)
				{
	...
	else if (upstream_info->type.len == del.len && njt_strncmp(upstream_info->type.data, del.data, upstream_info->type.len) == 0)
	{
		...
		if (rc == NJT_OK)
		{
			rc = njt_stream_dyn_upstream_delete_handler(upstream_info);
			if (rc == NJT_OK)
			{
				if (key->len > worker_str.len && njt_strncmp(key->data, worker_str.data, worker_str.len) == 0)
				{
...
static njt_str_t *njt_dyn_upstream_dump_conf(njt_cycle_t *cycle, njt_pool_t *pool)
{
	...
	njt_stream_upstream_srv_conf_t   **uscfp,*upstream;
...
njt_stream_dyn_upstream_init_worker(njt_cycle_t *cycle)
{
	...
	njt_str_t key = njt_string("stream_ups");
	...
	njt_str_t dyn_upstream_key = njt_string("dyn_stream_upstream");
...
njt_int_t njt_stream_check_top_server(njt_json_manager *json_body, njt_stream_dyn_upstream_info_t *upstream_info)
{
...
njt_stream_dyn_upstream_info_t *njt_stream_parser_upstream_data(njt_str_t json_str, njt_uint_t method)
{
	...
	upstream_info = njt_pcalloc(server_pool, sizeof(njt_stream_dyn_upstream_info_t));
	if (upstream_info == NULL)
	{
		njt_destroy_pool(server_pool);
		return NULL;
	...
	rc = njt_stream_check_top_server(&json_body, upstream_info);
	if (rc == NJT_ERROR)
	{
		goto end;
	}
...
static njt_int_t njt_stream_upstream_write_file(njt_fd_t fd, njt_stream_dyn_upstream_info_t *upstream_info)
{
...
static njt_int_t njt_stream_dyn_upstream_write_data(njt_stream_dyn_upstream_info_t *upstream_info)
{
	...
	upstream_info->upstream_name = upstream_info->old_upstream_name;
	if(njet_master_cycle != NULL) {
		upstream = njt_stream_util_find_upstream((njt_cycle_t *)njet_master_cycle, &upstream_info->upstream_name);
	} else {
		upstream = njt_stream_util_find_upstream((njt_cycle_t *)njt_cycle, &upstream_info->upstream_name);
	}
	...
	{
		njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "njt_stream_dyn_upstream_write_data njt_open_file[%V] error!", &server_full_file);
		rc = NJT_ERROR;
		goto out;
	}
	rc = njt_stream_upstream_write_file(fd, upstream_info);

	njt_close_file(fd);
	if (rc == NJT_ERROR)
	{
		njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "njt_stream_upstream_write_data error!");
		goto out;
	}
	...
	if (old_shpool && shm_zone->shm.exists == 0)
	{
		for (i = 0; i < umcf->upstreams.nelts; i++)
		{
			...
			if (uscf->hc_type == 2)
			{
				...
				njt_stream_upstream_zone_inherit_peer_status(uscf->peer.data, peers);
				uscf->reload = 1;
			}
		}
	}
...
njt_stream_dyn_upstream_init_zone_other(njt_shm_zone_t *shm_zone, njt_slab_pool_t *shpool)
{
	...
	for (i = 0; i < umcf->upstreams.nelts; i++)
	{
		...
		peersp = (njt_stream_upstream_rr_peers_t **)(void *)&old_shpool->data; // worker 直接用共享内存。获取peers。
		peers = *peersp;
