...
#define GOSSIP_APP_CLUSTER_LIMIT_CONN  0x57B7D7D
...
static void njt_http_cluster_limit_conn_sync(njt_event_t *ev)
{
	...
	if ( !njt_exiting) {
		...
		for(i = 0; i < clconn_ctxes->nelts; i++){
			if(zone_ctxes[i] == NULL){
				njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0," cluster_limit_conn_sync sitckey[%d] is null", i);
				continue;
			}
...
static void njt_cluster_limit_conn_update_conn(njt_str_t key_in, int other_conn, njt_http_cluster_limit_conn_ctx_t *ctx, njt_str_t sibling_node)
{
    ...
    if (node == NULL)
    {
        njt_log_error(NJT_LOG_INFO, njt_cycle->log, 0, "need create new tree node, receive:%d,%V", other_conn, &sibling_node);
        ...
        njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "new node:%d", lc->conn);
        njt_shmtx_unlock(&ctx->shpool->mutex);
    }
    else
    {
        lc = (njt_http_cluster_limit_conn_node_t *)&node->color;
        njt_log_error(NJT_LOG_INFO, njt_cycle->log, 0, "found tree node:%d, receive:%d,%V", lc->conn, other_conn, &sibling_node);
        for (i = 0; i < SIBLING_MAX; i++)
        {
            v = &lc->sibling[i];
            ...
            {
                njt_log_error(NJT_LOG_INFO, njt_cycle->log, 0, "%s,%d,check %V update sibling:%d", v->sibling_item.data, v->sibling_item.len, &sibling_node, other_conn);
        ...
        njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "overflow in array, found:%d, receive:%d,%V", lc->conn, other_conn, &sibling_node);
    }
}
...
static int njt_cluster_limit_conn_recv_data(const char* msg, void* data)
{
    ...
    {
        //todo
        njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "udp decode failed, maybe not for us", size);
        return NJT_ERROR;
    }
    ...
    {
        njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, " key is not nodeid:%V", &key);
        return NJT_ERROR;
    }
    ...
    {
        njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, " key is not zone:%V", &key);
        return NJT_ERROR;
    }
    ...
    if(ctx == NULL){
        njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, " not found ctx by zone:%V", &zone_val);
        return NJT_ERROR;
    }
        ...
        && njt_memcmp(sibling_node.data, ctx->node_name->data, sibling_node.len) == 0)
    {
        ...
        njt_log_error(NJT_LOG_DEBUG, njt_cycle->log, 0, "send from:%V, skip", &sibling_node);
        return NJT_ERROR;
    }
    ...
    for (size = 0; size < arr_cnt; size++)
    {
        ...
        {
            njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "not idx:%V", &key);
            return NJT_ERROR;
        }
        ...
        {
            njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "not now:%V", &key);
            return NJT_ERROR;
        }
    ...
    while (client != NULL)
    {
        ...
        if (now - client->q_item.last_changed >= SYNC_INT)
        {
            njt_log_error(NJT_LOG_ERR,njt_cycle->log,0,
                "===========not need syn, conn:%d  client:%p now:%M  last_changed:%M  diff:%M",
        ...
        njt_log_error(NJT_LOG_ERR,njt_cycle->log,0,
            "===========need syn, conn:%d  client:%p now:%M  last_changed:%M  diff:%M",
            client->q_item.sibling_item.conn, client, now, client->q_item.last_changed,
            now - client->q_item.last_changed);
        ...
        if (item == NULL)
        {
            njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0, "push error");
        }
        /*
    ...
    for (idx = 0; idx < out_arr.nelts; )
    {
		if (head == NULL ) {
			...
			if (buf_size <=0 || buf == NULL) {
				njt_log_error(NJT_LOG_ERR,njt_cycle->log,0,"apply buffer failed");
    			njt_shmtx_unlock(&ctx->shpool->mutex);
				return;
			}
 ...
 * */
        ...
        tail = mp_encode_array(tail, 1);
            njt_log_error(NJT_LOG_INFO,njt_cycle->log,0,
                    " =====cluster limit conn encode, node:%V zone:%V",
                    ctx->node_name, zone);
        msg_cnt = 0;
        for(i = 0; i < 15; i++){
            msg_cnt++;
            ...
            njt_log_error(NJT_LOG_INFO,njt_cycle->log,0,
                    " =====cluster limit conn encode, idx:%d key:%V  conn:%d",
                    idx, &tmp_str, item->conn);
			...
			//if (msg_cnt>=10) {
				...
				njt_log_error(NJT_LOG_INFO,njt_cycle->log,0," large sync pack:%d",msg_cnt);
	...
	if (msg_cnt > 0) {
		mp_encode_array(replace_cnt, msg_cnt);
		njt_log_error(NJT_LOG_INFO,njt_cycle->log,0,"sync pack:%d",msg_cnt);
		njt_gossip_send_app_msg_buf();
	}
...
njt_http_cluster_limit_conn_handler(njt_http_request_t *r)
{
    ...
    for (i = 0; i < lccf->limits.nelts; i++)
    {
        ...
        if (node == NULL)
        {
            ...
            njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0,
                          " ============new node, key:%V conn:1 last_changed:%M",
                          &key, now);
            client->prev = NULL;
            client->next = NULL;
        ...
        else
        {
            ...
            if (lc->snap == NULL)
            {
                ...
                lc->snap = client;
                njt_log_error(NJT_LOG_NOTICE, r->connection->log, 0, "init snap:%p,next:%p,%p", client, client->next, client->prev);
            
                njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0,
                          " ============snap is null, key:%V conn:%d last_changed:%M",
                          &key, lc->conn, now);
            }
            else
            {
                njt_log_error(NJT_LOG_ERR, njt_cycle->log, 0,
                          " ============snap is not null, key:%V conn:%d last_changed:%M",
                          &key, lc->conn, now);
        ...
        lccln->node = node;
        njt_log_error(NJT_LOG_DEBUG, r->connection->log, 0, "set node to cleanup:%p,%p", node, &node->color);
...
njt_http_cluster_limit_conn_cleanup(void *data)
{
    ...
    njt_log_error(NJT_LOG_DEBUG, lccln->shm_zone->shm.log, 0, "node in cleanup:%p,%p", node, lc);

    njt_shmtx_lock(&ctx->shpool->mutex);

    njt_log_debug2(NJT_LOG_DEBUG_HTTP, lccln->shm_zone->shm.log, 0,
                   "limit conn cleanup: %08Xi %d", node->key, lc->conn);
    njt_log_debug2(NJT_LOG_ERR, lccln->shm_zone->shm.log, 0,
                   "limit conn cleanup: %08Xi %d", node->key, lc->conn);
    lc->conn--;
    lc->snap->q_item.sibling_item.conn = lc->conn;
