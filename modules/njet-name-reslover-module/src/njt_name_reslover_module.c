...
#endif
static njt_int_t njt_name_reslover_init_process(
    njt_cycle_t *cycle);
...
static njt_stream_upstream_rr_peer_t *
njt_stream_upstream_zone_init_parent_peer(njt_stream_upstream_rr_peers_t *peers,
...
static njt_command_t njt_name_reslover_commands[] = {
    njt_null_command
};

static njt_http_module_t njt_name_reslover_module_ctx = {
...
njt_module_t njt_name_reslover_module = {
    NJT_MODULE_V1,
    &njt_name_reslover_module_ctx,  /* module context */
    njt_name_reslover_commands,     /* module directives */
    NJT_HTTP_MODULE,                /* module type */
    NULL,                           /* init master */
    NULL,                           /* init module */
    njt_name_reslover_init_process, /* init process */
    NULL,                           /* init thread */
    NULL,                           /* exit thread */
    NULL,                           /* exit process */
    NULL,                           /* exit master */
...
static njt_int_t njt_name_reslover_init_process_stream(
    njt_cycle_t *cycle)
{
...
static njt_int_t njt_name_reslover_init_process_http(
    njt_cycle_t *cycle)
{
    ...
    if(conf_ctx == NULL) {
         njt_log_debug(NJT_LOG_DEBUG_CORE, cycle->log, 0,
                  "start http name_reslover fail! row=%d",__LINE__);
        return NJT_OK;
    }
#if (NJT_HTTP_ADD_DYNAMIC_UPSTREAM)
...
{
    njt_conf_ext_t *mcf;
    mcf = (njt_conf_ext_t *) njt_get_conf(cycle->conf_ctx, njt_conf_ext_module);
    if (njet_master_cycle == NULL && (mcf->enabled == 0 || mcf->enabled == NJT_CONF_UNSET))
    {
        return NJT_OK;
    } else if (njet_master_cycle != NULL && mcf->enabled == 0) {
        return NJT_OK;
    }
    if (njet_master_cycle == NULL) {
    ...
    njt_name_reslover_init_process_http(cycle);
    njt_name_reslover_init_process_stream(cycle);
    return NJT_OK;
}
            ...
            if (dynamic_server[i].timer.timer_set)
            {
                njt_del_timer(&dynamic_server[i].timer);
                  ...
                  njt_log_debug(NJT_LOG_DEBUG_CORE, njt_cycle->log, 0,
                      "del name_reslover=%V",&dynamic_server[i].host);
            }
...
static void njt_http_upstream_dynamic_server_resolve(njt_event_t *ev)
{
    ...
    njt_http_upstream_server_t *us;
     njt_log_debug(NJT_LOG_DEBUG_CORE, ev->log, 0,
                      "njt_http_upstream_dynamic_server_resolve");
...
operation:
    ...
    if (peers->shpool)
    {
        ...
        for (peer = peers_data->peer, prev = NULL; peer; peer = next)
        {
            ...
            next = peer->next;
            if (njt_strncmp(peer->server.data, name.data, name.len) != 0 || njt_inet_get_port(peer->sockaddr) != port)
            {
                prev = peer;
                continue;
            }
            ...
            else
            {
                ...
                if (us->peer.ups_srv_handlers != NULL && us->peer.ups_srv_handlers->update_handler)
                {
                    us->peer.ups_srv_handlers->del_handler(peers->shpool, peer);
                }
        ...
        if (rc != NJT_ERROR && dynamic_server->parent_node->parent_id != -1)
        {
            ...
            for (i = 0; i < naddrs; ++i)
            {
                for (peer = peers_data->peer; peer; peer = peer->next)
                {
                    ...
                    if (njt_cmp_sockaddr(peer->sockaddr, peer->socklen, ctx->addrs[i].sockaddr,
                                         ctx->addrs[i].socklen, 1) == 0 &&
                        peer->parent_id == (njt_int_t)dynamic_server->parent_node->id)
                    {
                        goto skip_add;
                    }
                }
                ...
                else
                {
                    ...
                    if (us->peer.ups_srv_handlers != NULL && us->peer.ups_srv_handlers->update_handler)
                    {
                        us->peer.ups_srv_handlers->add_handler(peers->shpool, peer,dynamic_server->parent_node->app_data);
                    }
                }
...
void njt_http_upstream_notice_name_reslover(njt_http_upstream_srv_conf_t *uscf, njt_http_upstream_rr_peer_t *peer)
{
    ...
    njt_log_error(NJT_LOG_DEBUG, njt_cycle->log, 0,
                  "get a http domain message id=%d,parent_id=%d,name=%V,zone=%V!", peer->id, peer->parent_id, &peer->server, &peer->name);
    if (peer->parent_id == (njt_int_t)peer->id && peer->server.data == 0)
    { // patch
        njt_http_upstream_modify_dynamic_server(upstream_conf, peer,0);
        return;
    }
    else if (peer->parent_id != (njt_int_t)peer->id)
    { // delete
        njt_http_upstream_free_dynamic_server(upstream_conf, peer->server, peer->id,0);
        return; //
    }
    else
    {
        ...
        if (dynamic_server->server == NULL)
        { // new allocte, or reuse
        ...
        us->route.len = peer->route.len;
        if (us->route.len != 0 && us->route.data == NULL)
        {
            njt_pfree(uscf->pool, us->name.data);
            return;
        }
        njt_memcpy(us->name.data, peer->server.data, peer->server.len);
        njt_memcpy(us->route.data, peer->route.data, peer->route.len);
        ...
        if (parent_node == NULL)
        {
            ...
            if (us->route.data != NULL)
            {
                njt_pfree(uscf->pool, us->route.data);
            }
            return;
        }
        ...
        {
            if (njet_master_cycle != NULL && upstream_conf->resolver->log != njt_cycle->log)
            {
                upstream_conf->resolver->log = njt_cycle->log;
            }
            timer->handler = njt_http_upstream_dynamic_server_resolve;
                                                  ...
                                                  njt_str_t server, njt_int_t id,njt_int_t lock)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (p->upstream_conf == upstream_conf && id == (njt_int_t)p->parent_node->id)
        {
            ...
            {
                njt_pfree(dynamic_server->upstream_conf->pool, p->server->name.data);
                p->server->name.len = 0;
                p->server->name.data = 0;
            }
            if (p->server->route.len > 0)
            {
                njt_pfree(dynamic_server->upstream_conf->pool, p->server->route.data);
...
static void njt_stream_upstream_dynamic_server_resolve(njt_event_t *ev)
{
    ...
    if (dynamic_server->parent_node == NULL)
    {
        dynamic_server->parent_node = njt_stream_upstream_zone_copy_parent_peer(peers, &us->name, 0);
        if (dynamic_server->parent_node == NULL)
        {
            njt_log_error(NJT_LOG_ALERT, ev->log, 0,
                          "allocate njt_stream_upstream_zone_copy_parent_peer error for '%V'",
                          &dynamic_server->host);
            return;
        }
        dynamic_server->parent_node->id = us->parent_id;
    ...
    if (peers->shpool)
    {
        ...
        if (rc != NJT_ERROR && dynamic_server->parent_node->parent_id != -1)
        {
            ...
            for (i = 0; i < naddrs; ++i)
            {
                ...
                peer = njt_stream_upstream_zone_copy_peer(peers, NULL, &dynamic_server->parent_node->server, port,
                                                    ...
                                                    njt_str_t server, njt_int_t id,njt_int_t lock)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (p->upstream_conf == upstream_conf && id == (njt_int_t)p->parent_node->id)
        {
            ...
            if (p->server->name.len > 0)
            {
                njt_pfree(upstream_conf->pool, p->server->name.data);
                p->server->name.len = 0;
                p->server->name.data = 0;
            }
            p->upstream_conf = NULL;
...
void njt_stream_upstream_notice_name_reslover(njt_stream_upstream_srv_conf_t *uscf, njt_stream_upstream_rr_peer_t *peer)
{
    ...
    upstream_conf = uscf;
    njt_log_debug(NJT_LOG_DEBUG_CORE, njt_cycle->log, 0,
                  "get a stream domain message id=%d,parent_id=%d,name=%V,zone=%V!", peer->id, peer->parent_id, &peer->server, &peer->name);
    if (peer->parent_id == (njt_int_t)peer->id && peer->server.data == 0)
    { // patch
        njt_stream_upstream_modify_dynamic_server(upstream_conf, peer,0);
        return; //
    }
    else if (peer->parent_id != (njt_int_t)peer->id)
    { // delete
        njt_stream_upstream_free_dynamic_server(upstream_conf, peer->server, peer->id,0);
        return; //
    }
    else
    {
        ...
        if (parent_node == NULL)
        { // reuse
            parent_node = njt_stream_upstream_zone_copy_parent_peer(upstream_conf->peer.data, &us->name, 0);
        }
        else
        {
            parent_node = njt_stream_upstream_zone_init_parent_peer(upstream_conf->peer.data, &us->name, parent_node);
        }
        if (parent_node == NULL)
        {
            return;
...
static void njt_stream_upstream_check_dynamic_server(njt_event_t *ev)
{
	...
	njt_stream_upstream_srv_conf_t                  *uscf;
	njt_uint_t                                    refresh_in;
	 udsmcf = njt_stream_cycle_get_module_main_conf(njt_cycle,
             njt_stream_upstream_dynamic_servers_module);
