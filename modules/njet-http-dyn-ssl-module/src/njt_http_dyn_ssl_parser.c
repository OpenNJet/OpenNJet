...
static bool parse_dyn_ssl_servers_item_listens(njt_pool_t *pool, parse_state_t *parse_state, dyn_ssl_servers_item_listens_t *out, js2c_parse_error_t *err_ret) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        ((&((dyn_ssl_servers_item_listens_item_t*)out->elts)[i]))->data = (u_char*)njt_palloc(pool, (size_t)(token_size + 1));
...
static bool parse_dyn_ssl_servers_item_serverNames(njt_pool_t *pool, parse_state_t *parse_state, dyn_ssl_servers_item_serverNames_t *out, js2c_parse_error_t *err_ret) {
    ...
    for (i = 0; i < n; ++i) {
        int token_size =  CURRENT_STRING_LENGTH(parse_state) ;
        ((&((dyn_ssl_servers_item_serverNames_item_t*)out->elts)[i]))->data = (u_char*)njt_palloc(pool, (size_t)(token_size + 1));
