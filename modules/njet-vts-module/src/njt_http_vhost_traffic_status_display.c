...
#include "njt_http_vhost_traffic_status_module_html.h"
...
static njt_int_t njt_http_vhost_traffic_status_display_handler(njt_http_request_t *r);
static njt_int_t njt_http_vhost_traffic_status_display_handler_control(njt_http_request_t *r);
static njt_int_t njt_http_vhost_traffic_status_display_handler_default(njt_http_request_t *r);


static njt_int_t
njt_http_vhost_traffic_status_display_handler(njt_http_request_t *r)
{
    size_t                                     len;
    u_char                                    *p;
    njt_int_t                                  rc;
    ...
    if (p && len >= sizeof("/control") - 1) {
        ...
        if (njt_strncasecmp(p, (u_char *) "/control", sizeof("/control") - 1) == 0) {
            rc = njt_http_vhost_traffic_status_display_handler_control(r);
            goto done;
        }
    }

    /* default processing handler */
    rc = njt_http_vhost_traffic_status_display_handler_default(r);
...
static njt_int_t
njt_http_vhost_traffic_status_display_handler_control(njt_http_request_t *r)
{
    njt_int_t                                  size, rc;
    ...
    if (r->args.len) {
        ...
        if (njt_http_arg(r, (u_char *) "cmd", 3, &arg_cmd) == NJT_OK) {
            ...
            {
                control->command = NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_DELETE;
            }
            else if (arg_cmd.len == 5 && njt_strncmp(arg_cmd.data, "reset", 5) == 0)
            {
                control->command = NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_RESET;
            }
            else
            {
                control->command = NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_NONE;
        ...
        njt_http_vhost_traffic_status_node_control_range_set(control);
    }

    if (control->command == NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_STATUS) {
        size = njt_http_vhost_traffic_status_display_get_size(r,
        ...
        size = sizeof(NJT_HTTP_VHOST_TRAFFIC_STATUS_JSON_FMT_CONTROL)
               + arg_cmd.len + arg_group.len + arg_zone.len + njt_pagesize;
    }
    ...
    case NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_STATUS:
        njt_http_vhost_traffic_status_node_status(control);
        break;

    case NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_DELETE:
        njt_http_vhost_traffic_status_node_delete(control);
        break;

    case NJT_HTTP_VHOST_TRAFFIC_STATUS_CONTROL_CMD_RESET:
        njt_http_vhost_traffic_status_node_reset(control);
        break;

    default:
        *control->buf = njt_sprintf(*control->buf,
                                    NJT_HTTP_VHOST_TRAFFIC_STATUS_JSON_FMT_CONTROL,
                                    njt_http_vhost_traffic_status_boolean_to_string(0),
...
static njt_int_t
njt_http_vhost_traffic_status_display_handler_default(njt_http_request_t *r)
{
    size_t                                     len;
    u_char                                    *o, *s, *p;
    size_t                                     olen;
    ...
    format = NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_NONE;
    ...
    while(sizeof("/format/type") - 1 <= len) {
        if (njt_strncasecmp(s, (u_char *) "/format/", sizeof("/format/") - 1) == 0) {
            ...
            if (njt_strncasecmp(s, (u_char *) "jsonp", sizeof("jsonp") - 1) == 0) {
                format = NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_JSONP;

            } else if (njt_strncasecmp(s, (u_char *) "json", sizeof("json") - 1) == 0) {
                format = NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_JSON;

            } else if (njt_strncasecmp(s, (u_char *) "html", sizeof("html") - 1) == 0) {
                format = NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_HTML;

            } else if (njt_strncasecmp(s, (u_char *) "prometheus", sizeof("prometheus") - 1) == 0) {
                format = NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_PROMETHEUS;

            } else {
                s -= 2;
            }

            if (format != NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_NONE) {
                break;
            }
        }
    ...
    format = (format == NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_NONE) ? vtscf->format : format;
    ...
    size = njt_http_vhost_traffic_status_display_get_size(r, format);
    if (size == NJT_ERROR) {
        njt_log_error(NJT_LOG_ERR, r->connection->log, 0,
...
njt_int_t
njt_http_vhost_traffic_status_display_get_upstream_nelts(njt_http_request_t *r)
{
    njt_uint_t                      i, j, n;
...
njt_int_t
njt_http_vhost_traffic_status_display_get_size(njt_http_request_t *r,
    njt_int_t format)
{
    njt_uint_t                                 size, un;
    njt_slab_pool_t                           *shpool;
    ...
    un = shm_info->used_node
         + (njt_uint_t) njt_http_vhost_traffic_status_display_get_upstream_nelts(r);

    size = 0;

    switch (format) {

    case NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_JSON:
    case NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_JSONP:
    case NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_PROMETHEUS:
        size = sizeof(njt_http_vhost_traffic_status_node_t) / NJT_PTR_SIZE
               * NJT_ATOMIC_T_LEN * un  /* values size */
               + (un * 1024)            /* names  size */
               + 4096;                  /* main   size */
        break;

    case NJT_HTTP_VHOST_TRAFFIC_STATUS_FORMAT_HTML:
        size = sizeof(NJT_HTTP_VHOST_TRAFFIC_STATUS_HTML_DATA) + njt_pagesize;
        break;
    }
...
u_char *
njt_http_vhost_traffic_status_display_get_time_queue(
...
u_char *
njt_http_vhost_traffic_status_display_get_time_queue_times(
    njt_http_request_t *r,
    njt_http_vhost_traffic_status_node_time_queue_t *q)
{
    return njt_http_vhost_traffic_status_display_get_time_queue(r, q,
               offsetof(njt_http_vhost_traffic_status_node_time_t, time));
}


u_char *
njt_http_vhost_traffic_status_display_get_time_queue_msecs(
    njt_http_request_t *r,
    njt_http_vhost_traffic_status_node_time_queue_t *q)
{
    return njt_http_vhost_traffic_status_display_get_time_queue(r, q,
               offsetof(njt_http_vhost_traffic_status_node_time_t, msec));
}

    
u_char *
njt_http_vhost_traffic_status_display_get_histogram_bucket(
    njt_http_request_t *r,
...
u_char *
njt_http_vhost_traffic_status_display_get_histogram_bucket_msecs(
    njt_http_request_t *r,
    njt_http_vhost_traffic_status_node_histogram_bucket_t *b)
{
    return njt_http_vhost_traffic_status_display_get_histogram_bucket(r, b,
               offsetof(njt_http_vhost_traffic_status_node_histogram_t, msec), "%M,");
}


u_char *
njt_http_vhost_traffic_status_display_get_histogram_bucket_counters(
    njt_http_request_t *r,
    njt_http_vhost_traffic_status_node_histogram_bucket_t *b)
{
    return njt_http_vhost_traffic_status_display_get_histogram_bucket(r, b,
               offsetof(njt_http_vhost_traffic_status_node_histogram_t, counter), "%uA,");
}


char *
njt_http_vhost_traffic_status_display(njt_conf_t *cf, njt_command_t *cmd, void *conf)
{
    njt_http_core_loc_conf_t  *clcf;

    clcf = njt_http_conf_get_module_loc_conf(cf, njt_http_core_module);
    clcf->handler = njt_http_vhost_traffic_status_display_handler;
