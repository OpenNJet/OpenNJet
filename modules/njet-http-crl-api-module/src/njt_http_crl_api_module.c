...
static void
njt_http_dyn_crl_read_data(njt_http_request_t *r);

static njt_int_t
njt_http_dyn_crl_handler(njt_http_request_t *r);

static njt_int_t
njt_http_dyn_crl_init_worker(njt_cycle_t *cycle);

static void *
njt_http_dyn_crl_create_main_conf(njt_conf_t *cf);
...
static njt_int_t njt_http_dyn_crl_rpc_send(njt_http_request_t *r,njt_str_t *module_name,njt_str_t *msg, int retain);

static int njt_http_dyn_crl_request_output(njt_http_request_t *r,njt_int_t code, njt_str_t *msg);

typedef struct njt_http_dyn_crl_ctx_s {
} njt_http_dyn_crl_ctx_t, njt_stream_http_dyn_crl_ctx_t;


typedef struct njt_http_dyn_crl_main_conf_s {  //njt_http_dyn_crl_main_cf_t
	njt_http_request_t **reqs;
    njt_int_t size;
} njt_http_dyn_crl_main_conf_t;


typedef struct {
    njt_http_request_t *req;
    njt_int_t index;
    njt_http_dyn_crl_main_conf_t *dlmcf;
}njt_http_dyn_crl_rpc_ctx_t;
...
typedef struct {
    ...
    void* data;
    unsigned success:1;
}njt_http_crl_request_err_ctx_t;

static njt_http_module_t njt_http_crl_api_module_ctx = {
        NULL,                              /* preconfiguration */
        njt_http_dyn_crl_init,             /* postconfiguration */

        njt_http_dyn_crl_create_main_conf, /* create main configuration */
...
njt_module_t njt_http_crl_api_module = {
        NJT_MODULE_V1,
        &njt_http_crl_api_module_ctx, /* module context */
        NULL,                           /* module directives */
        NJT_HTTP_MODULE,                    /* module type */
        NULL,                               /* init master */
        NULL,                               /* init module */
        njt_http_dyn_crl_init_worker, /* init process */
        NULL,                               /* init thread */
        NULL,                               /* exit thread */
        NULL,                               /* exit process */
        NULL,                               /* exit master */
...
njt_http_dyn_crl_init(njt_conf_t *cf) {
    ...
    dlmcf = njt_http_conf_get_module_main_conf(cf,njt_http_crl_api_module);
    if(dlmcf == NULL){
        return NJT_ERROR;
    }
    ...
    if(dlmcf->reqs == NULL){
        njt_log_error(NJT_LOG_EMERG, njt_cycle->log, 0, "njt_http_dyn_crl_postconfiguration alloc mem error");
        return NJT_ERROR;
    }

    njt_str_t  module_key = njt_string("/v1/crl");
    njt_memzero(&h, sizeof(njt_http_api_reg_info_t));
    h.key = &module_key;
    h.handler = njt_http_dyn_crl_handler;
...
njt_http_dyn_crl_create_main_conf(njt_conf_t *cf) {
    ...
    njt_http_dyn_crl_main_conf_t *uclcf;

    //size = (ssize_t)(10 * njt_pagesize);
    uclcf = njt_pcalloc(cf->pool, sizeof(njt_http_dyn_crl_main_conf_t));
    if (uclcf == NULL) {
        njt_log_error(NJT_LOG_ERR, cf->log, 0, "malloc njt_http_dyn_crl_main_conf_t eror");
        return NULL;
    }
...
njt_http_dyn_crl_handler(njt_http_request_t *r) {
    ...
    if(r->method == NJT_HTTP_PUT){
        rc = njt_http_read_client_request_body(r, njt_http_dyn_crl_read_data);
    ...
    if(r->method == NJT_HTTP_GET){
        ...
        njt_str_set(&topic, "/worker_a/rpc/crl");

        rc = njt_http_dyn_crl_rpc_send(r, &topic, &smsg, 0);
        if(rc != NJT_OK){
            goto err;
        }
        ++r->main->count;
    ...
    err:
    return njt_http_dyn_crl_request_output(r, NJT_HTTP_INTERNAL_SERVER_ERROR, &srv_err);
}
...
static njt_int_t njt_http_dyn_crl_get_free_index(njt_http_dyn_crl_main_conf_t *dlmcf){
    njt_int_t i;
...
static void njt_http_dyn_crl_cleanup_handler(void *data){
    njt_http_dyn_crl_rpc_ctx_t *ctx;
...
static int njt_http_dyn_crl_rpc_msg_handler(njt_dyn_rpc_res_t* res, njt_str_t *msg){
    njt_http_dyn_crl_rpc_ctx_t          *ctx;
    njt_http_request_t                  *req;
    njt_int_t                           rc;
    ...
    if( ctx->dlmcf->size > ctx->index && ctx->dlmcf->reqs[ctx->index] == ctx->req){
        req =  ctx->req;
        if(res->rc == RPC_RC_OK){
            rc = njt_http_dyn_crl_request_output(req,NJT_OK,msg);
        }
        if(res->rc == RPC_RC_TIMEOUT){
            rc = njt_http_dyn_crl_request_output(req,NJT_HTTP_INTERNAL_SERVER_ERROR,&err_msg);
        }
...
static njt_int_t njt_http_dyn_crl_rpc_send(njt_http_request_t *r,njt_str_t *module_name,njt_str_t *msg, int retain){
    ...
    dlmcf = njt_http_get_module_main_conf(r,njt_http_crl_api_module);
    if(dlmcf == NULL){
        goto err;
    }
    index = njt_http_dyn_crl_get_free_index(dlmcf);
    if(index == -1 ){
    ...
    ctx = njt_pcalloc(r->pool, sizeof(njt_http_dyn_crl_rpc_ctx_t));
    if(ctx == NULL){
    ...
    cleanup->handler = njt_http_dyn_crl_cleanup_handler;
    cleanup->data = ctx;
    njt_log_error(NJT_LOG_INFO, r->pool->log, 0, "send rpc time : %M",njt_current_msec);
    rc = njt_dyn_rpc(module_name,msg, retain, index, njt_http_dyn_crl_rpc_msg_handler, ctx);
    if(rc == NJT_OK){
        dlmcf->reqs[index] = r;
    }
    return rc;
...
njt_http_dyn_crl_read_data(njt_http_request_t *r){
    ...
    uint32_t                            crc32;
    uint32_t						    topic_len = NJT_INT64_LEN  + 2 + 256; ///ins/crl/l_
    njt_str_t							topic_name;
    njt_http_crl_request_err_ctx_t      *err_ctx;
    ...
    api_data = json_parse_dyn_crl_api(pool, &json_str, &err_info);
    if (api_data == NULL)
    {
        njt_log_error(NJT_LOG_ERR, pool->log, 0, 
                "json_parse_dyn_crl err: %V",  &err_info.err_str);
    ...
    //check format
    if(!api_data->is_ssl_crl_set || api_data->ssl_crl.len < 1 ){
        njt_log_debug1(NJT_LOG_DEBUG_HTTP, pool->log, 0,
                       "ssl_crl is empty in function %s", __func__);
        njt_rpc_result_set_code(rpc_result,NJT_RPC_RSP_ERR_JSON);
        njt_rpc_result_set_msg(rpc_result, (u_char *)" ssl_crl is empty");
        rc = NJT_ERROR;
	...
	for (i = 0; i < api_data->serverNames->nelts; i++){
        serverName = get_dyn_crl_api_serverNames_item(api_data->serverNames, i);
        if(serverName->len > 0){
	...
	for(i = 0; i < api_data->listens->nelts; i++){
        listen_str = get_dyn_crl_api_listens_item(api_data->listens, i);
        if(listen_str->len > 0){
            njt_crc32_update(&crc32, listen_str->data, listen_str->len);
        }
	}

    njt_crc32_update(&crc32, api_data->ssl_crl.data, api_data->ssl_crl.len);
	...
	njt_log_error(NJT_LOG_INFO, r->connection->log, 0,
                      " dyn crl, crc32:%ui", crc32);    

    p = njt_snprintf(topic_name.data,topic_len,"/worker_a/ins/crl/l_%ui",crc32);

	topic_name.len = p - topic_name.data;
	rc = njt_http_dyn_crl_rpc_send(r, &topic_name, &json_str, 0);
	if(rc == NJT_OK) {
		++r->main->count;
	}
...
err:
    err_ctx = njt_pcalloc(r->pool, sizeof(njt_http_crl_request_err_ctx_t));
    err_ctx->success = 0;
    err_ctx->code = rc;
    njt_http_set_ctx(r, err_ctx, njt_http_crl_api_module);
    ...
    njt_http_dyn_crl_request_output(r, NJT_HTTP_BAD_REQUEST, &bad_req);
