...
njt_int_t cluster_limit_req_init_first_worker(njt_cycle_t *cycle)
{
    ...
    njt_gossip_reg_app_handler(njt_cluster_limit_req_recv_data, njt_cluster_limit_req_on_node_on, 
        NULL, NULL, GOSSIP_APP_CLUSTER_LIMIT_REQ, clreq_ctxes);
    ...
    for (idx = 0; idx < out_arr.nelts; )
    {
		if (head == NULL ) {
			...
			if (buf_size <=0 || buf == NULL) {
				njt_log_error(NJT_LOG_DEBUG,njt_cycle->log,0," cluster limit req apply buffer failed");
    			njt_shmtx_unlock(&ctx->shpool->mutex);
				return;
			}
