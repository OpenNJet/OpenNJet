...
static void to_oneline_json_accessLogFormat_escape(accessLogFormat_escape_t *out, njt_str_t* buf, njt_int_t flags) {
    ...
    char* cur = (char *)buf->data + buf->len;
    if (*out == ACCESSLOGFORMAT_ESCAPE_DEFAULT) {
        len = sprintf(cur, "\"default\"");
        buf->len += len;
        cur += len;
        return;
    }
    if (*out == ACCESSLOGFORMAT_ESCAPE_JSON) {
        len = sprintf(cur, "\"json\"");
        buf->len += len;
        cur += len;
        return;
    }
    if (*out == ACCESSLOGFORMAT_ESCAPE_NONE) {
        len = sprintf(cur, "\"none\"");
        buf->len += len;
        cur += len;
        return;
    }
...
static bool parse_dynlog_servers_item_listens(njt_pool_t *pool, parse_state_t *parse_state, dynlog_servers_item_listens_t *out, njt_str_t *err_str) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (((dynlog_servers_item_listens_item_t*)out->elts)[i]->data == NULL) {
            // TODO LOG_ERROR
            return true;
        }
        if (builtin_parse_string(pool, parse_state, (char *)(((dynlog_servers_item_listens_item_t*)out->elts)[i])->data, 0, (((dynlog_servers_item_listens_item_t*)out->elts)[i])->len, err_str)) {
            return true;
        }
        out->nelts ++;
    }
...
static bool parse_dynlog_servers_item_serverNames(njt_pool_t *pool, parse_state_t *parse_state, dynlog_servers_item_serverNames_t *out, njt_str_t *err_str) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (((dynlog_servers_item_serverNames_item_t*)out->elts)[i]->data == NULL) {
            // TODO LOG_ERROR
            return true;
        }
...
static void to_oneline_json_dynlog(dynlog_t *out, njt_str_t* buf, njt_int_t flags) {
    ...
    if (omit == 0) {
        len = sprintf(cur, "\"accessLogFormats\":");
        buf->len += len;
        cur += len;
