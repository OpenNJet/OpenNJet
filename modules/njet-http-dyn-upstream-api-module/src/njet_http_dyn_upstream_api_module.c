...
static void
njt_http_dyn_upstream_read_data(njt_http_request_t *r);

static njt_int_t
njt_http_dyn_upstream_handler(njt_http_request_t *r);

static njt_int_t
njt_http_dyn_upstream_init_worker(njt_cycle_t *cycle);

static void *
njt_http_dyn_upstream_create_loc_conf(njt_conf_t *cf);

static char *njt_http_dyn_upstream_merge_loc_conf(njt_conf_t *cf,
                                                  void *parent, void *child);

static void *
njt_http_dyn_upstream_create_main_conf(njt_conf_t *cf);

static njt_int_t
njt_http_dyn_upstream_init(njt_conf_t *cf);
...
{ // njt_http_dyn_upstream_main_cf_t
    ...
    njt_int_t size;
} njt_http_dyn_upstream_main_conf_t;

typedef struct
{
    njt_http_request_t *req;
    njt_int_t index;
    njt_http_dyn_upstream_main_conf_t *dlmcf;
} njt_http_dyn_upstream_rpc_ctx_t;

static njt_command_t njt_http_dyn_upstream_commands[] = {

    njt_null_command};

static njt_http_module_t njt_http_dyn_upstream_api_module_ctx = {
    NULL,                       /* preconfiguration */
    njt_http_dyn_upstream_init, /* postconfiguration */

    njt_http_dyn_upstream_create_main_conf, /* create main configuration */
    NULL,                                   /* init main configuration */

    NULL, /* create upstream configuration */
    NULL, /* merge upstream configuration */

    njt_http_dyn_upstream_create_loc_conf, /* create upstream configuration */
    njt_http_dyn_upstream_merge_loc_conf   /* merge upstream configuration */
};

njt_module_t njt_http_dyn_upstream_api_module = {
    NJT_MODULE_V1,
    &njt_http_dyn_upstream_api_module_ctx, /* module context */
...
njt_http_dyn_upstream_init(njt_conf_t *cf)
{
    ...
    dlmcf = njt_http_conf_get_module_main_conf(cf, njt_http_dyn_upstream_api_module);
    ...
    if (dlmcf->reqs == NULL)
    {
        njt_log_error(NJT_LOG_EMERG, njt_cycle->log, 0, "njt_http_dyn_upstream_postconfiguration alloc mem error");
        return NJT_ERROR;
    }

    njt_str_t module_key = njt_string("/v1/dyn_ups");
    njt_memzero(&h, sizeof(njt_http_api_reg_info_t));
    h.key = &module_key;
    h.handler = njt_http_dyn_upstream_handler;
...
njt_http_dyn_upstream_create_loc_conf(njt_conf_t *cf)
{
    ...
    njt_http_dyn_upstream_loc_conf_t *uclcf;
    // size = (ssize_t)(10 * njt_pagesize);
    uclcf = njt_pcalloc(cf->pool, sizeof(njt_http_dyn_upstream_loc_conf_t));
    if (uclcf == NULL)
    {
...
njt_http_dyn_upstream_create_main_conf(njt_conf_t *cf)
{
    ...
    njt_http_dyn_upstream_main_conf_t *uclcf;

    // size = (ssize_t)(10 * njt_pagesize);
    uclcf = njt_pcalloc(cf->pool, sizeof(njt_http_dyn_upstream_main_conf_t));
    if (uclcf == NULL)
    {
        njt_log_error(NJT_LOG_ERR, cf->log, 0, "malloc njt_http_dyn_upstream_main_conf_t eror");
        return NULL;
    }
...
static char *njt_http_dyn_upstream_merge_loc_conf(njt_conf_t *cf,
                                                  void *parent, void *child)
{
    njt_http_dyn_upstream_loc_conf_t *prev = parent;
    njt_http_dyn_upstream_loc_conf_t *conf = child;
...
static njt_buf_t *
njt_http_dyn_upstream_api_get_out_buf(njt_http_request_t *r, ssize_t len,
                                      njt_chain_t *out)
{
    njt_buf_t *b;
...
static njt_int_t
njt_http_dyn_upstream_api_insert_out_str(njt_http_request_t *r,
                                         njt_chain_t *out, njt_str_t *str)
{
    ...
    b = njt_http_dyn_upstream_api_get_out_buf(r, str->len, out);
    if (b == NULL)
    {
        njt_log_debug1(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
...
static ssize_t
njt_http_dyn_upstream_api_out_len(njt_chain_t *out)
{
    ssize_t len;
...
static int njt_http_dyn_upstream_request_output(njt_http_request_t *r, njt_int_t code, njt_str_t *msg)
{
    njt_int_t rc;
    njt_buf_t *buf;
    njt_chain_t out;
...
njt_http_dyn_upstream_handler(njt_http_request_t *r)
{
    ...
    {
        rc = njt_http_read_client_request_body(r, njt_http_dyn_upstream_read_data);
        // upstream_info = njt_http_get_module_ctx(r, njt_http_dyn_upstream_api_module);
...
out:
    ...
    return njt_http_dyn_upstream_request_output(r, rc, &msg);
err:

    return njt_http_dyn_upstream_request_output(r, NJT_HTTP_INTERNAL_SERVER_ERROR, &srv_err);
}
...
static njt_int_t njt_http_dyn_upstream_get_free_index(njt_http_dyn_upstream_main_conf_t *dlmcf)
{
    njt_int_t i;
...
static void njt_http_dyn_upstream_cleanup_handler(void *data)
{
    njt_http_dyn_upstream_rpc_ctx_t *ctx;
...
static int njt_http_dyn_upstream_rpc_msg_handler(njt_dyn_rpc_res_t *res, njt_str_t *msg)
{
    njt_http_dyn_upstream_rpc_ctx_t *ctx;
    njt_http_request_t *req;
    njt_int_t rc;
    ...
    if (ctx->dlmcf->size > ctx->index && ctx->dlmcf->reqs[ctx->index] == ctx->req)
    {
        ...
        {
            rc = njt_http_dyn_upstream_request_output(req, NJT_HTTP_INTERNAL_SERVER_ERROR, &err_msg);
        }
...
static njt_int_t njt_http_dyn_upstream_rpc_send(njt_http_request_t *r, njt_str_t *module_name, njt_str_t *msg, int retain)
{
    njt_http_dyn_upstream_main_conf_t *dlmcf;
    njt_int_t index;
    njt_int_t rc;
    ...
    dlmcf = njt_http_get_module_main_conf(r, njt_http_dyn_upstream_api_module);
    index = njt_http_dyn_upstream_get_free_index(dlmcf);
    if (index == -1)
    {
    ...
    ctx = njt_pcalloc(r->pool, sizeof(njt_http_dyn_upstream_rpc_ctx_t));
    if (ctx == NULL)
    {
        njt_log_debug1(NJT_LOG_DEBUG_HTTP, r->connection->log, 0,
    ...
    cleanup->handler = njt_http_dyn_upstream_cleanup_handler;
    cleanup->data = ctx;
    njt_log_error(NJT_LOG_INFO, r->pool->log, 0, "send rpc time : %M", njt_current_msec);
    rc = njt_dyn_rpc(module_name, msg, retain, index, njt_http_dyn_upstream_rpc_msg_handler, ctx);
    if (rc == NJT_OK)
    {
        dlmcf->reqs[index] = r;
    }
...
njt_http_dyn_upstream_read_data(njt_http_request_t *r)
{
    ...
    upstream_info = njt_http_parser_upstream_data(json_str, r->method);
    ...
    {
        p = njt_snprintf(topic_name.data, topic_len, "/worker_a/ins/ups/http_%V", &upstream_info->upstream_name);
    }
    ...
    rc = njt_http_dyn_upstream_rpc_send(r, &topic_name, &json_str, 0);
    if (rc == NJT_OK)
    {
        ++r->main->count;
    }
...
err:
    ...
    rc = njt_http_dyn_upstream_api_insert_out_str(r, &out, &insert);
    len = njt_http_dyn_upstream_api_out_len(&out);
