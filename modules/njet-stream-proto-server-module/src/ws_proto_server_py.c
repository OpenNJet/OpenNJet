...
int proto_server_process_message(tcc_stream_request_t *r, tcc_str_t *msg)
{
  ...
  //proto_server_log(NJT_LOG_DEBUG, "tcc proto_server_send_upstream msg=%V",msg);
  //proto_server_send_upstream(r,msg->data,msg->len);
  //r->used_len = msg->len;
  ...
  if (cli_ctx->handshake == 0)
  {
    ...
    data = cli_ctx->client.headers.buf;
      //proto_server_log(NJT_LOG_DEBUG, "1 tcc http error!");
    ...
    if (strstr(data, "\r\n\r\n") == NULL)
    {
     ...
     // proto_server_log(NJT_LOG_DEBUG, "2 tcc http error!");
    ...
    if (ws_verify_req_headers(&cli_ctx->client.headers) != 0)
    {
     ...
     // proto_server_log(NJT_LOG_DEBUG, "3 tcc http error!");
    ...
    rc = ws_app_on_connection(r, &message);
     // proto_server_log(NJT_LOG_DEBUG, "4 tcc http error!");
